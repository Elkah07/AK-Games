(function () {
  ...
  async function akCreatorLoadCloudReports() {
    ...
    if (!access.allowed) return { access, reports: [] };
  ...
  async function akCreatorDeleteReport(report) {
    ...
    if (report.cloudId) {
      const access = await akCreatorHasCloudAccess();
      if (access.allowed) await window.AKFirebase.db.ref(`contentReports/${report.cloudId}`).remove();
    }
  }
  ...
  async function akCreatorRenderDashboard() {
    ...
    screen.innerHTML = `
      ...
      <section class="ak-creator-dashboard-grid">
        <button type="button" class="ak-creator-tool-card" data-ak-open-inbox>
          <span>🚩</span><strong>Signalements</strong><small>${counts.open} à traiter · ${localReports.length} enregistrés localement</small>
        </button>
        <button type="button" class="ak-creator-tool-card" data-ak-open-browser>
          <span>🗃️</span><strong>Base de contenus</strong><small>Parcourir les questions, IDs et catégories</small>
        </button>
        <button type="button" class="ak-creator-tool-card" data-ak-copy-debug>
          <span>🩺</span><strong>Diagnostic</strong><small>Copier l’état actuel pour comprendre un bug</small>
        </button>
        <button type="button" class="ak-creator-tool-card" data-ak-reset-recents>
          <span>♻️</span><strong>Cartes récentes</strong><small>Réautoriser les questions vues récemment</small>
        </button>
        <button type="button" class="ak-creator-tool-card" data-ak-clear-cache>
          <span>🧹</span><strong>Forcer la mise à jour</strong><small>Vider le cache PWA et recharger le code</small>
        </button>
        <button type="button" class="ak-creator-tool-card" data-ak-lock-creator>
          <span>🔒</span><strong>Verrouiller</strong><small>Fermer les outils jusqu’au prochain code</small>
        </button>
      </section>
    ...
    screen.querySelector("[data-ak-reset-recents]")?.addEventListener("click", akCreatorClearRecentContent);
    screen.querySelector("[data-ak-clear-cache]")?.addEventListener("click", akCreatorClearCaches);
  ...
  async function akCreatorMountDock() {
    ...
    button.addEventListener("click", async () => {
      ...
      const values = await akCreatorModal({
        ...
        description: state.akCreatorLab ? `${state.players.length} joueurs virtuels sont actifs.` : "Ouvre le tableau de bord ou lance un test rapide.",
  ...
  async function akCreatorMountSettingsCard() {
    ...
    card.innerHTML = `
      <div><span>🛠️</span><div><strong>Mode créateur</strong><p>Tests express, signalements, navigateur JSON et diagnostic.</p></div></div>
  ...
  async function akCreatorMountHomeShortcut() {
    ...
    button.type = "button";
    button.innerHTML = `<span>🛠️</span><div><strong>Mode créateur actif</strong><small>Ouvrir le laboratoire et les signalements</small></div><b>›</b>`;
