...
const V014_GAME_CONFIGS = {
  ...
  "Imitation +18": { engine: "turn", icon: "🌶️", pack: "Pack adulte", data: "data/imitation-adulte.json", description: "Flirt, rendez-vous, couple et scènes intimes à jouer sans contact imposé.", defaultRounds: 10, privatePrompt: true, timer: 45, adultOnly: true },
  ...
  "La Bombe +18": { engine: "bomb", icon: "🌶️", pack: "Pack adulte", data: "data/bombe-adulte.json", description: "Confidences, séduction, ex, messages et chaos adulte avec filtres de confort.", defaultRounds: 8, timer: 25, adultOnly: true },
...
v014SetCategoryGames("adulte", ["Action ou Vérité +18", "Je n’ai jamais +18", "Tu préfères +18", "Imitation +18", "Questions osées", "Défis adultes", "Jeux à boire"]);
...
const AK_AUDIT8_GAME_META = {
  ...
  "Imitation +18": { minPlayers: 2, time: "10–15 min", goal: "Fais deviner une scène adulte de flirt, de couple ou d’intimité, sans contact obligatoire." },
    ...
    <section class="card mime-custom-card">
      ...
      <div class="mime-custom-form">
        ...
        <div class="mime-custom-grid">
          ...
          <div class="form-group"><label for="${prefix}CustomDifficulty">${adult ? "Intensité" : "Difficulté"}</label><select id="${prefix}CustomDifficulty" class="text-input">${Object.entries(akMimeDifficultyMeta).map(([id, meta]) => `<option value="${id}">${meta.icon} ${escapeHtml(meta.label)} · ${meta.points} pt${meta.points > 1 ? "s" : ""}</option>`).join("")}</select></div>
...
/* =========================================================
   AK'GAMES V3.1 - IMITATION
   600 cartes, themes, difficultes, sons, duos et cartes perso
   ========================================================= */

const AK_IMITATION_STORAGE_KEY = "akgames_custom_imitations_v1";
const AK_IMITATION_ADULT_STORAGE_KEY = "akgames_custom_imitations_adult_v1";
const AK_IMITATION_ADULT_GAME = "Imitation +18";

const akImitationAdultThemes = [
  { id: "flirt_seduction", icon: "💋", label: "Flirt & séduction" },
  { id: "rendez_vous", icon: "🍸", label: "Rendez-vous" },
  { id: "couple", icon: "💞", label: "Vie de couple" },
  { id: "ex_jalousie", icon: "👀", label: "Ex & jalousie" },
  { id: "messages_reseaux", icon: "📱", label: "Messages & réseaux" },
  { id: "soirees", icon: "🪩", label: "Soirées" },
  { id: "intimite", icon: "🛏️", label: "Intimité" },
  { id: "fantasmes_roleplay", icon: "🎭", label: "Fantasmes & jeux de rôle" },
  { id: "malaises_secrets", icon: "🫣", label: "Malaises & secrets" },
  { id: "duo_adulte", icon: "🫦", label: "Duos adultes" }
];

const akImitationAdultDifficultyMeta = {
  easy: { icon: "🌶️", label: "Léger", points: 1 },
  medium: { icon: "😏", label: "Osé", points: 2 },
  hard: { icon: "🔥", label: "Très osé", points: 3 },
  chaos: { icon: "☢️", label: "Chaos", points: 4 }
};

function akImitationIsAdult(value = state.megaGame) {
  if (typeof value === "boolean") return value;
  if (typeof value === "string") return value === AK_IMITATION_ADULT_GAME || value === "adult";
  return value?.gameName === AK_IMITATION_ADULT_GAME || value?.collection === "adult" || value?.imitationAdult === true;
}

function akImitationThemeList(value = state.megaGame) {
  return akImitationIsAdult(value) ? akImitationAdultThemes : akImitationThemes;
}

function akImitationDifficultyMap(value = state.megaGame) {
  return akImitationIsAdult(value) ? akImitationAdultDifficultyMeta : akImitationDifficultyMeta;
}

function akImitationCardTotal(value = state.megaGame) {
  return akImitationIsAdult(value) ? 300 : 600;
}

function akImitationDataPath(value = state.megaGame) {
  return akImitationIsAdult(value) ? "data/imitation-adulte.json" : "data/imitation.json";
}

function akImitationCustomStorageKey(value = state.megaGame) {
  return akImitationIsAdult(value) ? AK_IMITATION_ADULT_STORAGE_KEY : AK_IMITATION_STORAGE_KEY;
}

const akImitationThemes = [
  { id: "animaux", icon: "🐾", label: "Animaux" },
  { id: "metiers", icon: "🎭", label: "Métiers" },
  { id: "sports", icon: "🏟️", label: "Sports" },
  { id: "quotidien", icon: "🏠", label: "Vie quotidienne" },
  { id: "attitudes", icon: "😏", label: "Attitudes & émotions" },
  { id: "culture_pop", icon: "🎬", label: "Films, séries & personnages" },
  { id: "musique_scene", icon: "🎤", label: "Musique & scène" },
  { id: "objets_machines", icon: "🤖", label: "Objets & machines" },
  { id: "voyages", icon: "✈️", label: "Voyages" },
  { id: "situations", icon: "🚨", label: "Situations" },
  { id: "absurde_chaos", icon: "🌀", label: "Absurde & chaos" },
  { id: "duo", icon: "🧑‍🤝‍🧑", label: "Imitations en duo" }
];

const akImitationDifficultyMeta = {
  easy: { icon: "🌱", label: "Facile", points: 1 },
  medium: { icon: "⚡", label: "Moyen", points: 2 },
  hard: { icon: "🔥", label: "Difficile", points: 3 },
  chaos: { icon: "☢️", label: "Chaos", points: 4 }
...
function akImitationThemeMeta(id, context = null) {
  const themes = akImitationThemeList(context);
  return themes.find(theme => theme.id === id) || themes[0];
}

function akImitationDifficulty(itemOrValue) {
  const value = typeof itemOrValue === "string" ? itemOrValue : itemOrValue?.difficulty;
  return akImitationDifficultyMeta[value] ? value : "easy";
}

function akImitationPoints(item) {
  return akImitationDifficultyMap(item)[akImitationDifficulty(item)].points;
}

function akImitationPackId(item) {
  if (Number(item?.actors || 1) === 2) return akImitationIsAdult(item) ? "duo_adulte" : "duo";
  return item?.category || (akImitationIsAdult(item) ? "flirt_seduction" : "quotidien");
}

function akImitationLoadCustom(context = state.megaGame) {
  try {
    const parsed = JSON.parse(localStorage.getItem(akImitationCustomStorageKey(context)) || "[]");
    return Array.isArray(parsed) ? parsed.filter(item => item && item.text) : [];
  } catch (error) {
    console.warn("Imitations personnalisées illisibles", error);
    return [];
  }
}

function akImitationSaveCustom(items, context = state.megaGame) {
  localStorage.setItem(akImitationCustomStorageKey(context), JSON.stringify(Array.isArray(items) ? items : []));
}

function akImitationNormalizeThemes(values, context = state.megaGame) {
  const themes = akImitationThemeList(context);
  const valid = new Set(themes.map(theme => theme.id));
  if (!Array.isArray(values)) return themes.map(theme => theme.id);
  return [...new Set(values.filter(value => valid.has(value)))];
}

function akImitationNormalizeDifficulties(values, context = state.megaGame) {
  const difficultyMap = akImitationDifficultyMap(context);
  const valid = new Set(Object.keys(difficultyMap));
  if (!Array.isArray(values)) return Object.keys(difficultyMap);
  return [...new Set(values.filter(value => valid.has(value)))];
}

function ensureImitationGameConfig(game = state.megaGame) {
  if (!game || !["Imitation", AK_IMITATION_ADULT_GAME].includes(game.gameName)) return game;
  game.imitationAdult = akImitationIsAdult(game);
  game.roundCount = Math.max(3, Math.min(100, Number(game.roundCount || 10)));
  game.durationSeconds = [20, 30, 45, 60, 90].includes(Number(game.durationSeconds)) ? Number(game.durationSeconds) : 45;
  game.imitationThemes = akImitationNormalizeThemes(game.imitationThemes, game);
  if (state.players.length < 3) game.imitationThemes = game.imitationThemes.filter(theme => !["duo", "duo_adulte"].includes(theme));
  game.imitationDifficulties = akImitationNormalizeDifficulties(game.imitationDifficulties, game);
  game.imitationAllowSounds = game.imitationAllowSounds !== false;
  game.imitationIncludeCustom = game.imitationIncludeCustom !== false;
  game.imitationStage = game.imitationStage || "handoff";
  game.imitationStats = game.imitationStats || {
    actorSuccess: Object.fromEntries(state.players.map(player => [player.id, 0])),
    guesserSuccess: Object.fromEntries(state.players.map(player => [player.id, 0])),
    fastest: null
  };
...
function akImitationBadges(item) {
  const theme = akImitationThemeMeta(akImitationPackId(item), item);
  const difficulty = akImitationDifficultyMap(item)[akImitationDifficulty(item)];
  const actors = Number(item?.actors || 1);
  const sound = item?.soundMode === "required" ? "🔊 Son requis" : "🎙️ Son facultatif";
  return `<div class="mime-badge-row imitation-badge-row"><span>${theme.icon} ${escapeHtml(theme.label)}</span><span>${difficulty.icon} ${escapeHtml(difficulty.label)} · ${difficulty.points} pt${difficulty.points > 1 ? "s" : ""}</span><span>${actors === 2 ? "🧑‍🤝‍🧑 Duo" : "🧍 Solo"}</span><span>${sound}</span>${item?.custom ? `<span>✍️ Perso</span>` : ""}</div>`;
}

function akImitationConstraintMarkup(item, allowSounds = true) {
  const words = Array.isArray(item?.forbiddenWords) ? item.forbiddenWords.filter(Boolean) : [];
  return `<div class="imitation-constraint-stack">
    <div><span>${allowSounds ? "🎙️" : "🤫"}</span><p><strong>${allowSounds ? "Voix et sons autorisés" : "Mode silencieux"}</strong><small>${allowSounds ? "Tu peux parler et bruiter, mais jamais dire la réponse." : "Gestes, expressions et attitudes seulement."}</small></p></div>
    ${words.length ? `<div><span>🚫</span><p><strong>Mots interdits</strong><small>${words.map(word => escapeHtml(word)).join(" · ")}</small></p></div>` : ""}
    ${item?.twist ? `<div><span>🌀</span><p><strong>Contrainte Chaos</strong><small>${escapeHtml(item.twist)}</small></p></div>` : ""}
  </div>`;
}

function akImitationCustomSetupMarkup(game, prefix = "imitation") {
  const custom = akImitationLoadCustom(game);
  const themes = akImitationThemeList(game);
  const difficultyMap = akImitationDifficultyMap(game);
  const adult = akImitationIsAdult(game);
  const soloThemes = themes.filter(theme => !["duo", "duo_adulte"].includes(theme.id));
  return `
    <section class="card mime-custom-card imitation-custom-card">
      <div class="mime-section-heading"><div><small>TES PROPRES IDÉES</small><h3>Imitations personnalisées</h3></div><span>${custom.length}</span></div>
      <label class="option-card premium-toggle"><input id="${prefix}IncludeCustom" type="checkbox" ${game.imitationIncludeCustom ? "checked" : ""}><span><strong>Inclure mes cartes</strong><br><span class="helper">Elles restent enregistrées sur cet appareil.</span></span></label>
      <div class="mime-custom-form">
        <div class="form-group"><label for="${prefix}CustomText">Sujet à faire deviner</label><textarea id="${prefix}CustomText" class="text-input" rows="3" maxlength="260" placeholder="${adult ? "Ex. Une personne qui tente un flirt très assuré puis perd tous ses moyens" : "Ex. Un GPS vexé qui refuse de recalculer l’itinéraire"}"></textarea></div>
        <div class="mime-custom-grid">
          <div class="form-group"><label for="${prefix}CustomTheme">Thème</label><select id="${prefix}CustomTheme" class="text-input">${soloThemes.map(theme => `<option value="${theme.id}">${theme.icon} ${escapeHtml(theme.label)}</option>`).join("")}</select></div>
          <div class="form-group"><label for="${prefix}CustomDifficulty">Difficulté</label><select id="${prefix}CustomDifficulty" class="text-input">${Object.entries(difficultyMap).map(([id, meta]) => `<option value="${id}">${meta.icon} ${escapeHtml(meta.label)} · ${meta.points} pt${meta.points > 1 ? "s" : ""}</option>`).join("")}</select></div>
          <div class="form-group"><label for="${prefix}CustomActors">Imitateurs</label><select id="${prefix}CustomActors" class="text-input"><option value="1">🧍 Une personne</option><option value="2" ${state.players.length < 3 ? "disabled" : ""}>🧑‍🤝‍🧑 Deux personnes</option></select></div>
        </div>
        <label class="option-card premium-toggle"><input id="${prefix}CustomSound" type="checkbox"><span><strong>Cette carte exige des sons</strong><br><span class="helper">Elle sera retirée lorsque le mode silencieux est activé.</span></span></label>
        <button id="${prefix}AddCustom" class="secondary-btn full" type="button">＋ Ajouter cette imitation</button>
      </div>
      ${custom.length ? `<div class="mime-custom-list">${custom.slice().reverse().map(item => {
        const theme = akImitationThemeMeta(akImitationPackId(item), item);
        const difficulty = akImitationDifficultyMap(item)[akImitationDifficulty(item)];
        return `<article><div><small>${theme.icon} ${escapeHtml(theme.label)} · ${difficulty.icon} ${escapeHtml(difficulty.label)} · ${Number(item.actors || 1) === 2 ? "Duo" : "Solo"}</small><strong>${escapeHtml(item.text)}</strong></div><button type="button" data-${prefix}-delete-custom="${escapeHtml(item.id)}" aria-label="Supprimer cette imitation">×</button></article>`;
      }).join("")}</div>` : `<p class="helper top-gap">Aucune imitation personnelle pour le moment.</p>`}
    </section>`;
}

function akImitationSetupMarkup(game, prefix = "imitation") {
  ...
  const duoUnavailable = state.players.length < 3;
  return `
    <section class="card mime-setup-card imitation-setup-card">
      <div class="mime-section-heading"><div><small>FORMAT DE PARTIE</small><h3>Durée et nombre de manches</h3></div><span>${akImitationCardTotal(game)} cartes</span></div>
      <div class="choice-row mime-count-row">${[5, 10, 20, 40, 60, 100].map(value => `<button type="button" class="choice-pill ${game.roundCount === value ? "active" : ""}" data-${prefix}-count="${value}">${value}</button>`).join("")}</div>
      <div class="mime-custom-grid top-gap">
        <div class="form-group"><label for="${prefix}CustomCount">Nombre personnalisé</label><input id="${prefix}CustomCount" class="text-input" type="number" min="3" max="100" value="${game.roundCount}"></div>
        <div class="form-group"><label for="${prefix}Duration">Chronomètre</label><select id="${prefix}Duration" class="text-input">${[20, 30, 45, 60, 90].map(value => `<option value="${value}" ${game.durationSeconds === value ? "selected" : ""}>${value} secondes</option>`).join("")}</select></div>
      </div>
      <label class="option-card premium-toggle top-gap"><input id="${prefix}AllowSounds" type="checkbox" ${game.imitationAllowSounds ? "checked" : ""}><span><strong>🎙️ Autoriser les voix et les sons</strong><br><span class="helper">Désactive pour une partie uniquement basée sur les gestes, expressions et attitudes.</span></span></label>
    </section>
    <section class="card mime-theme-card imitation-theme-card">
      <div class="mime-section-heading"><div><small>PACKS D’IMITATIONS</small><h3>Choisis un ou plusieurs thèmes</h3></div><span>${game.imitationThemes.length}/${themes.length}</span></div>
      <div class="mime-theme-grid">${themes.map(theme => {
        const active = game.imitationThemes.includes(theme.id);
        const disabled = ["duo", "duo_adulte"].includes(theme.id) && duoUnavailable;
        return `<button type="button" class="mime-theme-option ${active ? "active" : ""}" data-${prefix}-theme="${theme.id}" aria-pressed="${active}" ${disabled ? "disabled" : ""}><span>${theme.icon}</span><strong>${escapeHtml(theme.label)}</strong><b>${active ? "✓" : "+"}</b></button>`;
      }).join("")}</div>
      <div class="toolbar top-gap"><button id="${prefix}AllThemes" class="secondary-btn" type="button">Tout sélectionner</button><button id="${prefix}NoThemes" class="secondary-btn" type="button">Tout désélectionner</button></div>
      ${duoUnavailable ? `<p class="helper top-gap">🧑‍🤝‍🧑 Les imitations en duo sont disponibles à partir de 3 joueurs.</p>` : ""}
    </section>
    <section class="card mime-difficulty-card imitation-difficulty-card">
      <div class="mime-section-heading"><div><small>${adult ? "INTENSITÉS" : "NIVEAUX"}</small><h3>${adult ? "Intensités combinables" : "Difficultés combinables"}</h3></div><span>1 à 4 pts</span></div>
      <div class="mime-difficulty-grid imitation-difficulty-grid">${Object.entries(difficultyMap).map(([id, meta]) => `<button type="button" class="mime-difficulty-option ${game.imitationDifficulties.includes(id) ? "active" : ""}" data-${prefix}-difficulty="${id}" aria-pressed="${game.imitationDifficulties.includes(id)}"><span>${meta.icon}</span><strong>${escapeHtml(meta.label)}</strong><small>${meta.points} point${meta.points > 1 ? "s" : ""}</small></button>`).join("")}</div>
      <p class="helper top-gap">Le ou les imitateurs gagnent la valeur du niveau. La première personne qui trouve gagne 1 point.</p>
    </section>
    ${adult ? `<div class="responsible-callout">🔞 Adultes consentants uniquement. Aucun contact, baiser, geste intime ou justification n’est obligatoire. Une carte peut être passée immédiatement.</div>` : ""}
    ${akImitationCustomSetupMarkup(game, prefix)}`;
}

function akImitationBindSetup(game, prefix = "imitation", rerender = renderMegaSetup) {
  document.querySelectorAll(`[data-${prefix}-count]`).forEach(button => button.addEventListener("click", () => {
    game.roundCount = Number(button.dataset[`${prefix}Count`]);
    rerender();
  }));
  ...
  document.querySelector(`#${prefix}Duration`)?.addEventListener("change", event => { game.durationSeconds = Number(event.target.value); });
  document.querySelector(`#${prefix}AllowSounds`)?.addEventListener("change", event => {
    game.imitationAllowSounds = Boolean(event.target.checked);
  });
  document.querySelectorAll(`[data-${prefix}-theme]`).forEach(button => button.addEventListener("click", () => {
    const id = button.dataset[`${prefix}Theme`];
    game.imitationThemes = game.imitationThemes.includes(id) ? game.imitationThemes.filter(value => value !== id) : [...game.imitationThemes, id];
    rerender();
  }));
  document.querySelector(`#${prefix}AllThemes`)?.addEventListener("click", () => {
    game.imitationThemes = akImitationThemeList(game).filter(theme => !["duo", "duo_adulte"].includes(theme.id) || state.players.length >= 3).map(theme => theme.id);
    rerender();
  });
  document.querySelector(`#${prefix}NoThemes`)?.addEventListener("click", () => { game.imitationThemes = []; rerender(); });
  document.querySelectorAll(`[data-${prefix}-difficulty]`).forEach(button => button.addEventListener("click", () => {
    const id = button.dataset[`${prefix}Difficulty`];
    game.imitationDifficulties = game.imitationDifficulties.includes(id) ? game.imitationDifficulties.filter(value => value !== id) : [...game.imitationDifficulties, id];
    rerender();
  }));
  document.querySelector(`#${prefix}IncludeCustom`)?.addEventListener("change", event => { game.imitationIncludeCustom = Boolean(event.target.checked); });
  document.querySelector(`#${prefix}AddCustom`)?.addEventListener("click", () => {
    const text = document.querySelector(`#${prefix}CustomText`)?.value?.trim();
    if (!text) return alert("Écris d’abord l’imitation à ajouter.");
    const category = document.querySelector(`#${prefix}CustomTheme`)?.value || "quotidien";
    const difficulty = document.querySelector(`#${prefix}CustomDifficulty`)?.value || "easy";
    const actors = Number(document.querySelector(`#${prefix}CustomActors`)?.value || 1);
    const soundMode = document.querySelector(`#${prefix}CustomSound`)?.checked ? "required" : "optional";
    if (actors === 2 && state.players.length < 3) return alert("Il faut au moins 3 joueurs pour enregistrer une imitation en duo dans cette partie.");
    const custom = akImitationLoadCustom(game);
    custom.push({
      id: `imitation_custom_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`,
      text,
      answer: text,
      category: actors === 2 ? (akImitationIsAdult(game) ? "duo_adulte" : "duo") : category,
      difficulty,
      actors,
      soundMode,
      forbiddenWords: [],
      collection: akImitationIsAdult(game) ? "adult" : "classic",
      custom: true
    });
    akImitationSaveCustom(custom, game);
    game.imitationIncludeCustom = true;
    rerender();
  });
  document.querySelectorAll(`[data-${prefix}-delete-custom]`).forEach(button => button.addEventListener("click", () => {
    const id = button.dataset[`${prefix}DeleteCustom`];
    akImitationSaveCustom(akImitationLoadCustom(game).filter(item => item.id !== id), game);
    rerender();
  }));
}

function akImitationFilterPool(pool, game) {
  const themes = new Set(game.imitationThemes || []);
  const difficulties = new Set(game.imitationDifficulties || []);
  return pool.filter(item => {
    if (Number(item.actors || 1) === 2 && state.players.length < 3) return false;
    if (!game.imitationAllowSounds && item.soundMode === "required") return false;
    return themes.has(akImitationPackId(item)) && difficulties.has(akImitationDifficulty(item));
  });
}

function akImitationBalancedSelect(pool, count, historyKey) {
  const safeCount = Math.min(Math.max(0, Number(count || 0)), pool.length);
  const grouped = new Map();
  pool.forEach(item => {
    const key = `${akImitationPackId(item)}:${akImitationDifficulty(item)}`;
    if (!grouped.has(key)) grouped.set(key, []);
    grouped.get(key).push(item);
  });
  const buckets = shuffleArray([...grouped.entries()]).map(([key, items]) => ({ key, items, quota: 0 }));
...
function akImitationAssignActors(items, players = state.players) {
  const ids = players.map(player => player.id);
  const soloOrder = shuffleArray([...ids]);
  const pairs = shuffleArray(ids.flatMap((id, index) => ids.slice(index + 1).map(other => [id, other])));
  let soloIndex = 0;
...
function akImitationActors(item) {
  return (item?.actorIds || []).map(id => state.players.find(player => player.id === id)).filter(Boolean);
}

function akImitationActorNames(item) {
  return akImitationActors(item).map(player => player.name).join(" et ");
}

function akImitationActorCards(item) {
  return akImitationActors(item).map(player => `<article><span>${avatarById(player.avatarId).emoji}</span><strong>${escapeHtml(player.name)}</strong></article>`).join("");
}

function akImitationCompleteRound(success, guesserId = null, timedOut = false) {
  const game = ensureImitationGameConfig();
  const item = game.items[game.currentIndex];
  const points = success ? akImitationPoints(item) : 0;
  const actorIds = item.actorIds || [];
  const elapsedSeconds = success
    ? Math.max(1, Math.min(game.durationSeconds, Number(game.imitationElapsedSeconds || Math.round((Date.now() - Number(game.imitationStartedAt || Date.now())) / 1000))))
    : game.durationSeconds;
  if (success) {
    actorIds.forEach(id => {
      game.scores[id] = Number(game.scores[id] || 0) + points;
      game.imitationStats.actorSuccess[id] = Number(game.imitationStats.actorSuccess[id] || 0) + 1;
    });
    if (guesserId && !actorIds.includes(guesserId)) {
      game.scores[guesserId] = Number(game.scores[guesserId] || 0) + 1;
      game.imitationStats.guesserSuccess[guesserId] = Number(game.imitationStats.guesserSuccess[guesserId] || 0) + 1;
    }
    if (!game.imitationStats.fastest || elapsedSeconds < Number(game.imitationStats.fastest.seconds || Infinity)) {
      game.imitationStats.fastest = { seconds: elapsedSeconds, text: item.text, actorIds: [...actorIds], guesserId };
    }
  }
  game.currentResult = { itemId: item.id, success, timedOut, actorIds: [...actorIds], guesserId, points, elapsedSeconds };
  game.rounds.push({ ...game.currentResult, text: item.text, difficulty: item.difficulty, category: item.category });
  game.imitationStage = "result";
  renderMegaTurn();
}

function akImitationRenderHandoff(game, item) {
  const actors = akImitationActors(item);
  screen.innerHTML = `
    ${v014Progress(game, game.gameName || "Imitation")}
    <section class="handoff-stage handoff-v07 mime-handoff imitation-handoff"><div class="mime-actor-row">${actors.map(player => `<span>${avatarById(player.avatarId).emoji}</span>`).join("")}</div><span class="category-chip">${Number(item.actors || 1) === 2 ? "IMITATION EN DUO" : "IMITATION SOLO"}</span><h2>${escapeHtml(akImitationActorNames(item))}</h2><p>${Number(item.actors || 1) === 2 ? "Donnez le téléphone aux deux imitateurs. Le reste du groupe ne doit pas voir le sujet." : `Donnez le téléphone à ${escapeHtml(actors[0]?.name || "la personne")}. Le groupe ne doit pas voir le sujet.`}</p><button id="imitationOpenPrivate" class="primary-btn">Afficher le sujet</button></section>`;
  document.querySelector("#imitationOpenPrivate")?.addEventListener("click", () => { game.imitationStage = "private"; renderMegaTurn(); });
}

function akImitationRenderPrivate(game, item) {
  screen.innerHTML = `
    ${v014Progress(game, game.gameName || "Imitation")}
    <section class="mime-private-card imitation-private-card">${akImitationBadges(item)}<span class="mime-private-icon">🤫</span><small>TON IMITATION SECRÈTE</small><h2>${escapeHtml(item.text)}</h2>${akImitationConstraintMarkup(item, game.imitationAllowSounds)}</section>
    <button id="imitationStartRound" class="primary-btn full">J’ai mémorisé · Lancer le chrono</button>`;
  document.querySelector("#imitationStartRound")?.addEventListener("click", () => {
    game.imitationStartedAt = Date.now();
    game.imitationStage = "playing";
    renderMegaTurn();
  });
}

function akImitationRenderPlaying(game, item) {
  const actors = akImitationActors(item);
  screen.innerHTML = `
    ${v014Progress(game, game.gameName || "Imitation")}
    <section class="mime-playing-card imitation-playing-card">
      ${akImitationBadges(item)}
      <div class="mime-live-icon">🎙️</div>
      <small>À VOUS DE DEVINER</small>
      <h2>${escapeHtml(akImitationActorNames(item))} ${actors.length > 1 ? "imitent ensemble" : "imite"}</h2>
      <div class="mime-performer-row">${akImitationActorCards(item)}</div>
      <div class="mega-mini-timer mime-timer"><strong id="v014Countdown">${game.durationSeconds}</strong><span>secondes</span><div class="progress-track"><div id="v014TimerFill" class="progress-fill" style="width:100%"></div></div></div>
      <p>${game.imitationAllowSounds ? "Voix, sons, expressions et gestes autorisés. La réponse et les mots interdits restent interdits." : "Mode silencieux : gestes, expressions et attitudes seulement."}</p>
    </section>
    <section class="decision-grid"><button id="imitationFound" class="primary-btn">✅ Trouvé !</button><button id="imitationSkip" class="secondary-btn">Passer</button></section>`;
  document.querySelector("#imitationFound")?.addEventListener("click", () => {
    clearV014Timer();
    game.imitationElapsedSeconds = Math.max(1, Math.min(game.durationSeconds, Math.round((Date.now() - Number(game.imitationStartedAt || Date.now())) / 1000)));
    game.imitationStage = "guesser";
    renderMegaTurn();
  });
  document.querySelector("#imitationSkip")?.addEventListener("click", () => { clearV014Timer(); akImitationCompleteRound(false, null, false); });
  startV014Timer(Date.now() + game.durationSeconds * 1000, "#v014Countdown", () => akImitationCompleteRound(false, null, true), game.durationSeconds);
}

function akImitationRenderGuesser(game, item) {
  const actorSet = new Set(item.actorIds || []);
  const candidates = state.players.filter(player => !actorSet.has(player.id));
  const elapsed = Math.max(1, Math.min(game.durationSeconds, Number(game.imitationElapsedSeconds || Math.round((Date.now() - Number(game.imitationStartedAt || Date.now())) / 1000))));
  screen.innerHTML = `
    ${v014Progress(game, game.gameName || "Imitation")}
    <section class="mime-guesser-card imitation-guesser-card"><span>🔔</span><small>PREMIÈRE BONNE RÉPONSE</small><h2>Qui a trouvé en premier ?</h2><p>Cette personne gagne 1 point. ${escapeHtml(akImitationActorNames(item))} ${Number(item.actors || 1) === 2 ? "gagnent" : "gagne"} ${akImitationPoints(item)} point${akImitationPoints(item) > 1 ? "s" : ""}. Sujet trouvé en environ ${elapsed} seconde${elapsed > 1 ? "s" : ""}.</p></section>
    <section class="mime-guesser-grid">${candidates.map(player => `<button type="button" data-imitation-guesser="${player.id}"><span>${avatarById(player.avatarId).emoji}</span><strong>${escapeHtml(player.name)}</strong><small>+1 point</small></button>`).join("")}</section>
    <button id="imitationNoGuesser" class="secondary-btn full">Réponse collective</button>`;
  document.querySelectorAll("[data-imitation-guesser]").forEach(button => button.addEventListener("click", () => akImitationCompleteRound(true, button.dataset.imitationGuesser, false)));
  document.querySelector("#imitationNoGuesser")?.addEventListener("click", () => akImitationCompleteRound(true, null, false));
}

function akImitationRenderResult(game, item) {
  const result = game.currentResult || {};
  const guesser = state.players.find(player => player.id === result.guesserId);
  screen.innerHTML = `
    ${v014Progress(game, game.gameName || "Imitation")}
    ...
    <section class="mime-result-summary">
      <article><span>${Number(item.actors || 1) === 2 ? "🧑‍🤝‍🧑" : "🎙️"}</span><strong>${escapeHtml(akImitationActorNames(item))}</strong><small>${result.success ? `+${result.points} point${result.points > 1 ? "s" : ""} ${Number(item.actors || 1) === 2 ? "chacun" : ""}` : "0 point"}</small></article>
      ${result.success ? `<article><span>🔎</span><strong>${guesser ? escapeHtml(guesser.name) : "Réponse collective"}</strong><small>${guesser ? "+1 point" : "Aucun point individuel"}</small></article>` : ""}
    </section>
    <button id="imitationNext" class="primary-btn full">${game.currentIndex + 1 >= game.items.length ? "Voir le classement" : "Imitation suivante"}</button>`;
  document.querySelector("#imitationNext")?.addEventListener("click", () => {
    game.currentIndex += 1;
    game.imitationStage = "handoff";
    game.currentResult = null;
    game.imitationStartedAt = null;
    game.imitationElapsedSeconds = null;
    renderMegaCurrent();
  });
}

if (V014_GAME_CONFIGS?.Imitation) {
  V014_GAME_CONFIGS.Imitation.icon = "🎙️";
  V014_GAME_CONFIGS.Imitation.description = "Voix, attitudes, situations et duos à faire reconnaître, avec quatre niveaux de difficulté.";
  V014_GAME_CONFIGS.Imitation.defaultRounds = 10;
  V014_GAME_CONFIGS.Imitation.timer = 45;
  if (typeof V014_GAME_ICONS === "object") V014_GAME_ICONS["Imitation"] = "🎙️";
}

if (typeof AK_AUDIT8_GAME_META === "object") {
  AK_AUDIT8_GAME_META["Imitation"] = { minPlayers: 2, time: "10 min", goal: "Fais reconnaître une voix, une attitude ou une situation avant la fin du chronomètre." };
}

const akImitationBaseResetMegaGame = resetMegaGame;
resetMegaGame = function (gameName, replayConfig = {}) {
  akImitationBaseResetMegaGame(gameName, replayConfig);
  if (gameName !== "Imitation" || !state.megaGame) return;
  state.megaGame.imitationThemes = akImitationNormalizeThemes(replayConfig.imitationThemes);
  state.megaGame.imitationDifficulties = akImitationNormalizeDifficulties(replayConfig.imitationDifficulties);
  state.megaGame.imitationAllowSounds = replayConfig.imitationAllowSounds !== false;
  state.megaGame.imitationIncludeCustom = replayConfig.imitationIncludeCustom !== false;
  ensureImitationGameConfig(state.megaGame);
};

const akImitationBaseRenderMegaSetup = renderMegaSetup;
renderMegaSetup = function () {
  const game = state.megaGame;
  if (!game || game.gameName !== "Imitation") return akImitationBaseRenderMegaSetup();
  ensureImitationGameConfig(game);
  clearV014Timer();
  title.textContent = "Imitation";
  setBackVisible(true);
  screen.innerHTML = `
    <section class="game-cover game-cover-mega imitation-cover"><span class="game-cover-icon">🎙️</span><div><small>DÉFIS & PERFORMANCE</small><h2>Imitation</h2><p>Fais reconnaître une voix, une attitude ou une situation. Les niveaux Chaos peuvent transformer une scène ordinaire en opéra de panne générale.</p></div></section>
    ${akImitationSetupMarkup(game, "imitation")}
    <button id="startMegaGame" class="primary-btn full">Lancer la partie d’Imitation</button>`;
  akImitationBindSetup(game, "imitation", renderMegaSetup);
  document.querySelector("#startMegaGame")?.addEventListener("click", startMegaGame);
};

const akImitationBaseStartMegaGame = startMegaGame;
startMegaGame = async function () {
  const game = state.megaGame;
  if (!game || game.gameName !== "Imitation") return akImitationBaseStartMegaGame();
  ensureImitationGameConfig(game);
  ...
  screen.innerHTML = `<div class="notice">Préparation des 600 imitations…</div>`;
  try {
    ...
    const selected = akImitationBalancedSelect(pool, Math.min(game.roundCount, pool.length), `solo:imitation:${game.imitationThemes.join("-")}:${game.imitationDifficulties.join("-")}:${game.imitationAllowSounds ? "sound" : "silent"}`);
    game.items = akImitationAssignActors(selected);
    game.currentIndex = 0;
    game.scores = v014ScoreMap();
    game.rounds = [];
    game.imitationStage = "handoff";
...
const akImitationBaseRenderMegaTurn = renderMegaTurn;
renderMegaTurn = function () {
  const game = state.megaGame;
  if (!game || game.gameName !== "Imitation") return akImitationBaseRenderMegaTurn();
  ensureImitationGameConfig(game);
  clearV014Timer();
  const item = game.items[game.currentIndex];
  if (!item) return renderMegaFinal();
  title.textContent = "Imitation";
  setBackVisible(false);
  if (game.imitationStage === "private") return akImitationRenderPrivate(game, item);
  if (game.imitationStage === "playing") return akImitationRenderPlaying(game, item);
  if (game.imitationStage === "guesser") return akImitationRenderGuesser(game, item);
  if (game.imitationStage === "result") return akImitationRenderResult(game, item);
  return akImitationRenderHandoff(game, item);
};

const akImitationBaseRenderMegaFinal = renderMegaFinal;
renderMegaFinal = function () {
  const game = state.megaGame;
  if (!game || game.gameName !== "Imitation") return akImitationBaseRenderMegaFinal();
  clearV014Timer();
  const ranking = [...state.players].sort((a, b) => Number(game.scores[b.id] || 0) - Number(game.scores[a.id] || 0));
  const best = Math.max(0, ...ranking.map(player => Number(game.scores[player.id] || 0)));
  const winners = ranking.filter(player => Number(game.scores[player.id] || 0) === best && best > 0);
  ...
  const actorTop = topBy(game.imitationStats?.actorSuccess);
  const guesserTop = topBy(game.imitationStats?.guesserSuccess);
  const fastest = game.imitationStats?.fastest;
  const names = top => top.players.map(player => escapeHtml(player.name)).join(" et ");
  ...
  screen.innerHTML = `
    ...
    <section class="final-ranking">${ranking.map((player, index) => `<div class="ranking-row"><span class="ranking-position">${index + 1}</span><span class="result-avatar">${avatarById(player.avatarId).emoji}</span><strong>${escapeHtml(player.name)}</strong><span>${Number(game.scores[player.id] || 0)} pts</span></div>`).join("")}</section>
    <section class="mime-final-awards imitation-final-awards">
      <article><span>🎙️</span><small>MEILLEUR IMITATEUR</small><strong>${actorTop.max ? names(actorTop) : "Aucune imitation trouvée"}</strong><p>${actorTop.max ? `${actorTop.max} imitation${actorTop.max > 1 ? "s" : ""} réussie${actorTop.max > 1 ? "s" : ""}` : "La prochaine voix sera la bonne."}</p></article>
      <article><span>🔎</span><small>MEILLEUR DEVINEUR</small><strong>${guesserTop.max ? names(guesserTop) : "Réponses collectives"}</strong><p>${guesserTop.max ? `${guesserTop.max} première${guesserTop.max > 1 ? "s" : ""} bonne${guesserTop.max > 1 ? "s" : ""} réponse${guesserTop.max > 1 ? "s" : ""}` : "Aucun point individuel attribué."}</p></article>
      <article><span>${fastest ? "⚡" : "🎬"}</span><small>IMITATION ÉCLAIR</small><strong>${fastest ? `${Number(fastest.seconds || 0)} secondes · ${actorNames(fastest.actorIds)}` : "Aucune manche trouvée"}</strong><p>${fastest ? escapeHtml(fastest.text || "") : "Le chrono réclame une revanche."}</p></article>
    </section>
    <div class="toolbar"><button id="replayMega" class="secondary-btn">Rejouer</button><button id="otherMega" class="primary-btn">Autre jeu</button></div>`;
  document.querySelector("#replayMega")?.addEventListener("click", () => {
    resetMegaGame("Imitation", {
      roundCount: game.roundCount,
      durationSeconds: game.durationSeconds,
      imitationThemes: game.imitationThemes,
      imitationDifficulties: game.imitationDifficulties,
      imitationAllowSounds: game.imitationAllowSounds,
      imitationIncludeCustom: game.imitationIncludeCustom
    });
    renderMegaSetup();
  });
  document.querySelector("#otherMega")?.addEventListener("click", () => { state.megaGame = null; renderPlayChoice(); });
...
/* =========================================================
   AK'GAMES V3.2 - IMITATION +18
   300 cartes adultes, intensites, duos et consentement
   ========================================================= */

if (V014_GAME_CONFIGS?.[AK_IMITATION_ADULT_GAME]) {
  V014_GAME_CONFIGS[AK_IMITATION_ADULT_GAME].icon = "🌶️";
  V014_GAME_CONFIGS[AK_IMITATION_ADULT_GAME].description = "Flirt, rendez-vous, couple, intimité et scènes à deux, sans contact imposé.";
  V014_GAME_CONFIGS[AK_IMITATION_ADULT_GAME].defaultRounds = 10;
  V014_GAME_CONFIGS[AK_IMITATION_ADULT_GAME].timer = 45;
  if (typeof V014_GAME_ICONS === "object") V014_GAME_ICONS[AK_IMITATION_ADULT_GAME] = "🌶️";
}

const akAdultImitationBaseResetMegaGame = resetMegaGame;
resetMegaGame = function (gameName, replayConfig = {}) {
  akAdultImitationBaseResetMegaGame(gameName, replayConfig);
  if (gameName !== AK_IMITATION_ADULT_GAME || !state.megaGame) return;
  state.megaGame.imitationAdult = true;
  state.megaGame.imitationThemes = akImitationNormalizeThemes(replayConfig.imitationThemes, state.megaGame);
  state.megaGame.imitationDifficulties = akImitationNormalizeDifficulties(replayConfig.imitationDifficulties, state.megaGame);
  state.megaGame.imitationAllowSounds = replayConfig.imitationAllowSounds !== false;
  state.megaGame.imitationIncludeCustom = replayConfig.imitationIncludeCustom !== false;
  ensureImitationGameConfig(state.megaGame);
};

const akAdultImitationBaseRenderMegaSetup = renderMegaSetup;
renderMegaSetup = function () {
  const game = state.megaGame;
  if (!game || game.gameName !== AK_IMITATION_ADULT_GAME) return akAdultImitationBaseRenderMegaSetup();
  ensureImitationGameConfig(game);
  clearV014Timer();
  title.textContent = AK_IMITATION_ADULT_GAME;
  setBackVisible(true);
  screen.innerHTML = `
    <section class="game-cover game-cover-mega imitation-cover imitation-adult-cover"><span class="game-cover-icon">🌶️</span><div><small>PACK ADULTE</small><h2>Imitation +18</h2><p>Joue des scènes de flirt, de couple ou d’intimité sans contact obligatoire. Le malaise doit rester fictif, pas s’installer dans le salon.</p></div></section>
    ${akImitationSetupMarkup(game, "imitationadult")}
    <button id="startMegaGame" class="primary-btn full">Lancer Imitation +18</button>`;
  akImitationBindSetup(game, "imitationadult", renderMegaSetup);
  document.querySelector("#startMegaGame")?.addEventListener("click", startMegaGame);
};

const akAdultImitationBaseStartMegaGame = startMegaGame;
startMegaGame = async function () {
  const game = state.megaGame;
  if (!game || game.gameName !== AK_IMITATION_ADULT_GAME) return akAdultImitationBaseStartMegaGame();
  ensureImitationGameConfig(game);
  ...
  screen.innerHTML = `<div class="notice">Préparation des 300 imitations adultes…</div>`;
  try {
    ...
    const selected = akImitationBalancedSelect(pool, Math.min(game.roundCount, pool.length), `solo:imitation-adult:${game.imitationThemes.join("-")}:${game.imitationDifficulties.join("-")}:${game.imitationAllowSounds ? "sound" : "silent"}`);
    game.items = akImitationAssignActors(selected);
    game.currentIndex = 0;
    game.scores = v014ScoreMap();
    game.rounds = [];
...
const akAdultImitationBaseRenderMegaTurn = renderMegaTurn;
renderMegaTurn = function () {
  const game = state.megaGame;
  if (!game || game.gameName !== AK_IMITATION_ADULT_GAME) return akAdultImitationBaseRenderMegaTurn();
  ensureImitationGameConfig(game);
  clearV014Timer();
  const item = game.items[game.currentIndex];
  if (!item) return renderMegaFinal();
...
const akAdultImitationBaseRenderMegaFinal = renderMegaFinal;
renderMegaFinal = function () {
  const game = state.megaGame;
  if (!game || game.gameName !== AK_IMITATION_ADULT_GAME) return akAdultImitationBaseRenderMegaFinal();
  clearV014Timer();
  const ranking = [...state.players].sort((a, b) => Number(game.scores[b.id] || 0) - Number(game.scores[a.id] || 0));
  const best = Math.max(0, ...ranking.map(player => Number(game.scores[player.id] || 0)));
  const winners = ranking.filter(player => Number(game.scores[player.id] || 0) === best && best > 0);
  ...
  setBackVisible(false);
  screen.innerHTML = `
    <section class="winner-stage winner-stage-v07 mega-final-stage imitation-final-stage"><div class="winner-crown">🌶️🎙️</div><h2>${winners.length ? winners.map(player => escapeHtml(player.name)).join(" et ") : "Rideau !"}</h2><p>${winners.length ? `${winners.length > 1 ? "terminent" : "termine"} en tête du studio +18.` : "Toutes les scènes sont terminées."}</p></section>
    <section class="final-ranking">${ranking.map((player, index) => `<div class="ranking-row"><span class="ranking-position">${index + 1}</span><span class="result-avatar">${avatarById(player.avatarId).emoji}</span><strong>${escapeHtml(player.name)}</strong><span>${Number(game.scores[player.id] || 0)} pts</span></div>`).join("")}</section>
    <section class="mime-final-awards imitation-final-awards">
      <article><span>🎙️</span><small>MEILLEUR IMITATEUR</small><strong>${actorTop.max ? names(actorTop) : "Aucune scène trouvée"}</strong><p>${actorTop.max ? `${actorTop.max} imitation${actorTop.max > 1 ? "s" : ""} réussie${actorTop.max > 1 ? "s" : ""}` : "La prochaine impro sera la bonne."}</p></article>
      <article><span>🔎</span><small>MEILLEUR DEVINEUR</small><strong>${guesserTop.max ? names(guesserTop) : "Réponses collectives"}</strong><p>${guesserTop.max ? `${guesserTop.max} première${guesserTop.max > 1 ? "s" : ""} bonne${guesserTop.max > 1 ? "s" : ""} réponse${guesserTop.max > 1 ? "s" : ""}` : "Aucun point individuel attribué."}</p></article>
      <article><span>${fastest ? "⚡" : "🎬"}</span><small>SCÈNE ÉCLAIR</small><strong>${fastest ? `${Number(fastest.seconds || 0)} secondes · ${actorNames(fastest.actorIds)}` : "Aucune manche trouvée"}</strong><p>${fastest ? escapeHtml(fastest.text || "") : "Le chrono réclame une revanche."}</p></article>
    </section>
    <div class="responsible-callout">🫶 Les résultats récompensent l’improvisation, jamais le niveau d’intimité accepté.</div>
    <div class="toolbar"><button id="replayMega" class="secondary-btn">Rejouer</button><button id="otherMega" class="primary-btn">Autre jeu</button></div>`;
  document.querySelector("#replayMega")?.addEventListener("click", () => {
    resetMegaGame(AK_IMITATION_ADULT_GAME, {
      roundCount: game.roundCount,
      durationSeconds: game.durationSeconds,
      imitationThemes: game.imitationThemes,
      imitationDifficulties: game.imitationDifficulties,
...
/* ===== AK'GAMES V3.9 — FAUX EXPERT 750 ===== */

const AK_FAKE_EXPERT_CLASSIC_THEMES_V39 = [
  { id: "science_nature", icon: "🔬", label: "Sciences et nature" },
  { id: "animaux", icon: "🦊", label: "Animaux" },
  { id: "nourriture", icon: "🍿", label: "Nourriture" },
  { id: "objets_quotidien", icon: "🧰", label: "Objets du quotidien" },
  { id: "technologie", icon: "💻", label: "Technologie" },
  { id: "histoire_geographie", icon: "🗺️", label: "Histoire et géographie" },
  { id: "culture_societe", icon: "🎭", label: "Culture et société" },
  { id: "sport", icon: "🏅", label: "Sport" },
  { id: "cinema_musique_jeux", icon: "🎬", label: "Cinéma, musique et jeux" },
  { id: "bizarre_mais_vrai", icon: "🧪", label: "Bizarre mais vrai" }
];

const AK_FAKE_EXPERT_ADULT_THEMES_V39 = [
  { id: "premiers_rendez_vous", icon: "🥂", label: "Premiers rendez-vous" },
  { id: "attirance_seduction", icon: "🫦", label: "Attirance et séduction" },
  { id: "couple_communication", icon: "💬", label: "Couple et communication" },
  { id: "jalousie_confiance", icon: "👀", label: "Jalousie et confiance" },
  { id: "ex_ruptures", icon: "💔", label: "Ex et ruptures" },
  { id: "applications_rencontre", icon: "📱", label: "Applications de rencontre" },
  { id: "messages_ambigus", icon: "💭", label: "Messages ambigus" },
  { id: "intimite_consentement", icon: "🔐", label: "Intimité et consentement" },
  { id: "corps_reactions", icon: "🔥", label: "Corps et réactions" },
  { id: "soirees_lendemains", icon: "🌙", label: "Soirées et lendemains" }
];

const AK_FAKE_EXPERT_DIFFICULTIES_V39 = [
  { id: "easy", icon: "🌱", label: "Facile" },
  { id: "medium", icon: "⚡", label: "Moyen" },
  { id: "hard", icon: "🔥", label: "Difficile" }
];

const AK_FAKE_EXPERT_ADULT_LEVELS_V39 = [
  { id: "soft", icon: "🌶️", label: "Léger" },
  { id: "bold", icon: "🌶️🌶️", label: "Osé" },
  { id: "hot", icon: "🌶️🌶️🌶️", label: "Sans filtre" }
];

const AK_FAKE_EXPERT_CUSTOM_KEY_V39 = "akgames.fakeExpert.custom.v39";

function loadFakeExpertCustomCardsV39() {
  try {
    const parsed = JSON.parse(localStorage.getItem(AK_FAKE_EXPERT_CUSTOM_KEY_V39) || "[]");
    return Array.isArray(parsed) ? parsed.filter(item => item?.topic && Array.isArray(item?.facts)) : [];
  } catch {
    return [];
  }
}

function saveFakeExpertCustomCardsV39(cards) {
  try { localStorage.setItem(AK_FAKE_EXPERT_CUSTOM_KEY_V39, JSON.stringify(cards || [])); }
  catch (error) { console.warn("Impossible de sauvegarder les dossiers personnalisés.", error); }
}

function normalizeFakeExpertCardV39(item, adult = false) {
  const safe = item && typeof item === "object" ? item : {};
  const facts = Array.isArray(safe.facts) ? safe.facts.map(value => String(value || "").trim()).filter(Boolean).slice(0, 3) : [];
  while (facts.length < 3) facts.push("Le contexte et les détails modifient cette explication.");
  const fakeTips = Array.isArray(safe.fakeTips)
    ? safe.fakeTips.map(value => String(value || "").trim()).filter(Boolean).slice(0, 3)
    : [String(safe.fakeTip || "Invente une théorie précise et défends-la avec aplomb.").trim()];
  while (fakeTips.length < 3) fakeTips.push(fakeTips[0] || "Invente une théorie précise et défends-la avec aplomb.");
  return {
    ...safe,
    id: String(safe.id || `expert_custom_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`),
    sourceId: String(safe.sourceId || safe.id || `expert_source_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`),
    topic: String(safe.topic || "Sujet mystère").trim(),
    facts,
    fakeTips,
    fakeTip: fakeTips[0],
    theme: String(safe.theme || (adult ? "premiers_rendez_vous" : "culture_societe")),
    category: String(safe.theme || safe.category || (adult ? "premiers_rendez_vous" : "culture_societe")),
...
function fakeExpertThemeGridV39(items, selected, attribute) {
  return `<div class="impostor-theme-grid expert-theme-grid-v39">${items.map(item => `
...
function fakeExpertDifficultyGridV39(items, selected, attribute) {
  return `<div class="expert-level-grid-v39">${items.map(item => `
...
function selectOneFakeExpertVariantPerSourceV39(pool) {
  const grouped = new Map();
  ...
  const groups = shuffleArray([...grouped.values()].map(group => shuffleArray([...group])));
  const ordered = [];
  const maxVariants = Math.max(0, ...groups.map(group => group.length));
  for (let pass = 0; pass < maxVariants; pass += 1) {
    shuffleArray([...groups]).forEach(group => {
      if (group[pass]) ordered.push(group[pass]);
    });
  }
  return ordered;
}

function pickFakeExpertTipV39(card) {
  const tips = Array.isArray(card?.fakeTips) && card.fakeTips.length ? card.fakeTips : [card?.fakeTip || "Invente une théorie crédible."];
  return tips[Math.floor(Math.random() * tips.length)] || tips[0];
}

async function buildFakeExpertPoolV39(game) {
  const classicRaw = await loadJsonFile("data/faux-expert.json", "Impossible de charger les 500 conférences classiques.");
  const classic = classicRaw
    .map(item => normalizeFakeExpertCardV39(item, false))
    .filter(item => game.classicThemes.includes(item.theme) && game.difficulties.includes(item.difficulty));

  let adult = [];
  if (state.adult && game.includeAdult) {
    const adultRaw = await loadJsonFile("data/faux-expert-adulte.json", "Impossible de charger les 250 conférences adultes.");
    adult = adultRaw
      .map(item => normalizeFakeExpertCardV39(item, true))
      .filter(item => game.adultThemes.includes(item.theme) && game.adultLevels.includes(item.difficulty));
  }

  const custom = game.includeCustom
    ? game.customCards
      .map(item => normalizeFakeExpertCardV39(item, Boolean(item.adult)))
      .filter(item => !item.adult || (state.adult && game.includeAdult))
    : [];

  return selectOneFakeExpertVariantPerSourceV39([...classic, ...adult, ...custom]);
}

resetFakeExpertState = function resetFakeExpertStateV39(config = {}) {
  const saved = loadFakeExpertCustomCardsV39();
  state.fakeExpert = {
    roundCount: Number(config.roundCount || Math.max(5, state.players.length)),
    includeAdult: Boolean(config.includeAdult),
    speechSeconds: Number(config.speechSeconds || 60),
    questionsEnabled: config.questionsEnabled !== false,
    classicThemes: Array.isArray(config.classicThemes) ? [...config.classicThemes] : AK_FAKE_EXPERT_CLASSIC_THEMES_V39.map(item => item.id),
    adultThemes: Array.isArray(config.adultThemes) ? [...config.adultThemes] : AK_FAKE_EXPERT_ADULT_THEMES_V39.map(item => item.id),
    difficulties: Array.isArray(config.difficulties) ? [...config.difficulties] : AK_FAKE_EXPERT_DIFFICULTIES_V39.map(item => item.id),
    adultLevels: Array.isArray(config.adultLevels) ? [...config.adultLevels] : AK_FAKE_EXPERT_ADULT_LEVELS_V39.map(item => item.id),
    includeCustom: Boolean((config.includeCustom ?? true) && saved.length),
    customCards: saved,
    items: [],
    currentIndex: 0,
    speakerOrder: shuffleArray(state.players.map(player => player.id)),
    ...
    currentVoterIndex: 0,
    scores: Object.fromEntries(state.players.map(player => [player.id, 0])),
    juryCorrect: Object.fromEntries(state.players.map(player => [player.id, 0])),
    fakeSpeakerPoints: Object.fromEntries(state.players.map(player => [player.id, 0])),
    realExpertWins: Object.fromEntries(state.players.map(player => [player.id, 0])),
    rounds: []
  };
};

renderFakeExpertSetup = function renderFakeExpertSetupV39() {
  clearV09Timer();
  if (!state.fakeExpert) resetFakeExpertState();
  const game = state.fakeExpert;
  ...
  setBackVisible(true);
  screen.innerHTML = `
    <section class="game-cover game-cover-expert"><span class="game-cover-icon">🎓</span><div><small>750 CONFÉRENCES</small><h2>Le Faux Expert</h2><p>Une personne reçoit de vrais faits ou une piste de bluff. Le jury écoute, questionne et tranche.</p></div></section>
    ...
    <section class="card setup-card-v07">
      <div class="form-group"><label for="expertRounds">Nombre de passages</label><select id="expertRounds" class="text-input">${roundOptions.map(value => `<option value="${value}" ${game.roundCount === value ? "selected" : ""}>${value} passages</option>`).join("")}</select></div>
      <div class="form-group top-gap"><label for="expertTimer">Temps de présentation</label><select id="expertTimer" class="text-input">${[30,45,60,90,120].map(value => `<option value="${value}" ${game.speechSeconds === value ? "selected" : ""}>${value} secondes</option>`).join("")}</select></div>
      <label class="option-card mini-option top-gap"><input id="expertQuestions" type="checkbox" ${game.questionsEnabled ? "checked" : ""}><span><strong>🙋 Questions du public</strong><br><small>Chaque personne peut poser une question avant le vote.</small></span></label>
    </section>

    <section class="card expert-config-card-v39">
      <div class="section-heading-row-v39"><div><small>CONFÉRENCES CLASSIQUES</small><h2>Choisis les thèmes</h2></div><div><button type="button" id="expertAllClassic" class="secondary-btn compact-btn-v39">Tout</button><button type="button" id="expertNoneClassic" class="secondary-btn compact-btn-v39">Aucun</button></div></div>
      ${fakeExpertThemeGridV39(AK_FAKE_EXPERT_CLASSIC_THEMES_V39, game.classicThemes, "data-expert-classic-theme")}
      <h3 class="expert-subtitle-v39">Difficulté des dossiers</h3>
      ${fakeExpertDifficultyGridV39(AK_FAKE_EXPERT_DIFFICULTIES_V39, game.difficulties, "data-expert-difficulty")}
    </section>

    ${state.adult ? `<section class="card expert-config-card-v39 adult-expert-card-v39">
      <label class="option-card premium-toggle"><input id="expertAdult" type="checkbox" ${game.includeAdult ? "checked" : ""}><span><strong>🌶️ Ajouter Faux Expert adulte</strong><br><span class="helper">250 conférences sur les dates, le couple, les messages, l’intimité et les soirées.</span></span></label>
      ${game.includeAdult ? `<div class="section-heading-row-v39 top-gap"><div><small>PACK ADULTE</small><h2>Thèmes et intensités</h2></div><div><button type="button" id="expertAllAdult" class="secondary-btn compact-btn-v39">Tout</button><button type="button" id="expertNoneAdult" class="secondary-btn compact-btn-v39">Aucun</button></div></div>
      ${fakeExpertThemeGridV39(AK_FAKE_EXPERT_ADULT_THEMES_V39, game.adultThemes, "data-expert-adult-theme")}
      <h3 class="expert-subtitle-v39">Intensité</h3>${fakeExpertDifficultyGridV39(AK_FAKE_EXPERT_ADULT_LEVELS_V39, game.adultLevels, "data-expert-adult-level")}` : ""}
    </section>` : ""}

    <section class="card expert-custom-card-v39">
      <h2 class="section-title">✍️ Mes conférences</h2>
      <p class="helper">Ajoute un sujet, trois faits vrais et une piste de bluff. Les cartes restent sur cet appareil.</p>
      <div class="form-group top-gap"><label for="expertCustomTopic">Sujet</label><input id="expertCustomTopic" class="text-input" maxlength="220" placeholder="Ex. Pourquoi les chats clignent-ils lentement des yeux ?"></div>
      <div class="form-group top-gap"><label for="expertCustomFacts">Trois faits vrais, un par ligne</label><textarea id="expertCustomFacts" class="text-input expert-textarea-v39" maxlength="800" placeholder="Premier fait\nDeuxième fait\nTroisième fait"></textarea></div>
      <div class="form-group top-gap"><label for="expertCustomTip">Piste de bluff</label><input id="expertCustomTip" class="text-input" maxlength="260" placeholder="Ex. Invente une théorie sur un code secret félin."></div>
      <div class="expert-custom-grid-v39">
        <div class="form-group"><label for="expertCustomTheme">Thème</label><select id="expertCustomTheme" class="text-input">${AK_FAKE_EXPERT_CLASSIC_THEMES_V39.map(item => `<option value="${item.id}">${item.icon} ${escapeHtml(item.label)}</option>`).join("")}${state.adult ? AK_FAKE_EXPERT_ADULT_THEMES_V39.map(item => `<option value="${item.id}" data-adult-theme="true">🌶️ ${escapeHtml(item.label)}</option>`).join("") : ""}</select></div>
        <div class="form-group"><label for="expertCustomDifficulty">Niveau</label><select id="expertCustomDifficulty" class="text-input">${[...AK_FAKE_EXPERT_DIFFICULTIES_V39, ...(state.adult ? AK_FAKE_EXPERT_ADULT_LEVELS_V39 : [])].map(item => `<option value="${item.id}">${item.icon} ${escapeHtml(item.label)}</option>`).join("")}</select></div>
      </div>
      <button type="button" id="expertAddCustom" class="secondary-btn full top-gap">Ajouter la conférence</button>
      ${custom.length ? `<label class="option-card mini-option top-gap"><input id="expertIncludeCustom" type="checkbox" ${game.includeCustom ? "checked" : ""}><span><strong>Utiliser mes ${custom.length} conférence${custom.length > 1 ? "s" : ""}</strong></span></label><details class="top-gap"><summary>Gérer mes conférences</summary><div class="impostor-custom-list">${custom.map(item => `<article><div><strong>${item.adult ? "🌶️ " : ""}${escapeHtml(item.topic)}</strong><small>${escapeHtml(item.fakeTips?.[0] || item.fakeTip || "")}</small></div><button type="button" class="danger-btn" data-remove-expert-custom="${item.id}">Supprimer</button></article>`).join("")}</div></details>` : ""}
    </section>

    <div class="notice"><strong>Barème :</strong> bon verdict +1 · vrai expert qui convainc la majorité +2 · faux expert +1 par personne trompée, avec +1 s’il trompe tout le monde.</div>
    <button id="startExpert" class="primary-btn full">Ouvrir la conférence</button>
  `;

  document.querySelector("#expertRounds")?.addEventListener("change", event => game.roundCount = Number(event.target.value));
  document.querySelector("#expertTimer")?.addEventListener("change", event => game.speechSeconds = Number(event.target.value));
  document.querySelector("#expertQuestions")?.addEventListener("change", event => game.questionsEnabled = event.target.checked);
  document.querySelector("#expertAdult")?.addEventListener("change", event => { game.includeAdult = event.target.checked; renderFakeExpertSetup(); });
  document.querySelector("#expertIncludeCustom")?.addEventListener("change", event => game.includeCustom = event.target.checked);

  document.querySelectorAll("[data-expert-classic-theme]").forEach(input => input.addEventListener("change", event => {
    const id = event.target.dataset.expertClassicTheme;
    game.classicThemes = event.target.checked ? [...new Set([...game.classicThemes, id])] : game.classicThemes.filter(value => value !== id);
  }));
  document.querySelectorAll("[data-expert-adult-theme]").forEach(input => input.addEventListener("change", event => {
    const id = event.target.dataset.expertAdultTheme;
    game.adultThemes = event.target.checked ? [...new Set([...game.adultThemes, id])] : game.adultThemes.filter(value => value !== id);
  }));
  document.querySelectorAll("[data-expert-difficulty]").forEach(input => input.addEventListener("change", event => {
    const id = event.target.dataset.expertDifficulty;
    game.difficulties = event.target.checked ? [...new Set([...game.difficulties, id])] : game.difficulties.filter(value => value !== id);
  }));
  document.querySelectorAll("[data-expert-adult-level]").forEach(input => input.addEventListener("change", event => {
    const id = event.target.dataset.expertAdultLevel;
    game.adultLevels = event.target.checked ? [...new Set([...game.adultLevels, id])] : game.adultLevels.filter(value => value !== id);
  }));

  document.querySelector("#expertAllClassic")?.addEventListener("click", () => { game.classicThemes = AK_FAKE_EXPERT_CLASSIC_THEMES_V39.map(item => item.id); renderFakeExpertSetup(); });
  document.querySelector("#expertNoneClassic")?.addEventListener("click", () => { game.classicThemes = []; renderFakeExpertSetup(); });
  document.querySelector("#expertAllAdult")?.addEventListener("click", () => { game.adultThemes = AK_FAKE_EXPERT_ADULT_THEMES_V39.map(item => item.id); renderFakeExpertSetup(); });
  document.querySelector("#expertNoneAdult")?.addEventListener("click", () => { game.adultThemes = []; renderFakeExpertSetup(); });

  document.querySelector("#expertAddCustom")?.addEventListener("click", () => {
    const topic = document.querySelector("#expertCustomTopic")?.value.trim() || "";
    const facts = (document.querySelector("#expertCustomFacts")?.value || "").split(/\n|;/).map(value => value.trim()).filter(Boolean);
    const tip = document.querySelector("#expertCustomTip")?.value.trim() || "";
    const theme = document.querySelector("#expertCustomTheme")?.value || "culture_societe";
    const difficulty = document.querySelector("#expertCustomDifficulty")?.value || "medium";
    ...
    if (!tip) return alert("Ajoute une piste de bluff pour le faux expert.");
  ...
  document.querySelectorAll("[data-remove-expert-custom]").forEach(button => button.addEventListener("click", () => {
    game.customCards = game.customCards.filter(item => item.id !== button.dataset.removeExpertCustom);
...
startFakeExpertGame = async function startFakeExpertGameV39() {
  const game = state.fakeExpert;
  ...
  screen.innerHTML = `<div class="notice">Préparation de 750 diplômes plus ou moins douteux…</div>`;
  try {
    ...
    game.items = selectFreshItems(pool, Math.min(game.roundCount, pool.length), `solo:fake-expert:v39:${game.classicThemes.join("-")}:${game.includeAdult}:${game.adultThemes.join("-")}:${game.difficulties.join("-")}:${game.adultLevels.join("-")}:${game.includeCustom}`);
    game.currentIndex = 0;
    game.speakerOrder = shuffleArray(Array.from({ length: game.items.length }, (_, index) => state.players[index % state.players.length].id));
    game.scores = Object.fromEntries(state.players.map(player => [player.id, 0]));
    game.juryCorrect = Object.fromEntries(state.players.map(player => [player.id, 0]));
    game.fakeSpeakerPoints = Object.fromEntries(state.players.map(player => [player.id, 0]));
    game.realExpertWins = Object.fromEntries(state.players.map(player => [player.id, 0]));
    game.rounds = [];
    prepareFakeExpertRound();
  } catch (error) {
    alert(error.message || "Impossible de lancer Faux Expert.");
    renderFakeExpertSetup();
  }
};

prepareFakeExpertRound = function prepareFakeExpertRoundV39() {
  const game = state.fakeExpert;
  if (game.currentIndex >= game.items.length) return renderFakeExpertEnd();
  game.speakerId = game.speakerOrder[game.currentIndex % game.speakerOrder.length];
  game.role = Math.random() < 0.5 ? "real" : "fake";
  game.currentFakeTip = pickFakeExpertTipV39(game.items[game.currentIndex]);
  game.votes = {};
  game.currentVoterIndex = 0;
  renderFakeExpertBriefGate();
};

renderFakeExpertBrief = function renderFakeExpertBriefV39() {
  const game = state.fakeExpert;
  const card = game.items[game.currentIndex];
  const isReal = game.role === "real";
  title.textContent = isReal ? "Vrai expert" : "Faux expert";
  screen.innerHTML = `
    ...
    <section class="secret-role-card expert-role ${isReal ? "civil" : "impostor"}">
      <span>${isReal ? "🎓" : "🎭"}</span><small>${isReal ? "VRAI EXPERT" : "FAUX EXPERT"}</small><h2>${escapeHtml(card.topic)}</h2>
      ${isReal ? `<ul class="expert-fact-list">${card.facts.map(fact => `<li>${escapeHtml(fact)}</li>`).join("")}</ul>` : `<p>${escapeHtml(game.currentFakeTip)}</p><em>Tu ne vois aucun fait vrai. Construis une théorie cohérente et évite les certitudes dangereuses.</em>`}
      <div class="expert-brief-badges-v39"><span>${card.adult ? "🌶️ Adulte" : "🎓 Classique"}</span><span>${escapeHtml(card.theme.replaceAll("_", " "))}</span></div>
    </section>
    <button id="startExpertSpeech" class="primary-btn full">Je suis prêt·e à parler</button>`;
  document.querySelector("#startExpertSpeech")?.addEventListener("click", renderFakeExpertSpeech);
};

renderFakeExpertSpeech = function renderFakeExpertSpeechV39() {
  const game = state.fakeExpert;
  const card = game.items[game.currentIndex];
  const speaker = state.players.find(player => player.id === game.speakerId);
  title.textContent = "Conférence express";
  screen.innerHTML = `
    ...
    <section class="timer-stage v09-timer-stage expert-stage">
      <span class="category-chip">${avatarById(speaker.avatarId).emoji} ${escapeHtml(speaker.name)}</span>
      <div id="v09TimerRing" class="v09-timer-ring"><strong id="v09Countdown">${game.speechSeconds}</strong><small>secondes</small></div>
      <h2>${escapeHtml(card.topic)}</h2>
      <p>${game.questionsEnabled ? "Chaque membre du jury peut poser une question courte. L’orateur répond sans rouvrir son brief." : "Le jury écoute sans interrompre, puis vote secrètement."}</p>
    </section>
    <button id="expertVoteNow" class="secondary-btn full">Passer au verdict</button>`;
  const vote = () => { clearV09Timer(); game.currentVoterIndex = 0; renderFakeExpertVoteGate(); };
  document.querySelector("#expertVoteNow")?.addEventListener("click", vote);
...
renderFakeExpertResult = function renderFakeExpertResultV39() {
  const game = state.fakeExpert;
  const card = game.items[game.currentIndex];
  const speaker = state.players.find(player => player.id === game.speakerId);
  const voters = fakeExpertVoters(game);
  ...
  correctIds.forEach(id => {
    game.scores[id] = Number(game.scores[id] || 0) + 1;
    game.juryCorrect[id] = Number(game.juryCorrect[id] || 0) + 1;
  });

  let speakerPoints = 0;
  if (game.role === "real" && correctIds.length > voters.length / 2) {
    speakerPoints = 2;
    game.realExpertWins[game.speakerId] = Number(game.realExpertWins[game.speakerId] || 0) + 1;
  }
  if (game.role === "fake") {
    speakerPoints = fooledIds.length + (fooledIds.length === voters.length && voters.length ? 1 : 0);
    game.fakeSpeakerPoints[game.speakerId] = Number(game.fakeSpeakerPoints[game.speakerId] || 0) + speakerPoints;
  }
  game.scores[game.speakerId] = Number(game.scores[game.speakerId] || 0) + speakerPoints;
  game.rounds.push({ itemId: card.id, sourceId: card.sourceId, topic: card.topic, speakerId: game.speakerId, role: game.role, votes: { ...game.votes }, correctIds, fooledIds, speakerPoints });

  const speakerMessage = game.role === "real"
    ? (speakerPoints ? "La majorité a reconnu le vrai dossier." : "Le vrai dossier n’a pas convaincu la majorité.")
    : (fooledIds.length === voters.length ? "Le faux expert a piégé tout le jury." : `${fooledIds.length} membre${fooledIds.length > 1 ? "s" : ""} du jury trompé${fooledIds.length > 1 ? "s" : ""}.`);

  title.textContent = "Diplôme révélé";
  setBackVisible(false);
  screen.innerHTML = `
    <section class="reveal-stage reveal-v07 expert-reveal"><span class="game-cover-icon">${game.role === "real" ? "🎓" : "🎭"}</span><h2>${escapeHtml(speaker.name)} était ${game.role === "real" ? "un vrai expert" : "un faux expert"}</h2><p>${escapeHtml(card.topic)}</p></section>
    <section class="who-vote-results">${voters.map(voter => {
      const correct = correctIds.includes(voter.id);
      return `<article class="who-vote-row ${correct ? "correct" : "fooled"}"><span>${avatarById(voter.avatarId).emoji}</span><strong>${escapeHtml(voter.name)}</strong><small>a voté ${game.votes[voter.id] === "real" ? "vrai expert" : "faux expert"}</small><em>${correct ? "+1 pt" : "trompé·e"}</em></article>`;
    }).join("")}</section>
    <details class="answer-wall-details" open><summary>Les trois faits vérifiés</summary><ul class="expert-fact-list">${card.facts.map(fact => `<li>${escapeHtml(fact)}</li>`).join("")}</ul></details>
    <div class="special-event"><strong>${game.role === "real" ? "🎓" : "🎭"} ${escapeHtml(speakerMessage)}</strong><p>+${speakerPoints} point${speakerPoints > 1 ? "s" : ""} pour l’orateur.</p></div>
    ${state.alcohol && fooledIds.length ? `<div class="alcohol-callout">🍻 Les personnes trompées peuvent trinquer avec la boisson de leur choix, sans obligation.</div>` : ""}
    <button id="nextExpertRound" class="primary-btn full">${game.currentIndex + 1 >= game.items.length ? "Voir les récompenses" : "Orateur suivant"}</button>`;
  document.querySelector("#nextExpertRound")?.addEventListener("click", () => { game.currentIndex += 1; prepareFakeExpertRound(); });
};

function fakeExpertTopPlayerV39(values) {
  const entries = state.players.map(player => ({ player, value: Number(values?.[player.id] || 0) })).sort((a, b) => b.value - a.value);
  return entries[0] || { player: state.players[0], value: 0 };
}

renderFakeExpertEnd = function renderFakeExpertEndV39() {
  const game = state.fakeExpert;
  const ranking = [...state.players].sort((a, b) => Number(game.scores[b.id] || 0) - Number(game.scores[a.id] || 0));
  const juror = fakeExpertTopPlayerV39(game.juryCorrect);
  const bluffer = fakeExpertTopPlayerV39(game.fakeSpeakerPoints);
  const professor = fakeExpertTopPlayerV39(game.realExpertWins);
  const trickiest = [...game.rounds].sort((a, b) => b.fooledIds.length - a.fooledIds.length)[0];
  title.textContent = "Palmarès des experts";
  setBackVisible(false);
  screen.innerHTML = `
    <section class="winner-stage winner-stage-v07 v09-final-stage"><div class="winner-crown">🎓🏆</div><h2>La faculté ferme ses portes</h2><p>Les diplômes étaient douteux, le classement est parfaitement officiel.</p></section>
    <section class="final-ranking">${ranking.map((player, index) => `<div class="ranking-row"><span class="ranking-position">${index + 1}</span><span class="result-avatar">${avatarById(player.avatarId).emoji}</span><strong>${escapeHtml(player.name)}</strong><span>${Number(game.scores[player.id] || 0)} pts</span></div>`).join("")}</section>
    <section class="impostor-awards">
      ${juror.value ? `<article><span>🔎</span><div><small>JURY LE PLUS LUCIDE</small><strong>${escapeHtml(juror.player.name)}</strong><p>${juror.value} verdict${juror.value > 1 ? "s" : ""} correct${juror.value > 1 ? "s" : ""}</p></div></article>` : ""}
      ${bluffer.value ? `<article><span>🎭</span><div><small>MEILLEUR FAUX EXPERT</small><strong>${escapeHtml(bluffer.player.name)}</strong><p>${bluffer.value} point${bluffer.value > 1 ? "s" : ""} gagné${bluffer.value > 1 ? "s" : ""} en bluff</p></div></article>` : ""}
      ${professor.value ? `<article><span>🎓</span><div><small>VRAI PROFESSEUR</small><strong>${escapeHtml(professor.player.name)}</strong><p>${professor.value} majorité${professor.value > 1 ? "s" : ""} convaincue${professor.value > 1 ? "s" : ""}</p></div></article>` : ""}
      ${trickiest ? `<article><span>🧠</span><div><small>SUJET LE PLUS TROMPEUR</small><strong>${escapeHtml(trickiest.topic)}</strong><p>${trickiest.fooledIds.length} personne${trickiest.fooledIds.length > 1 ? "s" : ""} trompée${trickiest.fooledIds.length > 1 ? "s" : ""}</p></div></article>` : ""}
    </section>
    <div class="toolbar"><button id="expertReplayV39" class="secondary-btn">Rejouer</button><button id="expertOtherV39" class="primary-btn">Autre jeu</button></div>`;
  document.querySelector("#expertReplayV39")?.addEventListener("click", () => {
    resetFakeExpertState({ roundCount: game.roundCount, includeAdult: game.includeAdult, speechSeconds: game.speechSeconds, questionsEnabled: game.questionsEnabled, classicThemes: game.classicThemes, adultThemes: game.adultThemes, difficulties: game.difficulties, adultLevels: game.adultLevels, includeCustom: game.includeCustom });
    renderFakeExpertSetup();
  });
...
/* =========================================================
   AK'GAMES V4.1 — DEVINETTES 600
   Réponses libres, indices, lecture tournante et validation
   ========================================================= */

const AK_RIDDLE_GAME_V41 = "Devinettes";
const AK_RIDDLE_CUSTOM_KEY_V41 = "akgames_riddles_custom_v41";
const AK_RIDDLE_THEMES_V41 = [
  { id: "objets", icon: "🧰", label: "Objets du quotidien" },
  { id: "animaux_nature", icon: "🦊", label: "Animaux & nature" },
  { id: "nourriture", icon: "🍓", label: "Nourriture" },
  { id: "corps_humain", icon: "🫀", label: "Corps humain" },
  { id: "lieux_voyages", icon: "🗺️", label: "Lieux & voyages" },
  { id: "metiers_personnages", icon: "🎭", label: "Métiers & personnages" },
  { id: "technologie", icon: "💻", label: "Technologie" },
  { id: "mots_expressions", icon: "🔤", label: "Mots & expressions" },
  { id: "logique", icon: "🧠", label: "Logique" },
  { id: "absurdes_pieges", icon: "🌀", label: "Absurdes & pièges" }
];
const AK_RIDDLE_DIFFICULTIES_V41 = [
  { id: "easy", icon: "🌱", label: "Facile" },
  { id: "medium", icon: "⚡", label: "Moyen" },
  { id: "hard", icon: "🔥", label: "Difficile" }
];

if (V014_GAME_CONFIGS?.[AK_RIDDLE_GAME_V41]) {
  Object.assign(V014_GAME_CONFIGS[AK_RIDDLE_GAME_V41], {
    engine: "quiz",
    icon: "🧩",
    pack: "Jeux rapides",
    data: "data/devinettes.json",
    description: "600 énigmes à résoudre avec des indices progressifs et des réponses libres.",
    defaultRounds: 20,
    riddleMode: true
  });
}

function akRiddleIsGameV41(game = state.megaGame) {
  return Boolean(game?.gameName === AK_RIDDLE_GAME_V41 || game?.config?.riddleMode);
}

function akRiddleThemeMetaV41(id) {
  return AK_RIDDLE_THEMES_V41.find(item => item.id === id) || AK_RIDDLE_THEMES_V41[0];
}

function akRiddleDifficultyMetaV41(id) {
  return AK_RIDDLE_DIFFICULTIES_V41.find(item => item.id === id) || AK_RIDDLE_DIFFICULTIES_V41[1];
}

function akRiddleNormalizeTextV41(value) {
  return String(value || "")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
...
function akRiddleEditDistanceV41(a, b) {
  const left = String(a || "");
...
function akRiddleAnswerMatchesV41(item, answer) {
  const candidate = akRiddleNormalizeTextV41(answer);
  if (!candidate) return false;
  const accepted = [...new Set([item?.answer, ...(item?.acceptedAnswers || [])]
    .map(akRiddleNormalizeTextV41)
    .filter(Boolean))];
  return accepted.some(expected => {
    if (candidate === expected) return true;
    if (candidate.length >= 5 && expected.length >= 5 && akRiddleEditDistanceV41(candidate, expected) <= 1) return true;
    return false;
  });
}

function akRiddleNormalizeCardV41(item, custom = false) {
  const theme = AK_RIDDLE_THEMES_V41.some(entry => entry.id === item?.theme) ? item.theme : "objets";
  const difficulty = AK_RIDDLE_DIFFICULTIES_V41.some(entry => entry.id === item?.difficulty) ? item.difficulty : "medium";
  const answer = String(item?.answer || "").trim();
  return {
    id: String(item?.id || `dev_custom_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`),
    question: String(item?.question || "").trim(),
    answer,
    acceptedAnswers: [...new Set([answer, ...(Array.isArray(item?.acceptedAnswers) ? item.acceptedAnswers : [])].map(value => String(value || "").trim()).filter(Boolean))],
    hints: (Array.isArray(item?.hints) ? item.hints : []).map(value => String(value || "").trim()).filter(Boolean).slice(0, 2),
    explanation: String(item?.explanation || `La réponse était ${answer}.`).trim(),
    theme,
    themeLabel: akRiddleThemeMetaV41(theme).label,
    difficulty,
    custom: Boolean(custom || item?.custom)
  };
}

function akRiddleReadCustomV41() {
  try {
    const parsed = JSON.parse(localStorage.getItem(AK_RIDDLE_CUSTOM_KEY_V41) || "[]");
    return Array.isArray(parsed) ? parsed.map(item => akRiddleNormalizeCardV41(item, true)).filter(item => item.question && item.answer) : [];
  } catch (error) {
    console.warn("Devinettes personnalisées illisibles :", error);
    return [];
  }
}

function akRiddleSaveCustomV41(cards) {
  try { localStorage.setItem(AK_RIDDLE_CUSTOM_KEY_V41, JSON.stringify((cards || []).slice(-150))); }
  catch (error) { console.warn("Impossible d’enregistrer les devinettes personnalisées :", error); }
}

function akRiddleEnsureV41(game, config = {}) {
  if (!game) return;
  game.riddleThemes = Array.isArray(config.riddleThemes || game.riddleThemes)
    ? [...new Set((config.riddleThemes || game.riddleThemes).filter(id => AK_RIDDLE_THEMES_V41.some(item => item.id === id)))]
    : AK_RIDDLE_THEMES_V41.map(item => item.id);
  if (!game.riddleThemes.length) game.riddleThemes = AK_RIDDLE_THEMES_V41.map(item => item.id);
  game.riddleDifficulties = Array.isArray(config.riddleDifficulties || game.riddleDifficulties)
    ? [...new Set((config.riddleDifficulties || game.riddleDifficulties).filter(id => AK_RIDDLE_DIFFICULTIES_V41.some(item => item.id === id)))]
    : AK_RIDDLE_DIFFICULTIES_V41.map(item => item.id);
  if (!game.riddleDifficulties.length) game.riddleDifficulties = AK_RIDDLE_DIFFICULTIES_V41.map(item => item.id);
  game.riddlePlayMode = ["all", "reader"].includes(config.riddlePlayMode || game.riddlePlayMode) ? (config.riddlePlayMode || game.riddlePlayMode) : "all";
  game.riddleValidationMode = ["automatic", "manual"].includes(config.riddleValidationMode || game.riddleValidationMode) ? (config.riddleValidationMode || game.riddleValidationMode) : "automatic";
  game.riddleTimerSeconds = [0, 15, 30, 45, 60, 90].includes(Number(config.riddleTimerSeconds ?? game.riddleTimerSeconds)) ? Number(config.riddleTimerSeconds ?? game.riddleTimerSeconds) : 45;
  game.riddleCustomCards = Array.isArray(config.riddleCustomCards) ? config.riddleCustomCards.map(item => akRiddleNormalizeCardV41(item, true)) : (Array.isArray(game.riddleCustomCards) ? game.riddleCustomCards : akRiddleReadCustomV41());
  game.riddleIncludeCustom = Boolean((config.riddleIncludeCustom ?? game.riddleIncludeCustom ?? game.riddleCustomCards.length > 0) && game.riddleCustomCards.length);
  game.roundCount = Number(config.roundCount || game.roundCount || 20);
}

function akRiddleRoundOptionsV41(selected) {
  return [10, 15, 20, 30, 40, 50, 75, 100]
    .map(value => `<option value="${value}" ${Number(selected) === value ? "selected" : ""}>${value} devinette${value > 1 ? "s" : ""}</option>`)
    .join("");
}

function akRiddleSetupMarkupV41(game, prefix = "riddle", readOnly = false) {
  akRiddleEnsureV41(game);
  const custom = game.riddleCustomCards || [];
  const fieldId = suffix => `${prefix}${suffix}`;
  return `
    <section class="card riddle-settings-v41">
      <div class="form-group"><label for="${fieldId("Rounds")}">Nombre de devinettes</label><select id="${fieldId("Rounds")}" class="text-input" ${readOnly ? "disabled" : ""}>${akRiddleRoundOptionsV41(game.roundCount)}</select></div>
      <div class="riddle-setup-columns-v41 top-gap">
        <div class="form-group"><label for="${fieldId("Mode")}">Mode de jeu</label><select id="${fieldId("Mode")}" class="text-input" ${readOnly ? "disabled" : ""}><option value="all" ${game.riddlePlayMode === "all" ? "selected" : ""}>✍️ Tous contre la devinette</option><option value="reader" ${game.riddlePlayMode === "reader" ? "selected" : ""}>📣 Lecteur tournant</option></select></div>
        <div class="form-group"><label for="${fieldId("Validation")}">Validation des réponses</label><select id="${fieldId("Validation")}" class="text-input" ${readOnly ? "disabled" : ""}><option value="automatic" ${game.riddleValidationMode === "automatic" ? "selected" : ""}>Automatique</option><option value="manual" ${game.riddleValidationMode === "manual" ? "selected" : ""}>Manuelle par le groupe</option></select></div>
        <div class="form-group"><label for="${fieldId("Timer")}">Chronomètre</label><select id="${fieldId("Timer")}" class="text-input" ${readOnly ? "disabled" : ""}>${[0,15,30,45,60,90].map(value => `<option value="${value}" ${Number(game.riddleTimerSeconds) === value ? "selected" : ""}>${value ? `${value} secondes` : "Sans chrono"}</option>`).join("")}</select></div>
      </div>
      <p class="helper top-gap">Sans indice : 3 points · après un indice : 2 points · après deux indices : 1 point.</p>
    </section>
    <section class="card riddle-filter-card-v41"><div class="riddle-filter-heading-v41"><div><small>THÈMES</small><h3>Choisis les univers</h3></div><button type="button" id="${fieldId("AllThemes")}" class="secondary-btn" ${readOnly ? "disabled" : ""}>Tout sélectionner</button></div><div class="riddle-theme-grid-v41">${AK_RIDDLE_THEMES_V41.map(theme => `<label class="form-check riddle-check-v41"><input class="form-check-input" type="checkbox" data-riddle-theme="${theme.id}" ${game.riddleThemes.includes(theme.id) ? "checked" : ""} ${readOnly ? "disabled" : ""}><span class="form-check-label"><b>${theme.icon}</b>${escapeHtml(theme.label)}</span></label>`).join("")}</div></section>
    <section class="card riddle-filter-card-v41"><div class="riddle-filter-heading-v41"><div><small>DIFFICULTÉ</small><h3>Mélange les niveaux</h3></div></div><div class="riddle-difficulty-grid-v41">${AK_RIDDLE_DIFFICULTIES_V41.map(level => `<label class="form-check riddle-check-v41 difficulty-${level.id}"><input class="form-check-input" type="checkbox" data-riddle-difficulty="${level.id}" ${game.riddleDifficulties.includes(level.id) ? "checked" : ""} ${readOnly ? "disabled" : ""}><span class="form-check-label"><b>${level.icon}</b>${escapeHtml(level.label)}</span></label>`).join("")}</div></section>
    ${readOnly ? "" : `<section class="card riddle-custom-card-v41"><h3>Mes devinettes</h3><p class="helper">Elles restent sur cet appareil et peuvent rejoindre le mélange.</p><div class="form-group top-gap"><label for="${fieldId("CustomQuestion")}">Devinette</label><textarea id="${fieldId("CustomQuestion")}" class="text-input" rows="3" maxlength="260" placeholder="Ex. Je monte et je descends sans bouger. Qui suis-je ?"></textarea></div><div class="riddle-custom-grid-v41"><div class="form-group"><label for="${fieldId("CustomAnswer")}">Réponse</label><input id="${fieldId("CustomAnswer")}" class="text-input" maxlength="80" placeholder="Un escalier"></div><div class="form-group"><label for="${fieldId("CustomHint1")}">Indice 1</label><input id="${fieldId("CustomHint1")}" class="text-input" maxlength="150" placeholder="On me trouve dans un immeuble."></div><div class="form-group"><label for="${fieldId("CustomHint2")}">Indice 2</label><input id="${fieldId("CustomHint2")}" class="text-input" maxlength="150" placeholder="Je suis composé de marches."></div><div class="form-group"><label for="${fieldId("CustomTheme")}">Thème</label><select id="${fieldId("CustomTheme")}" class="text-input">${AK_RIDDLE_THEMES_V41.map(theme => `<option value="${theme.id}">${theme.icon} ${escapeHtml(theme.label)}</option>`).join("")}</select></div><div class="form-group"><label for="${fieldId("CustomDifficulty")}">Difficulté</label><select id="${fieldId("CustomDifficulty")}" class="text-input">${AK_RIDDLE_DIFFICULTIES_V41.map(level => `<option value="${level.id}">${level.icon} ${escapeHtml(level.label)}</option>`).join("")}</select></div></div><button type="button" id="${fieldId("AddCustom")}" class="secondary-btn full">Ajouter la devinette</button>${custom.length ? `<label class="option-card mini-option top-gap"><input id="${fieldId("IncludeCustom")}" type="checkbox" ${game.riddleIncludeCustom ? "checked" : ""}><span><strong>Inclure mes ${custom.length} devinette${custom.length > 1 ? "s" : ""}</strong></span></label><details class="top-gap"><summary>Gérer mes devinettes</summary><div class="riddle-custom-list-v41">${custom.map(item => `<article><div><strong>${escapeHtml(item.question)}</strong><small>Réponse : ${escapeHtml(item.answer)}</small></div><button type="button" class="danger-btn" data-remove-riddle-custom="${escapeHtml(item.id)}">Supprimer</button></article>`).join("")}</div></details>` : ""}</section>`}
    <div class="riddle-mode-note-v41">${game.riddlePlayMode === "reader" ? "📣 Un lecteur différent connaît la solution à chaque manche et anime la devinette." : "✍️ Chaque personne donne une réponse secrète avant la révélation."}</div>`;
}

function akRiddleBindSetupV41(game, prefix = "riddle", rerender = renderMegaSetup) {
  const fieldId = suffix => `${prefix}${suffix}`;
  document.querySelector(`#${fieldId("Rounds")}`)?.addEventListener("change", event => game.roundCount = Number(event.target.value));
  document.querySelector(`#${fieldId("Mode")}`)?.addEventListener("change", event => { game.riddlePlayMode = event.target.value; rerender(); });
  document.querySelector(`#${fieldId("Validation")}`)?.addEventListener("change", event => game.riddleValidationMode = event.target.value);
  document.querySelector(`#${fieldId("Timer")}`)?.addEventListener("change", event => game.riddleTimerSeconds = Number(event.target.value));
  document.querySelectorAll("[data-riddle-theme]").forEach(input => input.addEventListener("change", () => {
    const selected = [...document.querySelectorAll("[data-riddle-theme]:checked")].map(node => node.dataset.riddleTheme);
    if (!selected.length) { input.checked = true; return alert("Garde au moins un thème sélectionné."); }
    game.riddleThemes = selected;
  }));
  document.querySelector(`#${fieldId("AllThemes")}`)?.addEventListener("click", () => { game.riddleThemes = AK_RIDDLE_THEMES_V41.map(item => item.id); rerender(); });
  document.querySelectorAll("[data-riddle-difficulty]").forEach(input => input.addEventListener("change", () => {
    const selected = [...document.querySelectorAll("[data-riddle-difficulty]:checked")].map(node => node.dataset.riddleDifficulty);
    if (!selected.length) { input.checked = true; return alert("Garde au moins une difficulté sélectionnée."); }
    game.riddleDifficulties = selected;
  }));
  document.querySelector(`#${fieldId("IncludeCustom")}`)?.addEventListener("change", event => game.riddleIncludeCustom = event.target.checked);
  document.querySelector(`#${fieldId("AddCustom")}`)?.addEventListener("click", () => {
    const question = document.querySelector(`#${fieldId("CustomQuestion")}`)?.value.trim();
    const answer = document.querySelector(`#${fieldId("CustomAnswer")}`)?.value.trim();
    const hint1 = document.querySelector(`#${fieldId("CustomHint1")}`)?.value.trim();
    const hint2 = document.querySelector(`#${fieldId("CustomHint2")}`)?.value.trim();
    if (!question || !answer || !hint1 || !hint2) return alert("Ajoute la devinette, sa réponse et les deux indices.");
    if (game.riddleCustomCards.some(item => item.question.toLocaleLowerCase("fr") === question.toLocaleLowerCase("fr"))) return alert("Cette devinette personnalisée existe déjà.");
    const item = akRiddleNormalizeCardV41({
      id: `dev_custom_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
      question,
      answer,
      acceptedAnswers: [answer],
      hints: [hint1, hint2],
      explanation: `La réponse était ${answer}.`,
      theme: document.querySelector(`#${fieldId("CustomTheme")}`)?.value || "objets",
      difficulty: document.querySelector(`#${fieldId("CustomDifficulty")}`)?.value || "medium",
      custom: true
    }, true);
    game.riddleCustomCards = [...game.riddleCustomCards, item].slice(-150);
    game.riddleIncludeCustom = true;
    akRiddleSaveCustomV41(game.riddleCustomCards);
    rerender();
  });
  document.querySelectorAll("[data-remove-riddle-custom]").forEach(button => button.addEventListener("click", () => {
    game.riddleCustomCards = game.riddleCustomCards.filter(item => item.id !== button.dataset.removeRiddleCustom);
    game.riddleIncludeCustom = game.riddleIncludeCustom && game.riddleCustomCards.length > 0;
    akRiddleSaveCustomV41(game.riddleCustomCards);
    rerender();
  }));
}

async function akRiddleBuildPoolV41(game) {
  const official = await loadJsonFile("data/devinettes.json", "Impossible de charger les devinettes.");
  const themes = new Set(game.riddleThemes || []);
  const difficulties = new Set(game.riddleDifficulties || []);
  let pool = official.map(item => akRiddleNormalizeCardV41(item)).filter(item => themes.has(item.theme) && difficulties.has(item.difficulty));
  if (game.riddleIncludeCustom) pool = pool.concat((game.riddleCustomCards || []).filter(item => themes.has(item.theme) && difficulties.has(item.difficulty)));
  return pool;
}

function akRiddleSelectItemsV41(pool, count, historyKey) {
  const conceptKey = item => akRiddleNormalizeTextV41(item?.answer) || String(item?.id || "").replace(/_[12]$/, "");
  const uniqueConceptCount = new Set(pool.map(conceptKey)).size;
  const safeCount = Math.min(Math.max(1, Number(count || 20)), uniqueConceptCount);
  const themes = [...new Set(pool.map(item => item.theme))];
  const chosen = [];
  const usedIds = new Set();
  const usedConcepts = new Set();
  const shuffledThemes = shuffleArray([...themes]);
  let cursor = 0;
  while (chosen.length < safeCount && cursor < safeCount * Math.max(3, themes.length)) {
    const theme = shuffledThemes[cursor % shuffledThemes.length];
    const candidates = pool.filter(item => item.theme === theme && !usedIds.has(item.id) && !usedConcepts.has(conceptKey(item)));
    if (candidates.length) {
      const fresh = selectFreshItems(candidates, 1, `${historyKey}:${theme}`)[0];
      if (fresh) {
        chosen.push(fresh);
        usedIds.add(fresh.id);
        usedConcepts.add(conceptKey(fresh));
      }
    }
    cursor += 1;
    if (shuffledThemes.every(id => !pool.some(item => item.theme === id && !usedConcepts.has(conceptKey(item))))) break;
  }
  if (chosen.length < safeCount) {
    const remaining = pool.filter(item => !usedIds.has(item.id) && !usedConcepts.has(conceptKey(item)));
    for (const fresh of selectFreshItems(remaining, safeCount - chosen.length, `${historyKey}:extra`)) {
      if (!usedConcepts.has(conceptKey(fresh))) {
        chosen.push(fresh);
        usedIds.add(fresh.id);
        usedConcepts.add(conceptKey(fresh));
      }
  ...
  return shuffleArray(chosen).slice(0, safeCount);
}

function akRiddlePointsV41(hintsUsed) {
  return Math.max(1, 3 - Math.max(0, Math.min(2, Number(hintsUsed || 0))));
}

function akRiddleBadgesV41(item) {
  const theme = akRiddleThemeMetaV41(item?.theme);
  const level = akRiddleDifficultyMetaV41(item?.difficulty);
  return `<div class="riddle-badges-v41"><span>${theme.icon} ${escapeHtml(theme.label)}</span><span>${level.icon} ${escapeHtml(level.label)}</span>${item?.custom ? `<span>✍️ Personnalisée</span>` : ""}</div>`;
}

const akRiddleBaseResetMegaV41 = resetMegaGame;
resetMegaGame = function resetMegaGameRiddleV41(gameName, replayConfig = {}) {
  akRiddleBaseResetMegaV41(gameName, replayConfig);
  if (!akRiddleIsGameV41()) return;
  akRiddleEnsureV41(state.megaGame, replayConfig);
};

const akRiddleBaseRenderSetupV41 = renderMegaSetup;
renderMegaSetup = function renderMegaSetupRiddleV41() {
  if (!akRiddleIsGameV41()) return akRiddleBaseRenderSetupV41();
  const game = state.megaGame;
  akRiddleEnsureV41(game);
  clearV014Timer();
  ...
  setBackVisible(true);
  screen.innerHTML = `
    <section class="game-cover game-cover-mega riddle-cover-v41"><span class="game-cover-icon">🧩</span><div><small>600 DEVINETTES · 10 THÈMES</small><h2>Devinettes</h2><p>Réponses libres, deux indices progressifs et jusqu’à trois points par énigme.</p></div></section>
    ...
    ${akRiddleSetupMarkupV41(game, "soloRiddle")}
    <button id="startRiddleV41" class="primary-btn full">Lancer les devinettes</button>`;
  akRiddleBindSetupV41(game, "soloRiddle", renderMegaSetup);
  document.querySelector("#startRiddleV41")?.addEventListener("click", startMegaGame);
};

const akRiddleBaseStartMegaV41 = startMegaGame;
startMegaGame = async function startMegaGameRiddleV41() {
  if (!akRiddleIsGameV41() || state.mode !== "single") return akRiddleBaseStartMegaV41();
  const game = state.megaGame;
  akRiddleEnsureV41(game);
  if (!game.riddleThemes.length || !game.riddleDifficulties.length) return alert("Choisis au moins un thème et une difficulté.");
  screen.innerHTML = `<div class="notice">Préparation des 600 énigmes…</div>`;
  try {
    ...
    game.items = akRiddleSelectItemsV41(pool, game.roundCount, `solo:riddles:v41:${game.riddleThemes.join("-")}:${game.riddleDifficulties.join("-")}`)
      .map((item, index) => ({ ...item, readerId: game.riddlePlayMode === "reader" ? state.players[index % state.players.length]?.id : null }));
    game.currentIndex = 0;
    game.currentVoterIndex = 0;
    game.scores = v014ScoreMap();
    game.rounds = [];
    game.riddleCorrectCounts = Object.fromEntries(state.players.map(player => [player.id, 0]));
    game.riddleBestTimes = Object.fromEntries(state.players.map(player => [player.id, null]));
    akRiddlePrepareRoundV41(game);
    renderMegaCurrent();
  } catch (error) {
    console.error(error);
...
function akRiddlePrepareRoundV41(game) {
  clearV014Timer();
  game.riddlePhase = "question";
  game.riddleHintsUsed = 0;
  game.riddleAnswers = {};
  game.riddleManualVerdicts = {};
  game.currentResult = null;
  game.currentVoterIndex = 0;
  game.riddleReaderRevealed = false;
  game.riddleRoundStartedAt = Date.now();
  game.riddleEndsAt = game.riddleTimerSeconds ? Date.now() + game.riddleTimerSeconds * 1000 : null;
}

const akRiddleBaseRenderMegaCurrentV41 = renderMegaCurrent;
renderMegaCurrent = function renderMegaCurrentRiddleV41() {
  if (!akRiddleIsGameV41() || state.mode !== "single") return akRiddleBaseRenderMegaCurrentV41();
  const game = state.megaGame;
  if (game.currentIndex >= game.items.length) return renderMegaFinal();
  if (game.riddlePlayMode === "reader") return akRiddleRenderReaderV41(game);
  if (game.riddlePhase === "question") return akRiddleRenderQuestionV41(game);
  if (game.riddlePhase === "answer") return akRiddleRenderAnswerGateV41(game);
  if (game.riddlePhase === "validation") return akRiddleRenderValidationV41(game);
  if (game.riddlePhase === "result") return akRiddleRenderResultV41(game);
  akRiddlePrepareRoundV41(game);
  akRiddleRenderQuestionV41(game);
};

function akRiddleTimerMarkupV41(game) {
  if (!game.riddleEndsAt) return "";
  const left = Math.max(0, Math.ceil((game.riddleEndsAt - Date.now()) / 1000));
  return `<div class="mega-mini-timer riddle-timer-v41"><strong id="v014Countdown">${left}</strong><span>secondes</span><div class="progress-track"><div id="v014TimerFill" class="progress-fill" style="width:100%"></div></div></div>`;
}

function akRiddleHintMarkupV41(game, item, readerPrivate = false) {
  const used = Number(game.riddleHintsUsed || 0);
  return `<section class="riddle-hints-v41">${[0,1].map(index => {
    const revealed = index < used || readerPrivate;
    return `<article class="${revealed ? "revealed" : "locked"}"><span>${index + 1}</span><div><small>INDICE ${index + 1}</small><p>${revealed ? escapeHtml(item.hints?.[index] || "Aucun indice supplémentaire.") : "Indice encore caché"}</p></div></article>`;
  }).join("")}</section>`;
}

function akRiddleRevealHintV41(game) {
  if (game.riddleHintsUsed >= 2) return;
  game.riddleHintsUsed += 1;
  renderMegaCurrent();
}

function akRiddleRenderQuestionV41(game) {
  const item = game.items[game.currentIndex];
  title.textContent = "À vos méninges";
  setBackVisible(false);
  screen.innerHTML = `
    ${v014Progress(game, "Devinette")}
    <section class="riddle-question-card-v41">${akRiddleBadgesV41(item)}<span class="riddle-big-icon-v41">🧩</span><h2>${escapeHtml(item.question)}</h2>${akRiddleTimerMarkupV41(game)}</section>
    ${akRiddleHintMarkupV41(game, item)}
    <div class="riddle-action-grid-v41"><button id="riddleHintV41" class="secondary-btn" ${game.riddleHintsUsed >= 2 ? "disabled" : ""}>💡 ${game.riddleHintsUsed ? "Indice suivant" : "Débloquer un indice"}</button><button id="riddleAnswerV41" class="primary-btn">✍️ Répondre</button></div>`;
  document.querySelector("#riddleHintV41")?.addEventListener("click", () => akRiddleRevealHintV41(game));
  document.querySelector("#riddleAnswerV41")?.addEventListener("click", () => { clearV014Timer(); game.riddlePhase = "answer"; game.currentVoterIndex = 0; renderMegaCurrent(); });
  if (game.riddleEndsAt) startV014Timer(game.riddleEndsAt, "#v014Countdown", () => { game.riddlePhase = "answer"; game.currentVoterIndex = 0; renderMegaCurrent(); }, game.riddleTimerSeconds);
}

function akRiddleRenderAnswerGateV41(game) {
  if (game.currentVoterIndex >= state.players.length) return akRiddleFinishAnswersV41(game);
  const item = game.items[game.currentIndex];
  const player = state.players[game.currentVoterIndex];
  title.textContent = `Réponse de ${player.name}`;
  screen.innerHTML = `
    ${v014Progress(game, "Devinette")}
    <section class="handoff-stage handoff-v07"><div class="giant-avatar">${avatarById(player.avatarId).emoji}</div><span class="category-chip">${escapeHtml(player.name).toUpperCase()}</span><h2>Réponse secrète</h2><p>Le groupe détourne les yeux, puis ${escapeHtml(player.name)} ouvre sa zone de réponse.</p><button id="openRiddleAnswerV41" class="primary-btn">Afficher la devinette</button></section>`;
  document.querySelector("#openRiddleAnswerV41")?.addEventListener("click", () => {
    const startedAt = Date.now();
    screen.innerHTML = `
      ${v014Progress(game, "Devinette")}
      <section class="riddle-answer-card-v41">${akRiddleBadgesV41(item)}<span class="category-chip">RÉPONSE DE ${escapeHtml(player.name).toUpperCase()}</span><h2>${escapeHtml(item.question)}</h2>${game.riddleHintsUsed ? `<p class="riddle-hint-recap-v41">${item.hints.slice(0, game.riddleHintsUsed).map((hint,index) => `💡 ${index + 1}. ${escapeHtml(hint)}`).join("<br>")}</p>` : ""}<label for="riddleAnswerInputV41">Ta réponse</label><input id="riddleAnswerInputV41" class="text-input" maxlength="100" autocomplete="off" placeholder="Écris ta réponse"><button id="submitRiddleAnswerV41" class="primary-btn full">Verrouiller ma réponse</button></section>`;
    const submit = () => {
      const text = document.querySelector("#riddleAnswerInputV41")?.value.trim() || "";
      game.riddleAnswers[player.id] = { text, elapsedMs: Math.max(0, Date.now() - startedAt) };
      game.currentVoterIndex += 1;
      renderMegaCurrent();
    };
    document.querySelector("#submitRiddleAnswerV41")?.addEventListener("click", submit);
    document.querySelector("#riddleAnswerInputV41")?.addEventListener("keydown", event => { if (event.key === "Enter") submit(); });
    document.querySelector("#riddleAnswerInputV41")?.focus();
  });
}

function akRiddleFinishAnswersV41(game) {
  const item = game.items[game.currentIndex];
  game.riddleManualVerdicts = Object.fromEntries(state.players.map(player => [player.id, akRiddleAnswerMatchesV41(item, game.riddleAnswers[player.id]?.text)]));
  if (game.riddleValidationMode === "manual") {
    game.riddlePhase = "validation";
    return akRiddleRenderValidationV41(game);
  }
  akRiddleCommitResultV41(game, game.riddleManualVerdicts);
}

function akRiddleRenderValidationV41(game) {
  const item = game.items[game.currentIndex];
  title.textContent = "Validation du groupe";
  screen.innerHTML = `
    ${v014Progress(game, "Devinette")}
    <section class="reveal-stage reveal-v07 riddle-solution-v41"><span class="game-cover-icon">🔐</span><small>SOLUTION</small><h2>${escapeHtml(item.answer)}</h2><p>Le groupe décide si les formulations proches sont acceptées.</p></section>
    <section class="riddle-validation-list-v41">${state.players.map(player => {
      const correct = Boolean(game.riddleManualVerdicts[player.id]);
      return `<article><span>${avatarById(player.avatarId).emoji}</span><div><strong>${escapeHtml(player.name)}</strong><p>${escapeHtml(game.riddleAnswers[player.id]?.text || "Aucune réponse")}</p></div><button type="button" data-riddle-verdict="${player.id}" class="${correct ? "primary-btn" : "secondary-btn"}">${correct ? "✓ Correcte" : "✗ Incorrecte"}</button></article>`;
    }).join("")}</section>
    <button id="confirmRiddleVerdictsV41" class="primary-btn full">Valider les résultats</button>`;
  document.querySelectorAll("[data-riddle-verdict]").forEach(button => button.addEventListener("click", () => {
    const id = button.dataset.riddleVerdict;
    game.riddleManualVerdicts[id] = !game.riddleManualVerdicts[id];
    akRiddleRenderValidationV41(game);
  }));
  document.querySelector("#confirmRiddleVerdictsV41")?.addEventListener("click", () => akRiddleCommitResultV41(game, game.riddleManualVerdicts));
}

function akRiddleCommitResultV41(game, verdicts, winnerId = null) {
  const item = game.items[game.currentIndex];
  const points = akRiddlePointsV41(game.riddleHintsUsed);
  const correctIds = winnerId ? [winnerId] : state.players.filter(player => verdicts?.[player.id]).map(player => player.id);
  const earned = {};
  correctIds.forEach(id => {
    earned[id] = points;
    game.scores[id] = Number(game.scores[id] || 0) + points;
    game.riddleCorrectCounts[id] = Number(game.riddleCorrectCounts[id] || 0) + 1;
    const elapsed = Number(game.riddleAnswers?.[id]?.elapsedMs || 0);
    if (elapsed > 0 && (!game.riddleBestTimes[id] || elapsed < game.riddleBestTimes[id])) game.riddleBestTimes[id] = elapsed;
  });
  game.currentResult = {
    itemId: item.id,
    ...
    correctIds,
    earned,
    answers: { ...(game.riddleAnswers || {}) },
    hintsUsed: Number(game.riddleHintsUsed || 0),
    points,
    readerId: item.readerId || null,
    fastestMs: Math.min(...correctIds.map(id => Number(game.riddleAnswers?.[id]?.elapsedMs || Number.MAX_SAFE_INTEGER)), Number.MAX_SAFE_INTEGER)
  };
  game.rounds.push(game.currentResult);
  game.riddlePhase = "result";
  akRiddleRenderResultV41(game);
}

function akRiddleRenderResultV41(game) {
  const item = game.items[game.currentIndex];
  const result = game.currentResult || {};
  title.textContent = result.correctIds?.length ? "Trouvé !" : "Mystère résolu";
  screen.innerHTML = `
    ${v014Progress(game, "Devinette")}
    <section class="reveal-stage reveal-v07 riddle-solution-v41 ${result.correctIds?.length ? "solved" : "unsolved"}"><span class="game-cover-icon">${result.correctIds?.length ? "🎉" : "🧩"}</span>${akRiddleBadgesV41(item)}<small>LA RÉPONSE ÉTAIT</small><h2>${escapeHtml(item.answer)}</h2><p>${escapeHtml(item.explanation || "Solution révélée.")}</p><em>${result.hintsUsed ? `${result.hintsUsed} indice${result.hintsUsed > 1 ? "s" : ""} utilisé${result.hintsUsed > 1 ? "s" : ""} · ${result.points} point${result.points > 1 ? "s" : ""}` : `Sans indice · ${result.points} points`}</em></section>
    ${game.riddlePlayMode === "reader" ? `<section class="answer-chip-wall">${state.players.map(player => `<span class="${result.correctIds?.includes(player.id) ? "correct" : player.id === result.readerId ? "reader" : "wrong"}">${avatarById(player.avatarId).emoji} ${escapeHtml(player.name)} · ${player.id === result.readerId ? "lecteur" : result.correctIds?.includes(player.id) ? `+${result.points}` : "pas trouvé"}</span>`).join("")}</section>` : `<section class="answer-chip-wall">${state.players.map(player => { const won = result.correctIds?.includes(player.id); return `<span class="${won ? "correct" : "wrong"}">${avatarById(player.avatarId).emoji} ${escapeHtml(player.name)} · ${escapeHtml(result.answers?.[player.id]?.text || "Aucune réponse")}${won ? ` · +${result.points}` : ""}</span>`; }).join("")}</section>`}
    <button id="nextRiddleV41" class="primary-btn full">${game.currentIndex + 1 >= game.items.length ? "Voir le palmarès" : "Devinette suivante"}</button>`;
  document.querySelector("#nextRiddleV41")?.addEventListener("click", () => { game.currentIndex += 1; akRiddlePrepareRoundV41(game); renderMegaCurrent(); });
}

function akRiddleRenderReaderV41(game) {
  const item = game.items[game.currentIndex];
  const reader = state.players.find(player => player.id === item.readerId) || state.players[game.currentIndex % state.players.length];
  if (game.riddlePhase === "result") return akRiddleRenderResultV41(game);
  if (!game.riddleReaderRevealed) {
    title.textContent = "Fiche du lecteur";
    screen.innerHTML = `${v014Progress(game, "Devinette")}<section class="handoff-stage handoff-v07"><div class="giant-avatar">${avatarById(reader.avatarId).emoji}</div><span class="category-chip">LECTEUR · ${escapeHtml(reader.name).toUpperCase()}</span><h2>Écran privé</h2><p>${escapeHtml(reader.name)} garde le téléphone et découvre la solution. Les autres détournent les yeux.</p><button id="openReaderBriefV41" class="primary-btn">Voir ma fiche</button></section>`;
    document.querySelector("#openReaderBriefV41")?.addEventListener("click", () => { game.riddleReaderRevealed = true; game.riddleRoundStartedAt = Date.now(); game.riddleEndsAt = game.riddleTimerSeconds ? Date.now() + game.riddleTimerSeconds * 1000 : null; akRiddleRenderReaderV41(game); });
    return;
  }
  title.textContent = "À faire deviner";
  const candidates = state.players.filter(player => player.id !== reader.id);
  screen.innerHTML = `
    ${v014Progress(game, "Devinette")}
    <section class="riddle-reader-card-v41">${akRiddleBadgesV41(item)}<span class="category-chip">FICHE PRIVÉE DE ${escapeHtml(reader.name).toUpperCase()}</span><h2>${escapeHtml(item.question)}</h2><div class="riddle-reader-solution-v41"><small>SOLUTION</small><strong>${escapeHtml(item.answer)}</strong></div>${akRiddleTimerMarkupV41(game)}</section>
    ${akRiddleHintMarkupV41(game, item, true)}
    <div class="riddle-action-grid-v41"><button id="readerUseHintV41" class="secondary-btn" ${game.riddleHintsUsed >= 2 ? "disabled" : ""}>💡 Marquer l’indice ${Math.min(2, game.riddleHintsUsed + 1)} comme utilisé</button><button id="readerNobodyV41" class="secondary-btn">Personne n’a trouvé</button></div>
    <section class="riddle-winner-grid-v41"><h3>Qui a trouvé en premier ?</h3>${candidates.map(player => `<button type="button" data-riddle-winner="${player.id}"><span>${avatarById(player.avatarId).emoji}</span><strong>${escapeHtml(player.name)}</strong><small>+${akRiddlePointsV41(game.riddleHintsUsed)} point${akRiddlePointsV41(game.riddleHintsUsed) > 1 ? "s" : ""}</small></button>`).join("")}</section>`;
  document.querySelector("#readerUseHintV41")?.addEventListener("click", () => { if (game.riddleHintsUsed < 2) game.riddleHintsUsed += 1; akRiddleRenderReaderV41(game); });
  document.querySelector("#readerNobodyV41")?.addEventListener("click", () => akRiddleCommitResultV41(game, {}, null));
  document.querySelectorAll("[data-riddle-winner]").forEach(button => button.addEventListener("click", () => {
    const winnerId = button.dataset.riddleWinner;
    game.riddleAnswers[winnerId] = { text: item.answer, elapsedMs: Math.max(0, Date.now() - game.riddleRoundStartedAt) };
    akRiddleCommitResultV41(game, { [winnerId]: true }, winnerId);
  }));
  if (game.riddleEndsAt) startV014Timer(game.riddleEndsAt, "#v014Countdown", () => akRiddleCommitResultV41(game, {}, null), game.riddleTimerSeconds);
}

const akRiddleBaseFinalV41 = renderMegaFinal;
renderMegaFinal = function renderMegaFinalRiddleV41() {
  if (!akRiddleIsGameV41() || state.mode !== "single") return akRiddleBaseFinalV41();
  const game = state.megaGame;
  clearV014Timer();
  const ranking = [...state.players].sort((a, b) => Number(game.scores[b.id] || 0) - Number(game.scores[a.id] || 0));
  const mostCorrect = state.players.map(player => ({ player, value: Number(game.riddleCorrectCounts[player.id] || 0) })).sort((a,b) => b.value - a.value)[0];
  const fastest = state.players.map(player => ({ player, value: Number(game.riddleBestTimes[player.id] || 0) })).filter(row => row.value > 0).sort((a,b) => a.value - b.value)[0];
  const trickiest = [...game.rounds].sort((a,b) => Number(a.correctIds?.length || 0) - Number(b.correctIds?.length || 0))[0];
  const replay = { roundCount: game.roundCount, riddleThemes: [...game.riddleThemes], riddleDifficulties: [...game.riddleDifficulties], riddlePlayMode: game.riddlePlayMode, riddleValidationMode: game.riddleValidationMode, riddleTimerSeconds: game.riddleTimerSeconds, riddleIncludeCustom: game.riddleIncludeCustom, riddleCustomCards: game.riddleCustomCards };
  title.textContent = "Palmarès des énigmes";
  setBackVisible(false);
  screen.innerHTML = `
    <section class="winner-stage winner-stage-v07 mega-final-stage riddle-final-v41"><div class="winner-crown">🧩🏆</div><h2>${ranking[0] ? escapeHtml(ranking[0].name) : "Partie terminée"}</h2><p>Le sphinx range ses fiches, mais garde un œil sur la revanche.</p></section>
    <section class="final-ranking">${ranking.map((player,index) => `<div class="ranking-row"><span class="ranking-position">${index + 1}</span><span class="result-avatar">${avatarById(player.avatarId).emoji}</span><strong>${escapeHtml(player.name)}</strong><span>${Number(game.scores[player.id] || 0)} pts</span></div>`).join("")}</section>
    <section class="impostor-awards">${mostCorrect?.value ? `<article><span>🧠</span><div><small>PLUS DE DEVINETTES RÉSOLUES</small><strong>${escapeHtml(mostCorrect.player.name)}</strong><p>${mostCorrect.value}/${game.items.length}</p></div></article>` : ""}${fastest?.value ? `<article><span>⚡</span><div><small>RÉPONSE LA PLUS RAPIDE</small><strong>${escapeHtml(fastest.player.name)}</strong><p>${(fastest.value / 1000).toFixed(1)} seconde${fastest.value >= 2000 ? "s" : ""}</p></div></article>` : ""}${trickiest ? `<article><span>🕳️</span><div><small>DEVINETTE LA PLUS PIÉGEUSE</small><strong>${escapeHtml(trickiest.question)}</strong><p>${Number(trickiest.correctIds?.length || 0)}/${game.riddlePlayMode === "reader" ? Math.max(1,state.players.length - 1) : state.players.length} bonne${Number(trickiest.correctIds?.length || 0) > 1 ? "s" : ""} réponse${Number(trickiest.correctIds?.length || 0) > 1 ? "s" : ""}</p></div></article>` : ""}</section>
    <div class="toolbar"><button id="riddleReplayV41" class="secondary-btn">Rejouer</button><button id="riddleOtherV41" class="primary-btn">Autre jeu</button></div>`;
...
/* AKGAMES LA BOMBE 600 V44 */
const AK_BOMB_GAME_V44 = "La Bombe";
const AK_BOMB_PREFS_KEY_V44 = "akgames:bomb:v44:prefs";
const AK_BOMB_CUSTOM_KEY_V44 = "akgames:bomb:v44:custom";
const AK_BOMB_HISTORY_KEY_V44 = "akgames:bomb:v44:history";
const AK_BOMB_THEMES_V44 = [
  ["categories","🧺","Catégories"],["lettres","🔤","Lettres imposées"],["precision","🎯","Contraintes rapides"],["associations","💭","Associations"],["personnel","🫵","Personnel léger"],
  ["culture_pop","🎬","Culture populaire"],["absurde","🌀","Absurde"],["vocal_geste","🎭","Voix & gestes"],["memoire","🧠","Mémoire"],["chaos","💥","Chaos"]
];
const AK_BOMB_LEVELS_V44 = [["facile","🌱","Facile"],["moyen","⚡","Moyen"],["difficile","🔥","Difficile"]];
const AK_BOMB_MODES_V44 = [
  ["classic","Classique","Les autres marquent 1 point quand la bombe explose."],
  ["elimination","Élimination","Chaque explosion retire une vie. Le dernier encore en jeu gagne."],
  ["points","À points","Chaque réponse rapporte 1 point et une explosion en retire 2."],
  ["teams","Équipes","L’explosion offre la manche à l’équipe adverse."],
  ["infernal","Bombe infernale","Chaque passage accélère la bombe."]
];
function akBombReadJsonV44(key, fallback) { try { const value=JSON.parse(localStorage.getItem(key)); return value ?? fallback; } catch (_) { return fallback; } }
function akBombWriteJsonV44(key, value) { try { localStorage.setItem(key, JSON.stringify(value)); } catch (_) {} }
function akBombCustomCardsV44() { const rows=akBombReadJsonV44(AK_BOMB_CUSTOM_KEY_V44,[]); return Array.isArray(rows)?rows:[]; }
function akBombSaveCustomCardsV44(rows) { akBombWriteJsonV44(AK_BOMB_CUSTOM_KEY_V44, rows); }
function akBombNormalizeV44(value) { return String(value||"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().replace(/[^a-z0-9]+/g," ").trim(); }
function akBombRandomDurationV44(source) { const settings=source?.settings||source||{}; const min=Math.max(10,Number(settings.bombMinSeconds||15)); const max=Math.max(min,Math.min(60,Number(settings.bombMaxSeconds||40))); return Math.floor(min+Math.random()*(max-min+1)); }
function akBombEnsureV44(game, replay={}) {
  if(!game||game.gameName!==AK_BOMB_GAME_V44)return game;
  const saved=akBombReadJsonV44(AK_BOMB_PREFS_KEY_V44,{});
  const themes=AK_BOMB_THEMES_V44.map(row=>row[0]), levels=AK_BOMB_LEVELS_V44.map(row=>row[0]);
  game.bombThemes=Array.isArray(replay.bombThemes||saved.bombThemes)?(replay.bombThemes||saved.bombThemes).filter(v=>themes.includes(v)):themes;
  game.bombDifficulties=Array.isArray(replay.bombDifficulties||saved.bombDifficulties)?(replay.bombDifficulties||saved.bombDifficulties).filter(v=>levels.includes(v)):levels;
  game.bombMode=String(replay.bombMode||saved.bombMode||"classic"); if(!AK_BOMB_MODES_V44.some(row=>row[0]===game.bombMode))game.bombMode="classic";
  game.bombMinSeconds=Math.max(10,Math.min(60,Number(replay.bombMinSeconds||saved.bombMinSeconds||15)));
  game.bombMaxSeconds=Math.max(game.bombMinSeconds,Math.min(60,Number(replay.bombMaxSeconds||saved.bombMaxSeconds||40)));
  game.bombOrder=String(replay.bombOrder||saved.bombOrder||"fixed");
  game.bombLives=Math.max(1,Math.min(5,Number(replay.bombLives||saved.bombLives||3)));
  game.bombSkipLimit=Math.max(0,Math.min(3,Number(replay.bombSkipLimit??saved.bombSkipLimit??1)));
  game.bombSound=(replay.bombSound??saved.bombSound)!==false; game.bombVibration=(replay.bombVibration??saved.bombVibration)!==false;
  game.bombIncludeCustom=(replay.bombIncludeCustom??saved.bombIncludeCustom)!==false;
  game.bombCustomCards=akBombCustomCardsV44(); game.roundCount=Math.max(5,Math.min(50,Number(replay.roundCount||game.roundCount||10)));
  return game;
}
function akBombSavePrefsV44(game){akBombWriteJsonV44(AK_BOMB_PREFS_KEY_V44,{bombThemes:game.bombThemes,bombDifficulties:game.bombDifficulties,bombMode:game.bombMode,bombMinSeconds:game.bombMinSeconds,bombMaxSeconds:game.bombMaxSeconds,bombOrder:game.bombOrder,bombLives:game.bombLives,bombSkipLimit:game.bombSkipLimit,bombSound:game.bombSound,bombVibration:game.bombVibration,bombIncludeCustom:game.bombIncludeCustom,roundCount:game.roundCount});}
function akBombSetupMarkupV44(game,prefix="bomb",readOnly=false){
  akBombEnsureV44(game); const dis=readOnly?"disabled":"";
  return `<section class="card setup-card-v07 bomb-setup-v44">
    <div class="setup-grid-v44"><div class="form-group"><label for="${prefix}Rounds">Nombre de manches</label><select id="${prefix}Rounds" class="text-input" ${dis}>${[5,8,10,12,15,20,25,30,40,50].map(v=>`<option value="${v}" ${game.roundCount===v?"selected":""}>${v} manches</option>`).join("")}</select></div>
    <div class="form-group"><label for="${prefix}Mode">Mode</label><select id="${prefix}Mode" class="text-input" ${dis}>${AK_BOMB_MODES_V44.map(row=>`<option value="${row[0]}" ${game.bombMode===row[0]?"selected":""}>${row[1]}</option>`).join("")}</select></div></div>
    <p class="helper bomb-mode-helper-v44">${escapeHtml(AK_BOMB_MODES_V44.find(row=>row[0]===game.bombMode)?.[2]||"")}</p>
    <div class="setup-grid-v44"><div class="form-group"><label for="${prefix}Min">Explosion au plus tôt</label><select id="${prefix}Min" class="text-input" ${dis}>${[10,15,20,25,30,40].map(v=>`<option value="${v}" ${game.bombMinSeconds===v?"selected":""}>${v} s</option>`).join("")}</select></div>
    <div class="form-group"><label for="${prefix}Max">Explosion au plus tard</label><select id="${prefix}Max" class="text-input" ${dis}>${[20,25,30,40,45,60].map(v=>`<option value="${v}" ${game.bombMaxSeconds===v?"selected":""}>${v} s</option>`).join("")}</select></div></div>
    <div class="setup-grid-v44"><div class="form-group"><label for="${prefix}Order">Ordre des joueurs</label><select id="${prefix}Order" class="text-input" ${dis}><option value="fixed" ${game.bombOrder==="fixed"?"selected":""}>Ordre fixe</option><option value="random" ${game.bombOrder==="random"?"selected":""}>Aléatoire à chaque passage</option></select></div>
    <div class="form-group"><label for="${prefix}Skips">Jokers par joueur</label><select id="${prefix}Skips" class="text-input" ${dis}>${[0,1,2,3].map(v=>`<option value="${v}" ${game.bombSkipLimit===v?"selected":""}>${v}</option>`).join("")}</select></div></div>
    <div class="form-group bomb-lives-wrap-v44" ${game.bombMode==="elimination"?"":"hidden"}><label for="${prefix}Lives">Vies par joueur</label><select id="${prefix}Lives" class="text-input" ${dis}>${[1,2,3,4,5].map(v=>`<option value="${v}" ${game.bombLives===v?"selected":""}>${v} vie${v>1?"s":""}</option>`).join("")}</select></div>
  </section>
  <section class="card"><h2 class="section-title">Thèmes</h2><div class="filter-chip-grid-v44">${AK_BOMB_THEMES_V44.map(row=>`<label class="form-check"><input class="form-check-input" type="checkbox" data-bomb-theme-v44="${row[0]}" ${game.bombThemes.includes(row[0])?"checked":""} ${dis}><span class="form-check-label">${row[1]} ${row[2]}</span></label>`).join("")}</div></section>
  <section class="card"><h2 class="section-title">Difficultés</h2><div class="filter-chip-grid-v44">${AK_BOMB_LEVELS_V44.map(row=>`<label class="form-check"><input class="form-check-input" type="checkbox" data-bomb-level-v44="${row[0]}" ${game.bombDifficulties.includes(row[0])?"checked":""} ${dis}><span class="form-check-label">${row[1]} ${row[2]}</span></label>`).join("")}</div></section>
  <section class="card"><h2 class="section-title">Effets</h2><div class="filter-chip-grid-v44"><label class="form-check form-switch"><input id="${prefix}Sound" class="form-check-input" type="checkbox" ${game.bombSound?"checked":""} ${dis}><span class="form-check-label">🔊 Son d’explosion</span></label><label class="form-check form-switch"><input id="${prefix}Vibration" class="form-check-input" type="checkbox" ${game.bombVibration?"checked":""} ${dis}><span class="form-check-label">📳 Vibration</span></label><label class="form-check form-switch"><input id="${prefix}Custom" class="form-check-input" type="checkbox" ${game.bombIncludeCustom?"checked":""} ${dis}><span class="form-check-label">✨ Mes cartes</span></label></div></section>
  ${readOnly?"":`<section class="card bomb-custom-v44"><h2 class="section-title">Ajouter une carte</h2><div class="form-group"><label for="${prefix}CustomPrompt">Consigne</label><input id="${prefix}CustomPrompt" class="text-input" maxlength="160" placeholder="Ex. Cite un objet que l’on emporte en festival."></div><div class="setup-grid-v44"><select id="${prefix}CustomTheme" class="text-input">${AK_BOMB_THEMES_V44.map(row=>`<option value="${row[0]}">${row[2]}</option>`).join("")}</select><select id="${prefix}CustomDifficulty" class="text-input">${AK_BOMB_LEVELS_V44.map(row=>`<option value="${row[0]}">${row[2]}</option>`).join("")}</select></div><button id="${prefix}AddCustom" class="secondary-btn full">Ajouter ma carte</button><div class="custom-card-list-v44">${game.bombCustomCards.map(card=>`<div><span>${escapeHtml(card.prompt)}</span><button class="icon-btn" data-bomb-delete-v44="${escapeHtml(card.id)}" aria-label="Supprimer">×</button></div>`).join("")||`<p class="helper">Aucune carte personnelle pour le moment.</p>`}</div></section>`}`;
}
function akBombBindSetupV44(game,prefix,rerender,readOnly=false){
  if(readOnly)return; const byId=id=>document.querySelector(`#${id}`);
  byId(`${prefix}Rounds`)?.addEventListener("change",e=>game.roundCount=Number(e.target.value));
  byId(`${prefix}Mode`)?.addEventListener("change",e=>{game.bombMode=e.target.value;rerender();});
  byId(`${prefix}Min`)?.addEventListener("change",e=>{game.bombMinSeconds=Number(e.target.value);if(game.bombMaxSeconds<game.bombMinSeconds)game.bombMaxSeconds=game.bombMinSeconds;rerender();});
  byId(`${prefix}Max`)?.addEventListener("change",e=>{game.bombMaxSeconds=Math.max(game.bombMinSeconds,Number(e.target.value));rerender();});
  byId(`${prefix}Order`)?.addEventListener("change",e=>game.bombOrder=e.target.value); byId(`${prefix}Skips`)?.addEventListener("change",e=>game.bombSkipLimit=Number(e.target.value)); byId(`${prefix}Lives`)?.addEventListener("change",e=>game.bombLives=Number(e.target.value));
  byId(`${prefix}Sound`)?.addEventListener("change",e=>game.bombSound=e.target.checked); byId(`${prefix}Vibration`)?.addEventListener("change",e=>game.bombVibration=e.target.checked); byId(`${prefix}Custom`)?.addEventListener("change",e=>game.bombIncludeCustom=e.target.checked);
  document.querySelectorAll("[data-bomb-theme-v44]").forEach(input=>input.addEventListener("change",()=>{game.bombThemes=[...document.querySelectorAll("[data-bomb-theme-v44]:checked")].map(n=>n.dataset.bombThemeV44);}));
  document.querySelectorAll("[data-bomb-level-v44]").forEach(input=>input.addEventListener("change",()=>{game.bombDifficulties=[...document.querySelectorAll("[data-bomb-level-v44]:checked")].map(n=>n.dataset.bombLevelV44);}));
  byId(`${prefix}AddCustom`)?.addEventListener("click",()=>{const prompt=byId(`${prefix}CustomPrompt`)?.value.trim();if(!prompt)return alert("Écris une consigne.");const rows=akBombCustomCardsV44();rows.push({id:`bombe_custom_${Date.now()}`,prompt,category:prompt,theme:byId(`${prefix}CustomTheme`).value,difficulty:byId(`${prefix}CustomDifficulty`).value,answerMode:"text",minimumPlayers:2,allowRepeats:false,examples:[],custom:true});akBombSaveCustomCardsV44(rows);game.bombCustomCards=rows;rerender();});
  document.querySelectorAll("[data-bomb-delete-v44]").forEach(button=>button.addEventListener("click",()=>{const rows=akBombCustomCardsV44().filter(card=>card.id!==button.dataset.bombDeleteV44);akBombSaveCustomCardsV44(rows);game.bombCustomCards=rows;rerender();}));
}
function akBombBuildPoolV44(raw,game){akBombEnsureV44(game);let pool=(Array.isArray(raw)?raw:[]).filter(card=>game.bombThemes.includes(card.theme)&&game.bombDifficulties.includes(card.difficulty));if(game.bombIncludeCustom)pool=pool.concat(game.bombCustomCards||[]);return pool;}
function akBombShuffleV44(rows){const out=[...rows];for(let i=out.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[out[i],out[j]]=[out[j],out[i]];}return out;}
function akBombFreshSelectV44(pool,count,key){const history=akBombReadJsonV44(AK_BOMB_HISTORY_KEY_V44,{}),recent=new Set(history[key]||[]);const selected=[...akBombShuffleV44(pool.filter(x=>!recent.has(x.id))),...akBombShuffleV44(pool.filter(x=>recent.has(x.id)))].slice(0,Math.min(count,pool.length));history[key]=[...selected.map(x=>x.id),...(history[key]||[])].filter((id,i,a)=>a.indexOf(id)===i).slice(0,180);akBombWriteJsonV44(AK_BOMB_HISTORY_KEY_V44,history);return selected;}
function akBombActivePlayersV44(game){return state.players.filter(p=>game.bombMode!=="elimination"||Number(game.bombLivesMap?.[p.id]||0)>0);}
function akBombChooseNextIndexV44(game,currentIndex){const active=akBombActivePlayersV44(game);if(!active.length)return 0;if(game.bombOrder==="random"&&active.length>1){const current=state.players[currentIndex%state.players.length]?.id;const choices=active.filter(p=>p.id!==current);return state.players.findIndex(p=>p.id===choices[Math.floor(Math.random()*choices.length)]?.id);}for(let step=1;step<=state.players.length;step++){const idx=(currentIndex+step)%state.players.length;if(active.some(p=>p.id===state.players[idx]?.id))return idx;}return currentIndex;}
function akBombStartRoundV44(game){game.bombRoundDuration=akBombRandomDurationV44(game);game.durationSeconds=game.bombRoundDuration;game.bombEndsAt=Date.now()+game.bombRoundDuration*1000;game.bombUsedAnswers=[];game.bombRejected="";game.bombPlayerIndex=Math.max(0,akBombChooseNextIndexV44(game,Math.floor(Math.random()*Math.max(1,state.players.length))-1));}
function akBombEffectTextV44(item){const map={reverse:"Passe ensuite la bombe dans le sens inverse.",double:"Deux réponses sont nécessaires.",chain:"Ta réponse doit commencer par la dernière lettre de la réponse précédente.",fast:"Deux secondes maximum.",one_word:"Un seul mot.",no_e:"Lettre E interdite.",no_a:"Lettre A interdite.",no_i:"Lettre I interdite."};return map[item?.chaosEffect]||"";}
function akBombFeedbackV44(game){return game.bombRejected?`<div class="notice danger-notice-v44">⚠️ ${escapeHtml(game.bombRejected)}</div>`:"";}
function akBombPlayExplosionV44(game){try{if(game.bombVibration&&navigator.vibrate)navigator.vibrate([180,70,300]);if(game.bombSound){const Ctx=window.AudioContext||window.webkitAudioContext;if(Ctx){const ctx=new Ctx(),osc=ctx.createOscillator(),gain=ctx.createGain();osc.type="sawtooth";osc.frequency.setValueAtTime(180,ctx.currentTime);osc.frequency.exponentialRampToValueAtTime(55,ctx.currentTime+.45);gain.gain.setValueAtTime(.18,ctx.currentTime);gain.gain.exponentialRampToValueAtTime(.001,ctx.currentTime+.5);osc.connect(gain);gain.connect(ctx.destination);osc.start();osc.stop(ctx.currentTime+.5);}}}catch(_){}}
function akBombSubmitLocalV44(skip=false){const game=state.megaGame,item=game.items[game.currentIndex],player=state.players[game.bombPlayerIndex%state.players.length];if(!game||!player)return;let answer="";if(!skip&&item.answerMode!=="manual"){answer=document.querySelector("#bombAnswerV44")?.value.trim()||"";if(!answer){game.bombRejected="Écris une réponse avant de passer.";return renderMegaBombRoundV44();}const normalized=akBombNormalizeV44(answer);if(game.bombUsedAnswers.some(row=>row.normalized===normalized)){game.bombRejected=`« ${answer} » a déjà été donné.`;return renderMegaBombRoundV44();}game.bombUsedAnswers.push({playerId:player.id,answer,normalized});}else if(!skip){game.bombUsedAnswers.push({playerId:player.id,answer:"Défi validé",normalized:`manual_${game.bombUsedAnswers.length}`});}
  if(skip){const left=Number(game.bombSkips?.[player.id]||0);if(left<=0)return;game.bombSkips[player.id]=left-1;}else{game.bombAnswerCounts[player.id]=Number(game.bombAnswerCounts[player.id]||0)+1;if(game.bombMode==="points")game.scores[player.id]=Number(game.scores[player.id]||0)+1;}
  if(game.bombMode==="infernal")game.bombEndsAt=Math.max(Date.now()+1200,game.bombEndsAt-1200);if(item.chaosEffect==="reverse")game.bombDirection*=-1;game.bombRejected="";game.bombPlayerIndex=akBombChooseNextIndexV44(game,game.bombPlayerIndex+(game.bombDirection<0?-2:0));renderMegaBombRoundV44();
}
const akBombBaseResetMegaV44=resetMegaGame;
resetMegaGame=function resetMegaGameBombV44(gameName,replayConfig={}){akBombBaseResetMegaV44(gameName,replayConfig);if(gameName===AK_BOMB_GAME_V44)akBombEnsureV44(state.megaGame,replayConfig);};
const akBombBaseRenderSetupV44=renderMegaSetup;
renderMegaSetup=function renderMegaSetupBombV44(){const game=state.megaGame;if(!game||game.gameName!==AK_BOMB_GAME_V44)return akBombBaseRenderSetupV44();akBombEnsureV44(game);clearV014Timer();title.textContent=AK_BOMB_GAME_V44;setBackVisible(true);screen.innerHTML=`<section class="game-cover game-cover-mega engine-bomb bomb-cover-v44"><span class="game-cover-icon">💣</span><div><small>600 CARTES · 5 MODES</small><h2>La Bombe</h2><p>Réponds, passe le téléphone et ne reste pas avec la patate chaude numérique.</p></div></section>${akBombSetupMarkupV44(game,"bombLocal",false)}<div class="notice">La durée exacte reste secrète. Le téléphone se passe de main en main, il ne se lance jamais.</div><button id="startMegaGame" class="primary-btn full">Allumer la mèche</button>`;akBombBindSetupV44(game,"bombLocal",renderMegaSetup,false);document.querySelector("#startMegaGame")?.addEventListener("click",startMegaGame);};
const akBombBaseStartMegaV44=startMegaGame;
startMegaGame=async function startMegaGameBombV44(){const game=state.megaGame;if(!game||game.gameName!==AK_BOMB_GAME_V44)return akBombBaseStartMegaV44();akBombEnsureV44(game);if(state.players.length<2)return alert("La Bombe demande au moins deux joueurs.");if(!game.bombThemes.length&&!(game.bombIncludeCustom&&game.bombCustomCards.length))return alert("Choisis au moins un thème ou ajoute une carte.");if(!game.bombDifficulties.length&&!(game.bombIncludeCustom&&game.bombCustomCards.length))return alert("Choisis au moins une difficulté.");screen.innerHTML=`<div class="notice">La mèche s’allume…</div>`;try{const raw=await loadJsonFile("data/bombe.json","Impossible de charger les cartes de La Bombe.");const pool=akBombBuildPoolV44(raw,game);if(!pool.length)throw new Error("Aucune carte ne correspond à ces réglages.");game.items=akBombFreshSelectV44(pool,game.roundCount,`solo:${game.bombMode}:${game.bombThemes.join("-")}:${game.bombDifficulties.join("-")}`);game.currentIndex=0;game.scores=v014ScoreMap();game.rounds=[];game.currentResult=null;game.bombDirection=1;game.bombLivesMap=Object.fromEntries(state.players.map(p=>[p.id,game.bombLives]));game.bombSkips=Object.fromEntries(state.players.map(p=>[p.id,game.bombSkipLimit]));game.bombAnswerCounts=v014ScoreMap();game.bombExplosions=v014ScoreMap();game.bombTeams=Object.fromEntries(state.players.map((p,i)=>[p.id,i%2]));akBombSavePrefsV44(game);akBombStartRoundV44(game);renderMegaCurrent();}catch(error){console.error(error);alert(error.message||"Impossible de lancer La Bombe.");renderMegaSetup();}};
const akBombBaseRenderCurrentV44=renderMegaCurrent;
renderMegaCurrent=function renderMegaCurrentBombV44(){if(state.megaGame?.gameName===AK_BOMB_GAME_V44){if(state.megaGame.currentIndex>=state.megaGame.items.length)return renderMegaFinal();return renderMegaBombRoundV44();}return akBombBaseRenderCurrentV44();};
function renderMegaBombRoundV44(){const game=state.megaGame;if(!game.bombEndsAt)akBombStartRoundV44(game);const item=game.items[game.currentIndex],player=state.players[game.bombPlayerIndex%state.players.length],leftSkip=Number(game.bombSkips?.[player.id]||0),manual=item.answerMode==="manual";title.textContent=game.gameName;setBackVisible(false);screen.innerHTML=`${v014Progress(game,"Bombe")}<section class="bomb-stage bomb-stage-v44"><div class="bomb-icon">💣</div><div class="bomb-secret-clock-v44"><strong>?</strong><span>durée secrète</span><span id="v014BombCountdown" class="bomb-hidden-count-v44">${Math.max(0,Math.ceil((game.bombEndsAt-Date.now())/1000))}</span></div><span class="category-chip">${avatarById(player.avatarId).emoji} ${escapeHtml(player.name).toUpperCase()}</span><h2>${escapeHtml(item.prompt||item.category)}</h2>${akBombEffectTextV44(item)?`<p class="bomb-effect-v44">💥 ${escapeHtml(akBombEffectTextV44(item))}</p>`:""}${item.alternative?`<details class="notice"><summary>Alternative douce</summary><p>${escapeHtml(item.alternative)}</p></details>`:""}<div class="progress-track"><div id="v014TimerFill" class="progress-fill" style="width:100%"></div></div></section>${akBombFeedbackV44(game)}${game.bombUsedAnswers.length?`<section class="bomb-answer-history-v44"><small>RÉPONSES DÉJÀ DONNÉES</small><div>${game.bombUsedAnswers.slice(-10).map(row=>`<span>${escapeHtml(row.answer)}</span>`).join("")}</div></section>`:""}<section class="card bomb-answer-card-v44">${manual?`<p class="helper">Le groupe valide le son ou le geste.</p><button id="bombPassV44" class="primary-btn full">Défi fait, je passe →</button>`:`<label for="bombAnswerV44">Ta réponse</label><div class="bomb-answer-row-v44"><input id="bombAnswerV44" class="text-input" maxlength="60" autocomplete="off" placeholder="Réponse différente…"><button id="bombPassV44" class="primary-btn">Je passe →</button></div>`}<div class="toolbar"><button id="bombSkipV44" class="secondary-btn" ${leftSkip>0?"":"disabled"}>Joker (${leftSkip})</button><button id="bombExplodeV44" class="danger-btn">💥 Elle a explosé</button></div></section>`;document.querySelector("#bombPassV44")?.addEventListener("click",()=>akBombSubmitLocalV44(false));document.querySelector("#bombAnswerV44")?.addEventListener("keydown",e=>{if(e.key==="Enter")akBombSubmitLocalV44(false);});document.querySelector("#bombSkipV44")?.addEventListener("click",()=>akBombSubmitLocalV44(true));document.querySelector("#bombExplodeV44")?.addEventListener("click",()=>finishMegaBombV44(player.id));startV014Timer(game.bombEndsAt,"#v014BombCountdown",()=>finishMegaBombV44(state.players[game.bombPlayerIndex%state.players.length].id),game.bombRoundDuration);document.querySelector("#bombAnswerV44")?.focus();}
function finishMegaBombV44(loserId){const game=state.megaGame;if(!game||game.currentResult)return;clearV014Timer();akBombPlayExplosionV44(game);const loser=state.players.find(p=>p.id===loserId);game.bombExplosions[loserId]=Number(game.bombExplosions[loserId]||0)+1;let resultText="";if(game.bombMode==="points"){game.scores[loserId]=Number(game.scores[loserId]||0)-2;resultText="−2 points pour la personne qui tenait la bombe.";}else if(game.bombMode==="elimination"){game.bombLivesMap[loserId]=Math.max(0,Number(game.bombLivesMap[loserId]||0)-1);const active=akBombActivePlayersV44(game);if(active.length===1)game.scores[active[0].id]=Number(game.scores[active[0].id]||0)+5;resultText=`Il reste ${game.bombLivesMap[loserId]} vie${game.bombLivesMap[loserId]===1?"":"s"} à ${loser?.name||"cette personne"}.`;}else if(game.bombMode==="teams"){const losingTeam=game.bombTeams[loserId],winningTeam=losingTeam===0?1:0;state.players.filter(p=>game.bombTeams[p.id]===winningTeam).forEach(p=>game.scores[p.id]=Number(game.scores[p.id]||0)+1);resultText=`L’équipe ${winningTeam===0?"A":"B"} gagne la manche.`;}else{state.players.filter(p=>p.id!==loserId).forEach(p=>game.scores[p.id]=Number(game.scores[p.id]||0)+1);resultText="Toutes les autres personnes marquent 1 point.";}game.currentResult={loserId,itemId:game.items[game.currentIndex]?.id,answers:[...game.bombUsedAnswers],duration:game.bombRoundDuration};game.rounds.push(game.currentResult);const eliminationDone=game.bombMode==="elimination"&&akBombActivePlayersV44(game).length<=1;title.textContent="BOOM !";screen.innerHTML=`${v014Progress(game,"Bombe")}<section class="winner-stage bomb-result-stage bomb-result-v44"><div class="winner-crown">💥</div><div class="giant-avatar">${avatarById(loser?.avatarId).emoji}</div><h2>La bombe explose chez ${escapeHtml(loser?.name||"un joueur")}</h2><p>${escapeHtml(resultText)}</p></section>${game.bombUsedAnswers.length?`<section class="bomb-answer-history-v44"><small>${game.bombUsedAnswers.length} RÉPONSES VALIDÉES</small><div>${game.bombUsedAnswers.map(row=>`<span>${escapeHtml(row.answer)}</span>`).join("")}</div></section>`:""}<button id="nextBombV44" class="primary-btn full">${eliminationDone||game.currentIndex+1>=game.items.length?"Voir le classement":"Nouvelle bombe"}</button>`;document.querySelector("#nextBombV44")?.addEventListener("click",()=>{if(eliminationDone)game.currentIndex=game.items.length;else game.currentIndex+=1;game.currentResult=null;game.bombEndsAt=null;if(game.currentIndex<game.items.length)akBombStartRoundV44(game);renderMegaCurrent();});}
const akBombBaseRenderFinalV44=renderMegaFinal;
renderMegaFinal=function renderMegaFinalBombV44(){const game=state.megaGame;if(!game||game.gameName!==AK_BOMB_GAME_V44)return akBombBaseRenderFinalV44();clearV014Timer();const ranking=[...state.players].sort((a,b)=>Number(game.scores[b.id]||0)-Number(game.scores[a.id]||0)),top=Number(game.scores[ranking[0]?.id]||0),winners=ranking.filter(p=>Number(game.scores[p.id]||0)===top),answerKing=[...state.players].sort((a,b)=>Number(game.bombAnswerCounts[b.id]||0)-Number(game.bombAnswerCounts[a.id]||0))[0],boomKing=[...state.players].sort((a,b)=>Number(game.bombExplosions[b.id]||0)-Number(game.bombExplosions[a.id]||0))[0];title.textContent="Bilan explosif";setBackVisible(false);screen.innerHTML=`<section class="winner-stage winner-stage-v07 mega-final-stage"><div class="winner-crown">💣🏆</div><h2>${winners.map(p=>escapeHtml(p.name)).join(" et ")||"Partie terminée"}</h2><p>La mèche est éteinte. Les réponses douteuses restent dans les archives.</p></section><section class="final-ranking">${ranking.map((p,i)=>`<div class="ranking-row"><span class="ranking-position">${i+1}</span><span class="result-avatar">${avatarById(p.avatarId).emoji}</span><strong>${escapeHtml(p.name)}</strong><span>${Number(game.scores[p.id]||0)} pts</span></div>`).join("")}</section><section class="impostor-awards">${answerKing?`<article><span>⚡</span><div><small>PLUS DE RÉPONSES</small><strong>${escapeHtml(answerKing.name)}</strong><p>${Number(game.bombAnswerCounts[answerKing.id]||0)} réponses</p></div></article>`:""}${boomKing?`<article><span>💥</span><div><small>A LE PLUS EXPLOSÉ</small><strong>${escapeHtml(boomKing.name)}</strong><p>${Number(game.bombExplosions[boomKing.id]||0)} explosions</p></div></article>`:""}</section><div class="toolbar"><button id="bombReplayV44" class="secondary-btn">Rejouer</button><button id="bombExitV44" class="primary-btn">Autre jeu</button></div>`;document.querySelector("#bombReplayV44")?.addEventListener("click",()=>{const replay={roundCount:game.roundCount,bombThemes:game.bombThemes,bombDifficulties:game.bombDifficulties,bombMode:game.bombMode,bombMinSeconds:game.bombMinSeconds,bombMaxSeconds:game.bombMaxSeconds,bombOrder:game.bombOrder,bombLives:game.bombLives,bombSkipLimit:game.bombSkipLimit,bombSound:game.bombSound,bombVibration:game.bombVibration,bombIncludeCustom:game.bombIncludeCustom};resetMegaGame(AK_BOMB_GAME_V44,replay);renderMegaSetup();});document.querySelector("#bombExitV44")?.addEventListener("click",()=>{state.megaGame=null;renderPlayChoice();});};


/* AKGAMES LA BOMBE ADULTE 350 V45 */
const AK_BOMB_ADULT_GAME_V45 = "La Bombe +18";
const AK_BOMB_ADULT_PREFS_KEY_V45 = "akgames:bomb-adult:v45:prefs";
const AK_BOMB_ADULT_CUSTOM_KEY_V45 = "akgames:bomb-adult:v45:custom";
const AK_BOMB_ADULT_HISTORY_KEY_V45 = "akgames:bomb-adult:v45:history";
const AK_BOMB_ADULT_THEMES_V45 = [
  ["confidences","🤫","Confidences"],["seduction","💘","Séduction"],["couple","💞","Couple"],["ex","↩️","Ex"],["messages","📱","Messages"],
  ["soirees","🎉","Soirées"],["preferences","🌶️","Préférences"],["vocal_geste","🎭","Voix & gestes"],["chaos","💥","Chaos"]
];
const AK_BOMB_ADULT_LEVELS_V45 = [["leger","🌶️","Léger"],["ose","🌶️🌶️","Osé"],["tres_ose","🌶️🌶️🌶️","Très osé"],["chaos","🌀","Chaos"]];
const AK_BOMB_ADULT_FILTERS_V45 = [
  ["allowEx","ex","↩️","Sujets sur les ex"],["allowIntimacy","intimacy","🌶️","Intimité et limites"],["allowCouple","couple","💞","Couple"],
  ["allowPhone","phone","📱","Téléphone et messages"],["allowPhysical","physical","🎭","Défis gestuels"],["allowTarget","target","🎯","Cartes visant le groupe"],["allowAlcohol","alcohol","🥂","Alcool"]
];
function akBombAdultCustomCardsV45(){const rows=akBombReadJsonV44(AK_BOMB_ADULT_CUSTOM_KEY_V45,[]);return Array.isArray(rows)?rows:[];}
function akBombAdultSaveCustomCardsV45(rows){akBombWriteJsonV44(AK_BOMB_ADULT_CUSTOM_KEY_V45,rows);}
function akBombAdultEnsureV45(game,replay={}){
  if(!game||game.gameName!==AK_BOMB_ADULT_GAME_V45)return game;
  const saved=akBombReadJsonV44(AK_BOMB_ADULT_PREFS_KEY_V45,{}),themes=AK_BOMB_ADULT_THEMES_V45.map(r=>r[0]),levels=AK_BOMB_ADULT_LEVELS_V45.map(r=>r[0]);
  game.bombThemes=Array.isArray(replay.bombThemes||saved.bombThemes)?(replay.bombThemes||saved.bombThemes).filter(v=>themes.includes(v)):themes;
  game.bombDifficulties=Array.isArray(replay.bombDifficulties||saved.bombDifficulties)?(replay.bombDifficulties||saved.bombDifficulties).filter(v=>levels.includes(v)):levels;
  game.bombMode=String(replay.bombMode||saved.bombMode||"classic");if(!AK_BOMB_MODES_V44.some(r=>r[0]===game.bombMode))game.bombMode="classic";
  game.bombMinSeconds=Math.max(10,Math.min(60,Number(replay.bombMinSeconds||saved.bombMinSeconds||15)));game.bombMaxSeconds=Math.max(game.bombMinSeconds,Math.min(60,Number(replay.bombMaxSeconds||saved.bombMaxSeconds||40)));
  game.bombOrder=String(replay.bombOrder||saved.bombOrder||"fixed");game.bombLives=Math.max(1,Math.min(5,Number(replay.bombLives||saved.bombLives||3)));game.bombSkipLimit=Math.max(0,Math.min(3,Number(replay.bombSkipLimit??saved.bombSkipLimit??1)));
  game.bombSound=(replay.bombSound??saved.bombSound)!==false;game.bombVibration=(replay.bombVibration??saved.bombVibration)!==false;game.bombIncludeCustom=(replay.bombIncludeCustom??saved.bombIncludeCustom)!==false;
  AK_BOMB_ADULT_FILTERS_V45.forEach(([key])=>{game[key]=(replay[key]??saved[key]??(key!=="allowAlcohol"))!==false;});
  game.bombCustomCards=akBombAdultCustomCardsV45();game.roundCount=Math.max(5,Math.min(50,Number(replay.roundCount||game.roundCount||10)));return game;
}
function akBombAdultSavePrefsV45(game){const out={bombThemes:game.bombThemes,bombDifficulties:game.bombDifficulties,bombMode:game.bombMode,bombMinSeconds:game.bombMinSeconds,bombMaxSeconds:game.bombMaxSeconds,bombOrder:game.bombOrder,bombLives:game.bombLives,bombSkipLimit:game.bombSkipLimit,bombSound:game.bombSound,bombVibration:game.bombVibration,bombIncludeCustom:game.bombIncludeCustom,roundCount:game.roundCount};AK_BOMB_ADULT_FILTERS_V45.forEach(([key])=>out[key]=game[key]);akBombWriteJsonV44(AK_BOMB_ADULT_PREFS_KEY_V45,out);}
function akBombAdultSetupMarkupV45(game,prefix="bombAdult",readOnly=false){akBombAdultEnsureV45(game);const dis=readOnly?"disabled":"";return `<section class="card setup-card-v07 bomb-setup-v44"><div class="setup-grid-v44"><div class="form-group"><label for="${prefix}Rounds">Nombre de manches</label><select id="${prefix}Rounds" class="text-input" ${dis}>${[5,8,10,12,15,20,25,30,40,50].map(v=>`<option value="${v}" ${game.roundCount===v?"selected":""}>${v} manches</option>`).join("")}</select></div><div class="form-group"><label for="${prefix}Mode">Mode</label><select id="${prefix}Mode" class="text-input" ${dis}>${AK_BOMB_MODES_V44.map(r=>`<option value="${r[0]}" ${game.bombMode===r[0]?"selected":""}>${r[1]}</option>`).join("")}</select></div></div><p class="helper bomb-mode-helper-v44">${escapeHtml(AK_BOMB_MODES_V44.find(r=>r[0]===game.bombMode)?.[2]||"")}</p><div class="setup-grid-v44"><div class="form-group"><label for="${prefix}Min">Explosion au plus tôt</label><select id="${prefix}Min" class="text-input" ${dis}>${[10,15,20,25,30,40].map(v=>`<option value="${v}" ${game.bombMinSeconds===v?"selected":""}>${v} s</option>`).join("")}</select></div><div class="form-group"><label for="${prefix}Max">Explosion au plus tard</label><select id="${prefix}Max" class="text-input" ${dis}>${[20,25,30,40,45,60].map(v=>`<option value="${v}" ${game.bombMaxSeconds===v?"selected":""}>${v} s</option>`).join("")}</select></div></div><div class="setup-grid-v44"><div class="form-group"><label for="${prefix}Order">Ordre</label><select id="${prefix}Order" class="text-input" ${dis}><option value="fixed" ${game.bombOrder==="fixed"?"selected":""}>Ordre fixe</option><option value="random" ${game.bombOrder==="random"?"selected":""}>Aléatoire</option></select></div><div class="form-group"><label for="${prefix}Skips">Jokers par joueur</label><select id="${prefix}Skips" class="text-input" ${dis}>${[0,1,2,3].map(v=>`<option value="${v}" ${game.bombSkipLimit===v?"selected":""}>${v}</option>`).join("")}</select></div></div><div class="form-group bomb-lives-wrap-v44" ${game.bombMode==="elimination"?"":"hidden"}><label for="${prefix}Lives">Vies</label><select id="${prefix}Lives" class="text-input" ${dis}>${[1,2,3,4,5].map(v=>`<option value="${v}" ${game.bombLives===v?"selected":""}>${v}</option>`).join("")}</select></div></section><section class="card"><h2 class="section-title">Thèmes</h2><div class="filter-chip-grid-v44">${AK_BOMB_ADULT_THEMES_V45.map(r=>`<label class="form-check"><input class="form-check-input" type="checkbox" data-bomb-adult-theme-v45="${r[0]}" ${game.bombThemes.includes(r[0])?"checked":""} ${dis}><span class="form-check-label">${r[1]} ${r[2]}</span></label>`).join("")}</div></section><section class="card"><h2 class="section-title">Intensités</h2><div class="filter-chip-grid-v44">${AK_BOMB_ADULT_LEVELS_V45.map(r=>`<label class="form-check"><input class="form-check-input" type="checkbox" data-bomb-adult-level-v45="${r[0]}" ${game.bombDifficulties.includes(r[0])?"checked":""} ${dis}><span class="form-check-label">${r[1]} ${r[2]}</span></label>`).join("")}</div></section><section class="card"><h2 class="section-title">Filtres de confort</h2><p class="helper">Décoche ce que le groupe préfère éviter. Le joker reste toujours sans pénalité.</p><div class="filter-chip-grid-v44">${AK_BOMB_ADULT_FILTERS_V45.map(([key,,icon,label])=>`<label class="form-check form-switch"><input id="${prefix}${key}" class="form-check-input" type="checkbox" ${game[key]?"checked":""} ${dis}><span class="form-check-label">${icon} ${label}</span></label>`).join("")}</div></section><section class="card"><h2 class="section-title">Effets</h2><div class="filter-chip-grid-v44"><label class="form-check form-switch"><input id="${prefix}Sound" class="form-check-input" type="checkbox" ${game.bombSound?"checked":""} ${dis}><span class="form-check-label">🔊 Son</span></label><label class="form-check form-switch"><input id="${prefix}Vibration" class="form-check-input" type="checkbox" ${game.bombVibration?"checked":""} ${dis}><span class="form-check-label">📳 Vibration</span></label><label class="form-check form-switch"><input id="${prefix}Custom" class="form-check-input" type="checkbox" ${game.bombIncludeCustom?"checked":""} ${dis}><span class="form-check-label">✨ Mes cartes</span></label></div></section>${readOnly?"":`<section class="card bomb-custom-v44"><h2 class="section-title">Ajouter une carte adulte</h2><input id="${prefix}CustomPrompt" class="text-input" maxlength="180" placeholder="Une consigne consentie et jouable…"><div class="setup-grid-v44"><select id="${prefix}CustomTheme" class="text-input">${AK_BOMB_ADULT_THEMES_V45.map(r=>`<option value="${r[0]}">${r[2]}</option>`).join("")}</select><select id="${prefix}CustomDifficulty" class="text-input">${AK_BOMB_ADULT_LEVELS_V45.map(r=>`<option value="${r[0]}">${r[2]}</option>`).join("")}</select></div><button id="${prefix}AddCustom" class="secondary-btn full">Ajouter ma carte</button><div class="custom-card-list-v44">${game.bombCustomCards.map(c=>`<div><span>${escapeHtml(c.prompt)}</span><button class="icon-btn" data-bomb-adult-delete-v45="${escapeHtml(c.id)}">×</button></div>`).join("")||`<p class="helper">Aucune carte personnelle.</p>`}</div></section>`}`;}
function akBombAdultBindSetupV45(game,prefix,rerender,readOnly=false){if(readOnly)return;const byId=id=>document.querySelector(`#${id}`);byId(`${prefix}Rounds`)?.addEventListener("change",e=>game.roundCount=Number(e.target.value));byId(`${prefix}Mode`)?.addEventListener("change",e=>{game.bombMode=e.target.value;rerender();});byId(`${prefix}Min`)?.addEventListener("change",e=>{game.bombMinSeconds=Number(e.target.value);if(game.bombMaxSeconds<game.bombMinSeconds)game.bombMaxSeconds=game.bombMinSeconds;rerender();});byId(`${prefix}Max`)?.addEventListener("change",e=>{game.bombMaxSeconds=Math.max(game.bombMinSeconds,Number(e.target.value));rerender();});byId(`${prefix}Order`)?.addEventListener("change",e=>game.bombOrder=e.target.value);byId(`${prefix}Skips`)?.addEventListener("change",e=>game.bombSkipLimit=Number(e.target.value));byId(`${prefix}Lives`)?.addEventListener("change",e=>game.bombLives=Number(e.target.value));byId(`${prefix}Sound`)?.addEventListener("change",e=>game.bombSound=e.target.checked);byId(`${prefix}Vibration`)?.addEventListener("change",e=>game.bombVibration=e.target.checked);byId(`${prefix}Custom`)?.addEventListener("change",e=>game.bombIncludeCustom=e.target.checked);AK_BOMB_ADULT_FILTERS_V45.forEach(([key])=>byId(`${prefix}${key}`)?.addEventListener("change",e=>game[key]=e.target.checked));document.querySelectorAll("[data-bomb-adult-theme-v45]").forEach(n=>n.addEventListener("change",()=>game.bombThemes=[...document.querySelectorAll("[data-bomb-adult-theme-v45]:checked")].map(x=>x.dataset.bombAdultThemeV45)));document.querySelectorAll("[data-bomb-adult-level-v45]").forEach(n=>n.addEventListener("change",()=>game.bombDifficulties=[...document.querySelectorAll("[data-bomb-adult-level-v45]:checked")].map(x=>x.dataset.bombAdultLevelV45)));byId(`${prefix}AddCustom`)?.addEventListener("click",()=>{const prompt=byId(`${prefix}CustomPrompt`)?.value.trim();if(!prompt)return alert("Écris une consigne.");const rows=akBombAdultCustomCardsV45();rows.push({id:`bombe_adulte_custom_${Date.now()}`,prompt,category:prompt,theme:byId(`${prefix}CustomTheme`).value,difficulty:byId(`${prefix}CustomDifficulty`).value,intensity:byId(`${prefix}CustomDifficulty`).value,answerMode:"text",minimumPlayers:2,allowRepeats:false,examples:[],sensitive:[],alternative:"Alternative douce : passe la carte sans perdre de point.",custom:true});akBombAdultSaveCustomCardsV45(rows);game.bombCustomCards=rows;rerender();});document.querySelectorAll("[data-bomb-adult-delete-v45]").forEach(b=>b.addEventListener("click",()=>{const rows=akBombAdultCustomCardsV45().filter(c=>c.id!==b.dataset.bombAdultDeleteV45);akBombAdultSaveCustomCardsV45(rows);game.bombCustomCards=rows;rerender();}));}
function akBombAdultBuildPoolV45(raw,game){akBombAdultEnsureV45(game);const blocked=new Set(AK_BOMB_ADULT_FILTERS_V45.filter(([key])=>!game[key]).map(([,tag])=>tag));let pool=(Array.isArray(raw)?raw:[]).filter(c=>game.bombThemes.includes(c.theme)&&game.bombDifficulties.includes(c.difficulty)&&!(c.sensitive||[]).some(tag=>blocked.has(tag)));if(game.bombIncludeCustom)pool=pool.concat(game.bombCustomCards||[]);return pool;}
function akBombAdultFreshSelectV45(pool,count,key){const history=akBombReadJsonV44(AK_BOMB_ADULT_HISTORY_KEY_V45,{}),recent=new Set(history[key]||[]);const selected=[...akBombShuffleV44(pool.filter(x=>!recent.has(x.id))),...akBombShuffleV44(pool.filter(x=>recent.has(x.id)))].slice(0,Math.min(count,pool.length));history[key]=[...selected.map(x=>x.id),...(history[key]||[])].filter((id,i,a)=>a.indexOf(id)===i).slice(0,180);akBombWriteJsonV44(AK_BOMB_ADULT_HISTORY_KEY_V45,history);return selected;}
const akBombAdultBaseEffectV45=akBombEffectTextV44;akBombEffectTextV44=function(item){const extra={seductive:"Interprétation charmeuse, sans viser ni toucher personne.",whisper:"Réponds en chuchotant.",robot:"Voix de robot.",dramatic:"Ajoute une interprétation dramatique.",presenter:"Parle comme un présentateur officiel.",slow:"Réponds très lentement.",poker:"Garde un visage parfaitement neutre.",sing:"Réponds en chantant.",eyes_closed:"Uniquement avec un geste simple et sûr."};return extra[item?.chaosEffect]||akBombAdultBaseEffectV45(item);};
const akBombAdultBaseResetV45=resetMegaGame;resetMegaGame=function(gameName,replayConfig={}){akBombAdultBaseResetV45(gameName,replayConfig);if(gameName===AK_BOMB_ADULT_GAME_V45)akBombAdultEnsureV45(state.megaGame,replayConfig);};
const akBombAdultBaseSetupV45=renderMegaSetup;renderMegaSetup=function(){const game=state.megaGame;if(!game||game.gameName!==AK_BOMB_ADULT_GAME_V45)return akBombAdultBaseSetupV45();akBombAdultEnsureV45(game);clearV014Timer();title.textContent=AK_BOMB_ADULT_GAME_V45;setBackVisible(true);screen.innerHTML=`<section class="game-cover game-cover-mega engine-bomb bomb-cover-v44"><span class="game-cover-icon">🌶️💣</span><div><small>350 CARTES · FILTRES DE CONFORT</small><h2>La Bombe +18</h2><p>Réponds vite, garde tes limites et évite l’explosion.</p></div></section>${akBombAdultSetupMarkupV45(game,"bombAdultLocal",false)}<div class="notice">🔞 Adultes consentants uniquement. Un joker permet toujours de passer sans justification.</div><button id="startMegaGame" class="primary-btn full">Allumer la mèche adulte</button>`;akBombAdultBindSetupV45(game,"bombAdultLocal",renderMegaSetup,false);document.querySelector("#startMegaGame")?.addEventListener("click",startMegaGame);};
const akBombAdultBaseStartV45=startMegaGame;startMegaGame=async function(){const game=state.megaGame;if(!game||game.gameName!==AK_BOMB_ADULT_GAME_V45)return akBombAdultBaseStartV45();akBombAdultEnsureV45(game);if(state.players.length<2)return alert("La Bombe +18 demande au moins deux joueurs.");if(!game.bombThemes.length&&!(game.bombIncludeCustom&&game.bombCustomCards.length))return alert("Choisis au moins un thème ou ajoute une carte.");if(!game.bombDifficulties.length&&!(game.bombIncludeCustom&&game.bombCustomCards.length))return alert("Choisis au moins une intensité.");screen.innerHTML=`<div class="notice">La mèche adulte s’allume…</div>`;try{const raw=await loadJsonFile("data/bombe-adulte.json","Impossible de charger La Bombe +18.");const pool=akBombAdultBuildPoolV45(raw,game);if(!pool.length)throw new Error("Aucune carte ne correspond à ces réglages et filtres.");game.items=akBombAdultFreshSelectV45(pool,game.roundCount,`solo:${game.bombMode}:${game.bombThemes.join("-")}:${game.bombDifficulties.join("-")}`);game.currentIndex=0;game.scores=v014ScoreMap();game.rounds=[];game.currentResult=null;game.bombDirection=1;game.bombLivesMap=Object.fromEntries(state.players.map(p=>[p.id,game.bombLives]));game.bombSkips=Object.fromEntries(state.players.map(p=>[p.id,game.bombSkipLimit]));game.bombAnswerCounts=v014ScoreMap();game.bombExplosions=v014ScoreMap();game.bombTeams=Object.fromEntries(state.players.map((p,i)=>[p.id,i%2]));akBombAdultSavePrefsV45(game);akBombStartRoundV44(game);renderMegaCurrent();}catch(error){console.error(error);alert(error.message||"Impossible de lancer La Bombe +18.");renderMegaSetup();}};
const akBombAdultBaseCurrentV45=renderMegaCurrent;renderMegaCurrent=function(){if(state.megaGame?.gameName===AK_BOMB_ADULT_GAME_V45){if(state.megaGame.currentIndex>=state.megaGame.items.length)return renderMegaFinal();return renderMegaBombRoundV44();}return akBombAdultBaseCurrentV45();};
const akBombAdultBaseFinalV45=renderMegaFinal;renderMegaFinal=function(){const game=state.megaGame;if(!game||game.gameName!==AK_BOMB_ADULT_GAME_V45)return akBombAdultBaseFinalV45();clearV014Timer();const ranking=[...state.players].sort((a,b)=>Number(game.scores[b.id]||0)-Number(game.scores[a.id]||0)),top=Number(game.scores[ranking[0]?.id]||0),winners=ranking.filter(p=>Number(game.scores[p.id]||0)===top),answerKing=[...state.players].sort((a,b)=>Number(game.bombAnswerCounts[b.id]||0)-Number(game.bombAnswerCounts[a.id]||0))[0],boomKing=[...state.players].sort((a,b)=>Number(game.bombExplosions[b.id]||0)-Number(game.bombExplosions[a.id]||0))[0];title.textContent="Bilan explosif +18";setBackVisible(false);screen.innerHTML=`<section class="winner-stage winner-stage-v07 mega-final-stage"><div class="winner-crown">🌶️💣🏆</div><h2>${winners.map(p=>escapeHtml(p.name)).join(" et ")||"Partie terminée"}</h2><p>La mèche est éteinte. Les confidences restent dans le groupe.</p></section><section class="final-ranking">${ranking.map((p,i)=>`<div class="ranking-row"><span class="ranking-position">${i+1}</span><span class="result-avatar">${avatarById(p.avatarId).emoji}</span><strong>${escapeHtml(p.name)}</strong><span>${Number(game.scores[p.id]||0)} pts</span></div>`).join("")}</section><section class="impostor-awards">${answerKing?`<article><span>⚡</span><div><small>PLUS DE RÉPONSES</small><strong>${escapeHtml(answerKing.name)}</strong><p>${Number(game.bombAnswerCounts[answerKing.id]||0)} réponses</p></div></article>`:""}${boomKing?`<article><span>💥</span><div><small>A LE PLUS EXPLOSÉ</small><strong>${escapeHtml(boomKing.name)}</strong><p>${Number(game.bombExplosions[boomKing.id]||0)} explosions</p></div></article>`:""}</section><div class="toolbar"><button id="bombAdultReplayV45" class="secondary-btn">Rejouer</button><button id="bombAdultExitV45" class="primary-btn">Autre jeu</button></div>`;document.querySelector("#bombAdultReplayV45")?.addEventListener("click",()=>{const replay={roundCount:game.roundCount,bombThemes:game.bombThemes,bombDifficulties:game.bombDifficulties,bombMode:game.bombMode,bombMinSeconds:game.bombMinSeconds,bombMaxSeconds:game.bombMaxSeconds,bombOrder:game.bombOrder,bombLives:game.bombLives,bombSkipLimit:game.bombSkipLimit,bombSound:game.bombSound,bombVibration:game.bombVibration,bombIncludeCustom:game.bombIncludeCustom};AK_BOMB_ADULT_FILTERS_V45.forEach(([key])=>replay[key]=game[key]);resetMegaGame(AK_BOMB_ADULT_GAME_V45,replay);renderMegaSetup();});document.querySelector("#bombAdultExitV45")?.addEventListener("click",()=>{state.megaGame=null;renderPlayChoice();});};


/* AK'GAMES V4.6 — ALERTE ROUGE 500 */
const AK_ALERT_GAME_V46 = "Alerte Rouge";
const AK_ALERT_THEMES_V46 = [
  ["soiree_maison","🏠","Soirée & maison"],
  ["telephone_reseaux","📱","Téléphone & réseaux"],
  ["voisins_ville","🏙️","Voisins & ville"],
  ["voyage_transport","🧳","Voyages & transports"],
  ["objets_disparus","🗝️","Objets mystérieux"],
  ["mystere_paranormal","👻","Mystère & paranormal"],
  ["travail_ecole","📚","Travail & école"],
  ["nature_exterieur","🌲","Nature & extérieur"],
  ["secrets_groupe","🤫","Secrets du groupe"],
  ["chaos_absurde","🌀","Chaos absurde"]
];
const AK_ALERT_INTENSITIES_V46 = [
  ["petit_probleme","🟡","Petit problème"],
  ["panique","🟠","Panique"],
  ["alerte_rouge","🔴","Alerte rouge"],
  ["chaos","🌀","Chaos"]
];
function akAlertAllV46(list){return list.map(row=>row[0]);}
function akAlertNormalizeV46(values,list){const valid=new Set(akAlertAllV46(list));const out=[...new Set((Array.isArray(values)?values:[]).filter(v=>valid.has(v)))];return out.length?out:akAlertAllV46(list);}
function akAlertEnsureV46(game,replay={}){if(!game||game.gameName!==AK_ALERT_GAME_V46)return;game.alertThemes=akAlertNormalizeV46(replay.alertThemes||game.alertThemes,AK_ALERT_THEMES_V46);game.alertIntensities=akAlertNormalizeV46(replay.alertIntensities||game.alertIntensities,AK_ALERT_INTENSITIES_V46);game.roundCount=Math.max(5,Math.min(50,Number(replay.roundCount||game.roundCount||12)));}
function akAlertToggleV46(game,key,value,list){const current=akAlertNormalizeV46(game[key],list);game[key]=current.includes(value)?current.filter(v=>v!==value):[...current,value];if(!game[key].length)game[key]=[value];}
function akAlertFilterMarkupV46(game,readOnly=false){akAlertEnsureV46(game);const dis=readOnly?'disabled':'';const rounds=[5,8,10,12,15,20,30,40,50];return `<section class="card alert-setup-v46"><div class="form-group"><label for="alertRoundsV46">Nombre de scénarios</label><select id="alertRoundsV46" class="text-input" ${dis}>${rounds.map(v=>`<option value="${v}" ${Number(game.roundCount)===v?'selected':''}>${v} scénario${v>1?'s':''}</option>`).join('')}</select></div><div class="alert-filter-head-v46"><strong>Univers</strong><button type="button" class="text-btn" data-alert-all-v46="themes" ${dis}>Tout sélectionner</button></div><div class="alert-filter-grid-v46">${AK_ALERT_THEMES_V46.map(([id,icon,label])=>`<button type="button" class="alert-filter-v46 ${game.alertThemes.includes(id)?'active':''}" data-alert-theme-v46="${id}" ${dis}><span>${icon}</span><strong>${label}</strong></button>`).join('')}</div><div class="alert-filter-head-v46"><strong>Intensité</strong><button type="button" class="text-btn" data-alert-all-v46="intensities" ${dis}>Tout sélectionner</button></div><div class="alert-filter-grid-v46 compact">${AK_ALERT_INTENSITIES_V46.map(([id,icon,label])=>`<button type="button" class="alert-filter-v46 ${game.alertIntensities.includes(id)?'active':''}" data-alert-intensity-v46="${id}" ${dis}><span>${icon}</span><strong>${label}</strong></button>`).join('')}</div></section>`;}
function akAlertBindSetupV46(game,rerender,readOnly=false){if(readOnly)return;document.querySelector('#alertRoundsV46')?.addEventListener('change',e=>game.roundCount=Number(e.target.value));document.querySelectorAll('[data-alert-theme-v46]').forEach(btn=>btn.addEventListener('click',()=>{akAlertToggleV46(game,'alertThemes',btn.dataset.alertThemeV46,AK_ALERT_THEMES_V46);rerender();}));document.querySelectorAll('[data-alert-intensity-v46]').forEach(btn=>btn.addEventListener('click',()=>{akAlertToggleV46(game,'alertIntensities',btn.dataset.alertIntensityV46,AK_ALERT_INTENSITIES_V46);rerender();}));document.querySelector('[data-alert-all-v46="themes"]')?.addEventListener('click',()=>{game.alertThemes=akAlertAllV46(AK_ALERT_THEMES_V46);rerender();});document.querySelector('[data-alert-all-v46="intensities"]')?.addEventListener('click',()=>{game.alertIntensities=akAlertAllV46(AK_ALERT_INTENSITIES_V46);rerender();});}
function akAlertBuildPoolV46(raw,game){akAlertEnsureV46(game);return (Array.isArray(raw)?raw:[]).filter(item=>game.alertThemes.includes(item.theme||'soiree_maison')&&game.alertIntensities.includes(item.intensity||'petit_probleme'));}
function akAlertMetaV46(item){const t=AK_ALERT_THEMES_V46.find(row=>row[0]===item?.theme)||AK_ALERT_THEMES_V46[0];const i=AK_ALERT_INTENSITIES_V46.find(row=>row[0]===item?.intensity)||AK_ALERT_INTENSITIES_V46[0];return `<div class="alert-meta-v46"><span>${t[1]} ${escapeHtml(t[2])}</span><span>${i[1]} ${escapeHtml(i[2])}</span></div>`;}
const akAlertBaseResetV46=resetMegaGame;resetMegaGame=function(gameName,replayConfig={}){akAlertBaseResetV46(gameName,replayConfig);if(gameName===AK_ALERT_GAME_V46)akAlertEnsureV46(state.megaGame,replayConfig);};
const akAlertBaseSetupV46=renderMegaSetup;renderMegaSetup=function(){const game=state.megaGame;if(!game||game.gameName!==AK_ALERT_GAME_V46)return akAlertBaseSetupV46();akAlertEnsureV46(game);clearV014Timer();title.textContent=AK_ALERT_GAME_V46;setBackVisible(true);screen.innerHTML=`<section class="game-cover game-cover-mega engine-scenario alert-cover-v46"><span class="game-cover-icon">🚨</span><div><small>500 MINI-AVENTURES À CHOIX</small><h2>Alerte Rouge</h2><p>Votez, appliquez la décision majoritaire et découvrez immédiatement sa conséquence.</p></div></section>${akAlertFilterMarkupV46(game,false)}<div class="notice">Chaque scénario possède trois choix et trois conséquences réellement liées à la situation.</div><button id="startMegaGame" class="primary-btn full">Déclencher l’alerte</button>`;akAlertBindSetupV46(game,renderMegaSetup,false);document.querySelector('#startMegaGame')?.addEventListener('click',startMegaGame);};
const akAlertBaseStartV46=startMegaGame;startMegaGame=async function(){const game=state.megaGame;if(!game||game.gameName!==AK_ALERT_GAME_V46)return akAlertBaseStartV46();akAlertEnsureV46(game);screen.innerHTML='<div class="notice">Préparation des scénarios…</div>';try{const raw=await loadJsonFile('data/alerte-rouge.json','Impossible de charger Alerte Rouge.');const pool=akAlertBuildPoolV46(raw,game);if(!pool.length)throw new Error('Aucun scénario ne correspond à ces filtres.');game.items=selectFreshItems(pool,Math.min(game.roundCount,pool.length),`solo:alert-v46:${game.alertThemes.join('-')}:${game.alertIntensities.join('-')}`);game.currentIndex=0;game.currentVoterIndex=0;game.votes={};game.scores=v014ScoreMap();game.rounds=[];game.currentResult=null;renderMegaCurrent();}catch(error){console.error(error);alert(error.message||'Impossible de lancer Alerte Rouge.');renderMegaSetup();}};
const akAlertBaseVoteV46=renderMegaScenarioVote;renderMegaScenarioVote=function(){const game=state.megaGame;if(!game||game.gameName!==AK_ALERT_GAME_V46)return akAlertBaseVoteV46();const item=game.items[game.currentIndex];screen.innerHTML=`${v014Progress(game,'Scénario')}${akAlertMetaV46(item)}<section class="scenario-card alert-card-v46"><span>🚨</span><small>${escapeHtml(item.title).toUpperCase()}</small><h2>${escapeHtml(item.text)}</h2></section><section class="mega-option-grid">${(item.options||[]).map((option,index)=>`<button class="mega-option-btn scenario-option" data-scenario-answer="${index}"><span>${index+1}</span><strong>${escapeHtml(option.label)}</strong></button>`).join('')}</section>`;document.querySelectorAll('[data-scenario-answer]').forEach(button=>button.addEventListener('click',()=>{const player=state.players[game.currentVoterIndex];game.votes[player.id]=Number(button.dataset.scenarioAnswer);game.currentVoterIndex+=1;renderMegaScenarioGate();}));};
const akAlertBaseRevealV46=renderMegaScenarioReveal;renderMegaScenarioReveal=function(){const game=state.megaGame;if(!game||game.gameName!==AK_ALERT_GAME_V46)return akAlertBaseRevealV46();const item=game.items[game.currentIndex],counts={};Object.values(game.votes).forEach(v=>counts[v]=Number(counts[v]||0)+1);const max=Math.max(...Object.values(counts),0),winners=Object.keys(counts).map(Number).filter(i=>counts[i]===max),chosen=winners[Math.floor(Math.random()*Math.max(1,winners.length))]??0;state.players.filter(p=>Number(game.votes[p.id])===chosen).forEach(p=>game.scores[p.id]=Number(game.scores[p.id]||0)+1);game.rounds.push({itemId:item.id,votes:{...game.votes},chosen});title.textContent='Conséquence';screen.innerHTML=`${v014Progress(game,'Scénario')}${akAlertMetaV46(item)}<section class="reveal-stage reveal-v07 scenario-reveal alert-reveal-v46"><span class="game-cover-icon">🚨</span><h2>${escapeHtml(item.options?.[chosen]?.label||'Décision prise')}</h2><p>${escapeHtml(item.options?.[chosen]?.outcome||'L’histoire continue.')}</p></section><section class="vote-distribution">${(item.options||[]).map((option,index)=>`<div class="${index===chosen?'winner':''}"><strong>${escapeHtml(option.label)}</strong><span>${Number(counts[index]||0)} vote${Number(counts[index]||0)>1?'s':''}</span></div>`).join('')}</section><button id="nextScenario" class="primary-btn full">${game.currentIndex+1>=game.items.length?'Voir le classement':'Scénario suivant'}</button>`;document.querySelector('#nextScenario')?.addEventListener('click',()=>{game.currentIndex+=1;game.currentVoterIndex=0;game.votes={};renderMegaCurrent();});};


/* AK'GAMES V4.7 — BLAGUES 800 + 350 ADULTES */
const AK_LAUGH_CLASSIC_CATEGORIES_V47 = [
  ["absurde","🌀","Absurde"],["quotidien","🛋️","Quotidien"],["travail_etudes","📚","Travail & études"],
  ["technologie","📱","Technologie"],["animaux","🐾","Animaux"],["nourriture","🍕","Nourriture"],
  ["famille_amis","🫂","Famille & amis"],["culture_pop","🎬","Culture pop"],["jeux_de_mots","🧠","Jeux de mots"],
  ["anti_blagues","🧱","Anti-blagues"]
];
const AK_LAUGH_ADULT_CATEGORIES_V47 = [
  ["couple","💞","Couple"],["rencontres","🥂","Rendez-vous"],["applis","📲","Applis de rencontre"],
  ["ex","🪦","Ex & ruptures"],["intimite","🌶️","Intimité"],["soirees","🍸","Soirées"],
  ["messages","💬","Messages compromettants"],["jalousie","👀","Jalousie"],["vie_commune","🧺","Vie à deux"],
  ["famille_indiscrete","🫣","Famille indiscrète"]
];
const AK_LAUGH_STYLES_V47 = [
  ["dialogue","💬","Dialogue"],["question_reponse","❓","Question-réponse"],["observation","👁️","Observation"],
  ["mini_histoire","📖","Mini-histoire"],["jeu_de_mots","🔤","Jeu de mots"],["anti_blague","🧱","Anti-blague"]
];
const AK_LAUGH_LEVELS_V47 = [["leger","🌶️","Léger"],["ose","🌶️🌶️","Osé"],["sans_filtre","🌶️🌶️🌶️","Sans filtre"]];
const AK_LAUGH_CUSTOM_KEY_V47 = "akgames_laugh_custom_v47";
const AK_LAUGH_PREFS_KEY_V47 = "akgames_laugh_prefs_v47";
function akLaughIdsV47(list){return list.map(r=>r[0]);}
function akLaughReadV47(key,fallback){try{const value=JSON.parse(localStorage.getItem(key)||"null");return value??fallback;}catch{return fallback;}}
function akLaughWriteV47(key,value){try{localStorage.setItem(key,JSON.stringify(value));}catch{}}
function akLaughNormalizeV47(value,list){const valid=new Set(akLaughIdsV47(list));const out=[...new Set((Array.isArray(value)?value:[]).filter(v=>valid.has(v)))];return out.length?out:akLaughIdsV47(list);}
function akLaughEnsureV47(game){
  const saved=akLaughReadV47(AK_LAUGH_PREFS_KEY_V47,{});
  game.gameFormat=game.gameFormat||saved.gameFormat||"duel";
  game.mode=game.mode||saved.mode||"sudden";
  game.categories=akLaughNormalizeV47(game.categories?.filter(v=>akLaughIdsV47(AK_LAUGH_CLASSIC_CATEGORIES_V47).includes(v))||saved.categories,AK_LAUGH_CLASSIC_CATEGORIES_V47);
  game.styles=akLaughNormalizeV47(game.styles||saved.styles,AK_LAUGH_STYLES_V47);
  game.includeAdult=Boolean(game.includeAdult??saved.includeAdult);
  game.adultCategories=akLaughNormalizeV47(game.adultCategories||saved.adultCategories,AK_LAUGH_ADULT_CATEGORIES_V47);
  game.adultLevels=akLaughNormalizeV47(game.adultLevels||saved.adultLevels,AK_LAUGH_LEVELS_V47);
  game.includeCustom=Boolean(game.includeCustom??saved.includeCustom??true);
  game.customJokes=Array.isArray(akLaughReadV47(AK_LAUGH_CUSTOM_KEY_V47,[]))?akLaughReadV47(AK_LAUGH_CUSTOM_KEY_V47,[]):[];
  game.tournamentPlayers=Array.isArray(game.tournamentPlayers)&&game.tournamentPlayers.length?game.tournamentPlayers:state.players.map(p=>p.id);
  game.skipCount=Number(game.skipCount||0);
  return game;
}
function akLaughSavePrefsV47(game){akLaughWriteV47(AK_LAUGH_PREFS_KEY_V47,{gameFormat:game.gameFormat,mode:game.mode,categories:game.categories,styles:game.styles,includeAdult:game.includeAdult,adultCategories:game.adultCategories,adultLevels:game.adultLevels,includeCustom:game.includeCustom});}
function akLaughLabelV47(joke){const list=joke?.adult?AK_LAUGH_ADULT_CATEGORIES_V47:AK_LAUGH_CLASSIC_CATEGORIES_V47;const row=list.find(r=>r[0]===joke?.category);const style=AK_LAUGH_STYLES_V47.find(r=>r[0]===joke?.style);return `${row?.[1]||"😂"} ${escapeHtml(row?.[2]||"Blague")}${style?` · ${style[1]} ${escapeHtml(style[2])}`:""}`;}
function akLaughToggleV47(game,key,value,list){const current=akLaughNormalizeV47(game[key],list);game[key]=current.includes(value)?current.filter(v=>v!==value):[...current,value];if(!game[key].length)game[key]=[value];}
function akLaughCustomMarkupV47(game){return `<section class="card laugh-custom-v47"><h2 class="section-title">Mes blagues</h2><p class="helper">Ajoute une chute personnelle au tirage de l’application. Le bouton « J’en ai une » reste disponible à chaque tour.</p><input id="laughCustomSetupV47" class="text-input" maxlength="180" placeholder="Début de la blague…"><input id="laughCustomPunchV47" class="text-input top-gap" maxlength="220" placeholder="Chute…"><div class="setup-grid-v44 top-gap"><select id="laughCustomCategoryV47" class="text-input">${AK_LAUGH_CLASSIC_CATEGORIES_V47.map(r=>`<option value="${r[0]}">${r[1]} ${r[2]}</option>`).join("")}</select><select id="laughCustomStyleV47" class="text-input">${AK_LAUGH_STYLES_V47.map(r=>`<option value="${r[0]}">${r[1]} ${r[2]}</option>`).join("")}</select></div><button id="laughAddCustomV47" class="secondary-btn full top-gap">Ajouter la blague</button><div class="custom-card-list-v44">${game.customJokes.map(j=>`<div><span>${escapeHtml(j.setup)}</span><button class="icon-btn" data-laugh-delete-v47="${escapeHtml(j.id)}">×</button></div>`).join("")||`<p class="helper">Aucune blague personnelle enregistrée.</p>`}</div></section>`;}

function resetLaughDuelState(){
  const previous=state.laughDuel||{};
  state.laughDuel={player1Id:state.players[0]?.id||null,player2Id:state.players[1]?.id||null,mode:"sudden",gameFormat:"duel",categories:akLaughIdsV47(AK_LAUGH_CLASSIC_CATEGORIES_V47),styles:akLaughIdsV47(AK_LAUGH_STYLES_V47),includeAdult:false,adultCategories:akLaughIdsV47(AK_LAUGH_ADULT_CATEGORIES_V47),adultLevels:akLaughIdsV47(AK_LAUGH_LEVELS_V47),includeCustom:true,jokePool:[],usedJokeIds:[],currentTurnId:null,currentJoke:null,punchlineVisible:false,lives:{},tournamentPlayers:state.players.map(p=>p.id),tournamentRoundPlayers:[],tournamentNextRound:[],tournamentMatchIndex:0,tournamentRound:1,tournamentChampionId:null,skipCount:0,...previous};
  akLaughEnsureV47(state.laughDuel);
}
function akLaughSetupFiltersV47(game,list,key,title,attr){return `<section class="card"><div class="section-heading-row"><div><h2 class="section-title">${title}</h2></div><button class="text-btn" data-laugh-all-v47="${key}">Tout sélectionner</button></div><div class="laugh-filter-grid-v47">${list.map(([id,icon,label])=>`<button type="button" class="laugh-filter-v47 ${game[key].includes(id)?"active":""}" ${attr}="${id}"><span>${icon}</span><strong>${label}</strong></button>`).join("")}</div></section>`;}
function renderLaughDuelSetup(){
  if(!state.laughDuel)resetLaughDuelState();const game=akLaughEnsureV47(state.laughDuel);title.textContent="Le premier qui rit a perdu";setBackVisible(true);
  const tournamentAllowed=state.players.length>=3;
  screen.innerHTML=`<section class="game-cover laugh-cover-v47"><span class="game-cover-icon">😂</span><div><small>800 BLAGUES · 350 ADULTES</small><h2>Le premier qui rit a perdu</h2><p>Face à face, chute masquée et dignité fragile.</p></div></section>
  <section class="card"><h2 class="section-title">Format</h2><div class="stacked-choice"><label class="option-card mini-option"><input type="radio" name="laughFormatV47" value="duel" ${game.gameFormat==="duel"?"checked":""}><span><strong>⚔️ Duel</strong><br><span class="helper">Deux joueurs face à face.</span></span></label><label class="option-card mini-option ${tournamentAllowed?"":"disabled"}"><input type="radio" name="laughFormatV47" value="tournament" ${game.gameFormat==="tournament"?"checked":""} ${tournamentAllowed?"":"disabled"}><span><strong>🏆 Tournoi</strong><br><span class="helper">Tous les joueurs s’affrontent jusqu’à la finale.</span></span></label></div></section>
  ${game.gameFormat==="duel"?`<section class="card"><h2 class="section-title">Adversaires</h2><div class="duel-player-select"><div><label class="helper" for="laughPlayer1">Joueur 1</label><select id="laughPlayer1" class="text-input">${state.players.map(p=>`<option value="${p.id}" ${game.player1Id===p.id?"selected":""}>${escapeHtml(p.name)}</option>`).join("")}</select></div><div class="duel-vs">VS</div><div><label class="helper" for="laughPlayer2">Joueur 2</label><select id="laughPlayer2" class="text-input">${state.players.map(p=>`<option value="${p.id}" ${game.player2Id===p.id?"selected":""}>${escapeHtml(p.name)}</option>`).join("")}</select></div></div></section>`:`<section class="card"><h2 class="section-title">Participants au tournoi</h2><div class="laugh-player-grid-v47">${state.players.map(p=>`<label class="form-check"><input class="form-check-input" type="checkbox" data-laugh-player-v47="${p.id}" ${game.tournamentPlayers.includes(p.id)?"checked":""}><span class="form-check-label">${avatarById(p.avatarId).emoji} ${escapeHtml(p.name)}</span></label>`).join("")}</div></section>`}
  <section class="card"><h2 class="section-title">Règle du duel</h2><div class="stacked-choice"><label class="option-card mini-option"><input type="radio" name="laughMode" value="sudden" ${game.mode==="sudden"?"checked":""}><span><strong>⚡ Mort subite</strong><br><span class="helper">Le premier rire perd le duel.</span></span></label><label class="option-card mini-option"><input type="radio" name="laughMode" value="lives" ${game.mode==="lives"?"checked":""}><span><strong>❤️ 3 vies</strong><br><span class="helper">Chaque rire retire une vie.</span></span></label></div></section>
  ${akLaughSetupFiltersV47(game,AK_LAUGH_CLASSIC_CATEGORIES_V47,"categories","Univers classiques","data-laugh-cat-v47")}
  ${akLaughSetupFiltersV47(game,AK_LAUGH_STYLES_V47,"styles","Styles d’humour","data-laugh-style-v47")}
  ${state.adult?`<section class="card"><label class="option-card"><input id="laughAdultToggle" type="checkbox" ${game.includeAdult?"checked":""}><span><strong>🔞 Ajouter les 350 blagues adultes</strong><br><span class="helper">Couple, rendez-vous, ex, intimité, soirées et messages compromettants.</span></span></label></section>${game.includeAdult?akLaughSetupFiltersV47(game,AK_LAUGH_ADULT_CATEGORIES_V47,"adultCategories","Thèmes adultes","data-laugh-adult-cat-v47")+akLaughSetupFiltersV47(game,AK_LAUGH_LEVELS_V47,"adultLevels","Intensité adulte","data-laugh-level-v47"):""}`:""}
  <section class="card"><label class="form-check form-switch"><input id="laughIncludeCustomV47" class="form-check-input" type="checkbox" ${game.includeCustom?"checked":""}><span class="form-check-label">✨ Mélanger mes blagues enregistrées</span></label></section>${akLaughCustomMarkupV47(game)}
  <button id="startLaughDuel" class="primary-btn full">${game.gameFormat==="tournament"?"Lancer le tournoi":"Lancer le duel"}</button>`;
  document.querySelectorAll('input[name="laughFormatV47"]').forEach(el=>el.addEventListener('change',e=>{game.gameFormat=e.target.value;renderLaughDuelSetup();}));
  document.querySelector('#laughPlayer1')?.addEventListener('change',e=>game.player1Id=e.target.value);document.querySelector('#laughPlayer2')?.addEventListener('change',e=>game.player2Id=e.target.value);
  document.querySelectorAll('[data-laugh-player-v47]').forEach(el=>el.addEventListener('change',()=>{const id=el.dataset.laughPlayerV47;game.tournamentPlayers=el.checked?[...new Set([...game.tournamentPlayers,id])]:game.tournamentPlayers.filter(v=>v!==id);}));
  document.querySelectorAll('input[name="laughMode"]').forEach(el=>el.addEventListener('change',e=>game.mode=e.target.value));
  [['data-laugh-cat-v47','categories',AK_LAUGH_CLASSIC_CATEGORIES_V47],['data-laugh-style-v47','styles',AK_LAUGH_STYLES_V47],['data-laugh-adult-cat-v47','adultCategories',AK_LAUGH_ADULT_CATEGORIES_V47],['data-laugh-level-v47','adultLevels',AK_LAUGH_LEVELS_V47]].forEach(([attr,key,list])=>document.querySelectorAll(`[${attr}]`).forEach(btn=>btn.addEventListener('click',()=>{akLaughToggleV47(game,key,btn.getAttribute(attr),list);renderLaughDuelSetup();})));
  document.querySelectorAll('[data-laugh-all-v47]').forEach(btn=>btn.addEventListener('click',()=>{const key=btn.dataset.laughAllV47;const list=key==='categories'?AK_LAUGH_CLASSIC_CATEGORIES_V47:key==='styles'?AK_LAUGH_STYLES_V47:key==='adultCategories'?AK_LAUGH_ADULT_CATEGORIES_V47:AK_LAUGH_LEVELS_V47;game[key]=akLaughIdsV47(list);renderLaughDuelSetup();}));
  document.querySelector('#laughAdultToggle')?.addEventListener('change',e=>{game.includeAdult=e.target.checked;renderLaughDuelSetup();});document.querySelector('#laughIncludeCustomV47')?.addEventListener('change',e=>game.includeCustom=e.target.checked);
  document.querySelector('#laughAddCustomV47')?.addEventListener('click',()=>{const setup=document.querySelector('#laughCustomSetupV47').value.trim(),punchline=document.querySelector('#laughCustomPunchV47').value.trim();if(!setup||!punchline)return alert('Écris le début et la chute.');game.customJokes.push({id:`custom_${Date.now()}`,setup,punchline,category:document.querySelector('#laughCustomCategoryV47').value,style:document.querySelector('#laughCustomStyleV47').value,adult:false,custom:true});akLaughWriteV47(AK_LAUGH_CUSTOM_KEY_V47,game.customJokes);renderLaughDuelSetup();});
  document.querySelectorAll('[data-laugh-delete-v47]').forEach(btn=>btn.addEventListener('click',()=>{game.customJokes=game.customJokes.filter(j=>j.id!==btn.dataset.laughDeleteV47);akLaughWriteV47(AK_LAUGH_CUSTOM_KEY_V47,game.customJokes);renderLaughDuelSetup();}));
  document.querySelector('#startLaughDuel')?.addEventListener('click',startLaughDuel);
}
function akLaughBuildPoolV47(classicRows,adultRows,game){let pool=(classicRows||[]).filter(j=>game.categories.includes(j.category)&&game.styles.includes(j.style));if(state.adult&&game.includeAdult)pool=pool.concat((adultRows||[]).filter(j=>game.adultCategories.includes(j.category)&&game.adultLevels.includes(j.intensity)&&game.styles.includes(j.style)));if(game.includeCustom)pool=pool.concat(game.customJokes.filter(j=>game.categories.includes(j.category)&&game.styles.includes(j.style)));return pool;}
function akLaughInitMatchV47(game,p1,p2){game.player1Id=p1;game.player2Id=p2;game.currentTurnId=Math.random()<.5?p1:p2;game.currentJoke=null;game.punchlineVisible=false;game.lives={[p1]:game.mode==='lives'?3:1,[p2]:game.mode==='lives'?3:1};game.skipCount=0;}
function akLaughInitTournamentV47(game){let ids=shuffleArray(game.tournamentPlayers.filter(id=>state.players.some(p=>p.id===id)));game.tournamentRound=1;game.tournamentRoundPlayers=ids;game.tournamentNextRound=[];game.tournamentMatchIndex=0;game.tournamentChampionId=null;if(ids.length%2===1)game.tournamentNextRound.push(ids[ids.length-1]);akLaughInitMatchV47(game,ids[0],ids[1]);}
async function startLaughDuel(){const game=akLaughEnsureV47(state.laughDuel);if(game.gameFormat==='duel'&&(!game.player1Id||!game.player2Id||game.player1Id===game.player2Id))return alert('Choisis deux joueurs différents.');if(game.gameFormat==='tournament'&&game.tournamentPlayers.length<3)return alert('Choisis au moins trois participants.');akLaughSavePrefsV47(game);screen.innerHTML='<div class="notice">Sélection des meilleures chutes…</div>';try{const [classicRows,adultRows]=await Promise.all([loadJsonFile('data/blagues.json','Impossible de charger les blagues.'),state.adult&&game.includeAdult?loadJsonFile('data/blagues-adulte.json','Impossible de charger les blagues adultes.'):Promise.resolve([])]);const pool=akLaughBuildPoolV47(classicRows,adultRows,game);if(!pool.length)throw new Error('Aucune blague ne correspond à ces filtres.');game.jokePool=shuffleArray(pool);game.usedJokeIds=[];if(game.gameFormat==='tournament')akLaughInitTournamentV47(game);else akLaughInitMatchV47(game,game.player1Id,game.player2Id);state.history=[];renderLaughDuelIntro();}catch(error){console.error(error);alert(error.message||'Impossible de lancer le jeu.');renderLaughDuelSetup();}}
function renderLaughJokeCard(){const game=state.laughDuel,teller=state.players.find(p=>p.id===game.currentTurnId),listener=getOtherLaughPlayer(teller.id),joke=game.currentJoke;title.textContent='Fais-le/la craquer';setBackVisible(false);screen.innerHTML=`${renderLaughLives()}${game.gameFormat==='tournament'?`<div class="laugh-tournament-chip-v47">🏆 Tour ${game.tournamentRound} · Duel ${Math.floor(game.tournamentMatchIndex/2)+1}</div>`:''}<section class="joke-card"><span class="category-chip">${akLaughLabelV47(joke)}</span><h2>${escapeHtml(joke.setup)}</h2>${game.punchlineVisible?`<div class="punchline">${escapeHtml(joke.punchline)}</div>`:`<button id="revealPunchline" class="secondary-btn">Révéler la chute</button>`}</section>${game.punchlineVisible?`<button id="skipLaughJokeV47" class="text-btn full">Cette blague ne me parle pas, en tirer une autre</button>${renderLaughOutcomeButtons(teller,listener)}`:''}`;document.querySelector('#revealPunchline')?.addEventListener('click',()=>{game.punchlineVisible=true;renderLaughJokeCard();});document.querySelector('#skipLaughJokeV47')?.addEventListener('click',()=>{game.skipCount+=1;drawLaughJoke();});bindLaughOutcomeButtons();}
function akLaughAdvanceTournamentV47(winnerId){const game=state.laughDuel;game.tournamentNextRound.push(winnerId);const nextIndex=game.tournamentMatchIndex+2;if(nextIndex+1<game.tournamentRoundPlayers.length){game.tournamentMatchIndex=nextIndex;akLaughInitMatchV47(game,game.tournamentRoundPlayers[nextIndex],game.tournamentRoundPlayers[nextIndex+1]);return 'next-match';}const next=[...new Set(game.tournamentNextRound)];if(next.length===1){game.tournamentChampionId=next[0];return 'champion';}game.tournamentRound+=1;game.tournamentRoundPlayers=shuffleArray(next);game.tournamentNextRound=[];game.tournamentMatchIndex=0;if(game.tournamentRoundPlayers.length%2===1)game.tournamentNextRound.push(game.tournamentRoundPlayers[game.tournamentRoundPlayers.length-1]);akLaughInitMatchV47(game,game.tournamentRoundPlayers[0],game.tournamentRoundPlayers[1]);return 'next-round';}
function renderLaughDuelEnd(winner,loser){const game=state.laughDuel;title.textContent=game.gameFormat==='tournament'?'Résultat du duel':'Fin du duel';setBackVisible(false);screen.innerHTML=`<section class="winner-stage"><div class="winner-crown">👑</div><div class="giant-avatar">${avatarById(winner.avatarId).emoji}</div><h2>${escapeHtml(winner.name)} gagne${game.gameFormat==='tournament'?' ce duel':' !'}</h2><p>${escapeHtml(loser.name)} a craqué en premier.${game.skipCount?` ${game.skipCount} blague${game.skipCount>1?'s':''} remplacée${game.skipCount>1?'s':''}.`:''}</p></section>${game.gameFormat==='tournament'?`<button id="laughTournamentNextV47" class="primary-btn full">Continuer le tournoi</button>`:`<div class="toolbar"><button id="laughRematch" class="secondary-btn">Revanche</button><button id="laughOtherGame" class="primary-btn">Choisir un autre jeu</button></div>`}`;
  document.querySelector('#laughTournamentNextV47')?.addEventListener('click',()=>{const result=akLaughAdvanceTournamentV47(winner.id);if(result==='champion'){const champion=state.players.find(p=>p.id===game.tournamentChampionId);title.textContent='Champion du tournoi';screen.innerHTML=`<section class="winner-stage laugh-champion-v47"><div class="winner-crown">🏆</div><div class="giant-avatar">${avatarById(champion.avatarId).emoji}</div><h2>${escapeHtml(champion.name)} n’a presque pas ri !</h2><p>Champion officiel du visage impassible.</p></section><div class="toolbar"><button id="laughTournamentAgainV47" class="secondary-btn">Nouveau tournoi</button><button id="laughTournamentExitV47" class="primary-btn">Autre jeu</button></div>`;document.querySelector('#laughTournamentAgainV47')?.addEventListener('click',()=>{resetLaughDuelState();state.laughDuel.gameFormat='tournament';renderLaughDuelSetup();});document.querySelector('#laughTournamentExitV47')?.addEventListener('click',()=>{state.laughDuel=null;renderPlayChoice();});}else renderLaughDuelIntro();});
  document.querySelector('#laughRematch')?.addEventListener('click',()=>{akLaughInitMatchV47(game,game.player1Id,game.player2Id);game.usedJokeIds=[];renderLaughDuelIntro();});document.querySelector('#laughOtherGame')?.addEventListener('click',()=>{state.laughDuel=null;renderPlayChoice();});
}
